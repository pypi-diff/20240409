# Comparing `tmp/django_prose_editor-0.2.1.tar.gz` & `tmp/django_prose_editor-0.3.0.tar.gz`

## Comparing `django_prose_editor-0.2.1.tar` & `django_prose_editor-0.3.0.tar`

### file list

```diff
@@ -1,15 +1,15 @@
--rw-r--r--   0        0        0        5 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/.gitignore
--rw-r--r--   0        0        0       18 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/__init__.py
--rw-r--r--   0        0        0     1499 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/fields.py
--rw-r--r--   0        0        0      674 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/sanitized.py
--rw-r--r--   0        0        0      547 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/widgets.py
--rw-r--r--   0        0        0     6230 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/editor.css
--rw-r--r--   0        0        0   223050 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/editor.js
--rw-r--r--   0        0        0      635 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/init.js
--rw-r--r--   0        0        0      624 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/material-icons.css
--rw-r--r--   0        0        0   128616 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/material-icons.woff2
--rw-r--r--   0        0        0      204 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/.gitignore
--rw-r--r--   0        0        0     1519 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/LICENSE
--rw-r--r--   0        0        0     3883 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/README.rst
--rw-r--r--   0        0        0     2262 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/pyproject.toml
--rw-r--r--   0        0        0     5023 2020-02-02 00:00:00.000000 django_prose_editor-0.2.1/PKG-INFO
+-rw-r--r--   0        0        0        5 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/.gitignore
+-rw-r--r--   0        0        0       18 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/__init__.py
+-rw-r--r--   0        0        0     1647 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/fields.py
+-rw-r--r--   0        0        0      303 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/sanitized.py
+-rw-r--r--   0        0        0      771 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/widgets.py
+-rw-r--r--   0        0        0     6245 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/editor.css
+-rw-r--r--   0        0        0   224395 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/editor.js
+-rw-r--r--   0        0        0      672 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/init.js
+-rw-r--r--   0        0        0      624 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/material-icons.css
+-rw-r--r--   0        0        0   128616 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/material-icons.woff2
+-rw-r--r--   0        0        0      204 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/.gitignore
+-rw-r--r--   0        0        0     1519 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/LICENSE
+-rw-r--r--   0        0        0     4358 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/README.rst
+-rw-r--r--   0        0        0     2251 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/pyproject.toml
+-rw-r--r--   0        0        0     5487 2020-02-02 00:00:00.000000 django_prose_editor-0.3.0/PKG-INFO
```

### Comparing `django_prose_editor-0.2.1/django_prose_editor/fields.py` & `django_prose_editor-0.3.0/django_prose_editor/fields.py`

 * *Files 8% similar despite different names*

```diff
@@ -9,14 +9,15 @@
 def _identity(x):
     return x
 
 
 class ProseEditorField(models.TextField):
     def __init__(self, *args, **kwargs):
         self.sanitize = kwargs.pop("sanitize", _identity)
+        self.config = kwargs.pop("config", {})
         super().__init__(*args, **kwargs)
 
     def clean(self, value, instance):
         return self.sanitize(super().clean(value, instance))
 
     def contribute_to_class(self, cls, name, **kwargs):
         """Add a ``get_*_excerpt`` method to models which returns a
@@ -31,17 +32,19 @@
         )
 
     def deconstruct(self):
         name, _path, args, kwargs = super().deconstruct()
         return (name, "django.db.models.TextField", args, kwargs)
 
     def formfield(self, **kwargs):
-        defaults = {"form_class": ProseEditorFormField} | kwargs
+        defaults = {"config": self.config, "form_class": ProseEditorFormField} | kwargs
         return super().formfield(**defaults)
 
 
 class ProseEditorFormField(forms.CharField):
     widget = ProseEditorWidget
 
     def __init__(self, *args, **kwargs):
+        config = kwargs.pop("config", {})
         kwargs["widget"] = ProseEditorWidget
         super().__init__(*args, **kwargs)
+        self.widget.config = config
```

### Comparing `django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/editor.css` & `django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/editor.css`

 * *Files 1% similar despite different names*

```diff
@@ -1 +1 @@
-.ProseMirror{color:#000!important;padding:1rem 1.75rem;outline:none!important;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Open Sans,Helvetica Neue,sans-serif;margin:0!important}.ProseMirror *{color:var(--body-fg)!important}.ProseMirror p{font-size:15px!important}.ProseMirror h1{display:block!important;font-size:2em!important;margin-block-start:.67em!important;margin-block-end:.67em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important}.ProseMirror h2{padding:0;display:block!important;font-size:1.5em!important;margin-block-start:.83em!important;margin-block-end:.83em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important;background:#fff!important}.ProseMirror h3{display:block!important;font-size:1.17em!important;margin-block-start:1em!important;margin-block-end:1em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important;padding:0!important}.ProseMirror>*+*{margin-top:.75em!important}.ProseMirror p.is-editor-empty:first-child:before{content:attr(data-placeholder);float:left;color:#a8a8a8;pointer-events:none;height:0}.ProseMirror ol{padding-left:0!important;list-style-type:decimal!important}.ProseMirror ol li::marker{font-size:16px!important}.ProseMirror ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:lower-latin!important}.ProseMirror ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:lower-roman!important}.ProseMirror ol>li>ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:upper-latin!important}.ProseMirror ol>li>ol>li>ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:upper-roman!important}.ProseMirror ul{list-style-type:disc!important;margin-block-start:0!important;margin-block-end:0!important;margin-inline-start:0px!important;margin-inline-end:0px!important}.ProseMirror ul li{list-style:disc!important}.ProseMirror ul>li>ul{padding-left:10px!important;margin-left:10px!important;list-style-type:disc!important}.ProseMirror{margin:1rem 0}.ProseMirror h1,.ProseMirror h2,.ProseMirror h3,.ProseMirror h4,.ProseMirror h5,.ProseMirror h6{line-height:1.1;text-transform:none;padding:0;background:none!important;border:none!important}.ProseMirror pre{background:#0d0d0d!important;color:#fff!important;font-family:JetBrainsMono,monospace!important;padding:.75rem 1rem!important;border-radius:.5rem!important}.ProseMirror pre code{color:#fff!important;padding:0!important;background:none!important;font-size:.8rem!important}.ProseMirror img{max-width:100%;height:auto}.ProseMirror blockquote{padding-left:1rem;border-left:2px solid rgba(13,13,13,.1)}.ProseMirror hr{border:none;border-top:2px solid rgba(13,13,13,.1);margin:2rem 0!important}.ProseMirror a{text-decoration:underline}.ProseMirror table{border-collapse:collapse;table-layout:fixed;width:100%;margin:0;overflow:hidden}.ProseMirror table td,.ProseMirror table th,.ProseMirror table[show_borders=false]:hover td,.ProseMirror table[show_borders=false]:hover th{min-width:1em;border:2px solid #ced4da;padding:3px 5px;vertical-align:top;box-sizing:border-box;position:relative}.ProseMirror table[show_borders=false] td,.ProseMirror table[show_borders=false] th{border:none;box-sizing:border-box}.ProseMirror table td>*,.ProseMirror table th>*{margin-bottom:0}.ProseMirror table th{font-weight:700;text-align:left;background-color:#f1f3f5}.ProseMirror tr{background-color:#fff!important}.ProseMirror table .selectedCell:after{z-index:2;position:absolute;content:"";inset:0;background:#c8c8ff66;pointer-events:none}.ProseMirror table .column-resize-handle{position:absolute;right:-2px;top:0;bottom:-2px;width:4px;background-color:#adf;pointer-events:none}.tableWrapper{overflow-x:auto}.resize-cursor{cursor:ew-resize;cursor:col-resize}.ProseMirror{padding:0 6px;border:1px solid var(--border-color);border-radius:4px}.prose-editor *,.prose-editor *:before,.prose-editor *:after{box-sizing:inherit}.prose-editor{box-sizing:border-box;position:relative;max-width:120ch;flex-grow:1}.prose-editor+textarea{display:none}.prose-menubar{font-size:14px;background:var(--body-bg);display:inline-flex;gap:8px;flex-wrap:wrap;margin-bottom:2px}.prose-menubar__group{display:flex}.prose-menubar__button{cursor:pointer;padding:0 .25em;min-width:2em;transition:all .25s;border:1px solid var(--border-color);text-align:center;display:inline-flex;position:relative}.prose-menubar__button--heading .level{position:absolute;right:3px;bottom:2px;font-size:80%}.prose-menubar__button:first-child{border-top-left-radius:4px;border-bottom-left-radius:4px}.prose-menubar__button:last-child{border-top-right-radius:4px;border-bottom-right-radius:4px}.prose-menubar__button+.prose-menubar__button{border-left:none}.prose-menubar__button.material-icons{padding:0 .125em;min-width:auto}.prose-menubar__button:hover{filter:brightness(110%)}.prose-menubar__button.active{background-color:var(--primary)}.prose-menubar__button.disabled{background:var(--border-color);filter:brightness(100%);cursor:not-allowed;opacity:.3}.prose-editor-dialog{background:var(--body-bg);color:var(--body-fg);border:1px solid var(--border-color)}.prose-editor-dialog label{display:block}label:empty:has(+.prose-editor){display:none}.ProseMirror{position:relative}.ProseMirror{word-wrap:break-word;white-space:pre-wrap;white-space:break-spaces;font-variant-ligatures:none;font-feature-settings:"liga" 0}.ProseMirror pre{white-space:pre-wrap}.ProseMirror li{position:relative}.ProseMirror-hideselection *::selection{background:transparent}.ProseMirror-hideselection *::-moz-selection{background:transparent}.ProseMirror-hideselection{caret-color:transparent}.ProseMirror [draggable][contenteditable=false]{-webkit-user-select:text;-moz-user-select:text;user-select:text}.ProseMirror-selectednode{outline:2px solid #8cf}li.ProseMirror-selectednode{outline:none}li.ProseMirror-selectednode:after{content:"";position:absolute;inset:-2px -2px -2px -32px;border:2px solid #8cf;pointer-events:none}img.ProseMirror-separator{display:inline!important;border:none!important;margin:0!important}
+.ProseMirror{color:#000!important;padding:1rem 1.75rem;outline:none!important;font-family:-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Oxygen,Ubuntu,Cantarell,Open Sans,Helvetica Neue,sans-serif;margin:0!important}.ProseMirror *{color:var(--body-fg)!important}.ProseMirror p{font-size:15px!important}.ProseMirror h1{display:block!important;font-size:2em!important;margin-block-start:.67em!important;margin-block-end:.67em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important}.ProseMirror h2{padding:0;display:block!important;font-size:1.5em!important;margin-block-start:.83em!important;margin-block-end:.83em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important;background:#fff!important}.ProseMirror h3{display:block!important;font-size:1.17em!important;margin-block-start:1em!important;margin-block-end:1em!important;margin-inline-start:0px!important;margin-inline-end:0px!important;font-weight:700!important;padding:0!important}.ProseMirror>*+*{margin-top:.75em!important}.ProseMirror p.is-editor-empty:first-child:before{content:attr(data-placeholder);float:left;color:#a8a8a8;pointer-events:none;height:0}.ProseMirror ol{padding-left:0!important;list-style-type:decimal!important}.ProseMirror ol li::marker{font-size:16px!important}.ProseMirror ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:lower-latin!important}.ProseMirror ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:lower-roman!important}.ProseMirror ol>li>ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:upper-latin!important}.ProseMirror ol>li>ol>li>ol>li>ol>li>ol{padding-left:10px!important;margin-left:10px!important;list-style-type:upper-roman!important}.ProseMirror ul{list-style-type:disc!important;margin-block-start:0!important;margin-block-end:0!important;margin-inline-start:0px!important;margin-inline-end:0px!important}.ProseMirror ul li{list-style:disc!important}.ProseMirror ul>li>ul{padding-left:10px!important;margin-left:10px!important;list-style-type:disc!important}.ProseMirror{margin:1rem 0}.ProseMirror h1,.ProseMirror h2,.ProseMirror h3,.ProseMirror h4,.ProseMirror h5,.ProseMirror h6{line-height:1.1;text-transform:none;padding:0;background:none!important;border:none!important}.ProseMirror pre{background:#0d0d0d!important;color:#fff!important;font-family:JetBrainsMono,monospace!important;padding:.75rem 1rem!important;border-radius:.5rem!important}.ProseMirror pre code{color:#fff!important;padding:0!important;background:none!important;font-size:.8rem!important}.ProseMirror img{max-width:100%;height:auto}.ProseMirror blockquote{padding-left:1rem;border-left:2px solid rgba(13,13,13,.1)}.ProseMirror hr{border:none;border-top:2px solid rgba(13,13,13,.1);margin:2rem 0!important}.ProseMirror a{text-decoration:underline}.ProseMirror table{border-collapse:collapse;table-layout:fixed;width:100%;margin:0;overflow:hidden}.ProseMirror table td,.ProseMirror table th,.ProseMirror table[show_borders=false]:hover td,.ProseMirror table[show_borders=false]:hover th{min-width:1em;border:2px solid #ced4da;padding:3px 5px;vertical-align:top;box-sizing:border-box;position:relative}.ProseMirror table[show_borders=false] td,.ProseMirror table[show_borders=false] th{border:none;box-sizing:border-box}.ProseMirror table td>*,.ProseMirror table th>*{margin-bottom:0}.ProseMirror table th{font-weight:700;text-align:left;background-color:#f1f3f5}.ProseMirror tr{background-color:#fff!important}.ProseMirror table .selectedCell:after{z-index:2;position:absolute;content:"";inset:0;background:#c8c8ff66;pointer-events:none}.ProseMirror table .column-resize-handle{position:absolute;right:-2px;top:0;bottom:-2px;width:4px;background-color:#adf;pointer-events:none}.tableWrapper{overflow-x:auto}.resize-cursor{cursor:ew-resize;cursor:col-resize}.ProseMirror{padding:0 6px;border:1px solid var(--border-color);border-radius:4px}.prose-editor *,.prose-editor *:before,.prose-editor *:after{box-sizing:inherit}.prose-editor{box-sizing:border-box;position:relative;max-width:120ch;min-width:40ch;flex-grow:1}.prose-editor+textarea{display:none}.prose-menubar{font-size:14px;background:var(--body-bg);display:inline-flex;gap:8px;flex-wrap:wrap;margin-bottom:2px}.prose-menubar__group{display:flex}.prose-menubar__button{cursor:pointer;padding:0 .25em;min-width:2em;transition:all .25s;border:1px solid var(--border-color);text-align:center;display:inline-flex;position:relative}.prose-menubar__button--heading .level{position:absolute;right:3px;bottom:2px;font-size:80%}.prose-menubar__button:first-child{border-top-left-radius:4px;border-bottom-left-radius:4px}.prose-menubar__button:last-child{border-top-right-radius:4px;border-bottom-right-radius:4px}.prose-menubar__button+.prose-menubar__button{border-left:none}.prose-menubar__button.material-icons{padding:0 .125em;min-width:auto}.prose-menubar__button:hover{filter:brightness(110%)}.prose-menubar__button.active{background-color:var(--primary)}.prose-menubar__button.disabled{background:var(--border-color);filter:brightness(100%);cursor:not-allowed;opacity:.3}.prose-editor-dialog{background:var(--body-bg);color:var(--body-fg);border:1px solid var(--border-color)}.prose-editor-dialog label{display:block}label:empty:has(+.prose-editor){display:none}.ProseMirror{position:relative}.ProseMirror{word-wrap:break-word;white-space:pre-wrap;white-space:break-spaces;font-variant-ligatures:none;font-feature-settings:"liga" 0}.ProseMirror pre{white-space:pre-wrap}.ProseMirror li{position:relative}.ProseMirror-hideselection *::selection{background:transparent}.ProseMirror-hideselection *::-moz-selection{background:transparent}.ProseMirror-hideselection{caret-color:transparent}.ProseMirror [draggable][contenteditable=false]{-webkit-user-select:text;-moz-user-select:text;user-select:text}.ProseMirror-selectednode{outline:2px solid #8cf}li.ProseMirror-selectednode{outline:none}li.ProseMirror-selectednode:after{content:"";position:absolute;inset:-2px -2px -2px -32px;border:2px solid #8cf;pointer-events:none}img.ProseMirror-separator{display:inline!important;border:none!important;margin:0!important}
```

### Comparing `django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/editor.js` & `django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/editor.js`

 * *Files 1% similar despite different names*

#### js-beautify {}

```diff
@@ -1,46 +1,46 @@
 var DjangoProseEditor = (() => {
-    var Rt = Object.defineProperty;
-    var zs = Object.getOwnPropertyDescriptor;
-    var Bs = Object.getOwnPropertyNames,
-        Br = Object.getOwnPropertySymbols;
-    var Vr = Object.prototype.hasOwnProperty,
-        Fs = Object.prototype.propertyIsEnumerable;
-    var Fr = (n, e, t) => e in n ? Rt(n, e, {
+    var zt = Object.defineProperty;
+    var Fs = Object.getOwnPropertyDescriptor;
+    var Vs = Object.getOwnPropertyNames,
+        Vr = Object.getOwnPropertySymbols;
+    var Jr = Object.prototype.hasOwnProperty,
+        Ls = Object.prototype.propertyIsEnumerable;
+    var Lr = (n, e, t) => e in n ? zt(n, e, {
             enumerable: !0,
             configurable: !0,
             writable: !0,
             value: t
         }) : n[e] = t,
-        Sn = (n, e) => {
-            for (var t in e || (e = {})) Vr.call(e, t) && Fr(n, t, e[t]);
-            if (Br)
-                for (var t of Br(e)) Fs.call(e, t) && Fr(n, t, e[t]);
+        Cn = (n, e) => {
+            for (var t in e || (e = {})) Jr.call(e, t) && Lr(n, t, e[t]);
+            if (Vr)
+                for (var t of Vr(e)) Ls.call(e, t) && Lr(n, t, e[t]);
             return n
         };
-    var Vs = (n, e) => {
-            for (var t in e) Rt(n, t, {
+    var Js = (n, e) => {
+            for (var t in e) zt(n, t, {
                 get: e[t],
                 enumerable: !0
             })
         },
-        Ls = (n, e, t, r) => {
+        Ws = (n, e, t, r) => {
             if (e && typeof e == "object" || typeof e == "function")
-                for (let i of Bs(e)) !Vr.call(n, i) && i !== t && Rt(n, i, {
+                for (let i of Vs(e)) !Jr.call(n, i) && i !== t && zt(n, i, {
                     get: () => e[i],
-                    enumerable: !(r = zs(e, i)) || r.enumerable
+                    enumerable: !(r = Fs(e, i)) || r.enumerable
                 });
             return n
         };
-    var Js = n => Ls(Rt({}, "__esModule", {
+    var qs = n => Ws(zt({}, "__esModule", {
         value: !0
     }), n);
-    var Vf = {};
-    Vs(Vf, {
-        createEditor: () => Ps
+    var _f = {};
+    Js(_f, {
+        createEditor: () => zs
     });
 
     function v(n) {
         this.content = n
     }
     v.prototype = {
         constructor: v,
@@ -106,39 +106,39 @@
     v.from = function(n) {
         if (n instanceof v) return n;
         var e = [];
         if (n)
             for (var t in n) e.push(t, n[t]);
         return new v(e)
     };
-    var Mn = v;
+    var On = v;
 
-    function Hr(n, e, t) {
+    function _r(n, e, t) {
         for (let r = 0;; r++) {
             if (r == n.childCount || r == e.childCount) return n.childCount == e.childCount ? null : t;
             let i = n.child(r),
                 o = e.child(r);
             if (i == o) {
                 t += i.nodeSize;
                 continue
             }
             if (!i.sameMarkup(o)) return t;
             if (i.isText && i.text != o.text) {
                 for (let s = 0; i.text[s] == o.text[s]; s++) t++;
                 return t
             }
             if (i.content.size || o.content.size) {
-                let s = Hr(i.content, o.content, t + 1);
+                let s = _r(i.content, o.content, t + 1);
                 if (s != null) return s
             }
             t += i.nodeSize
         }
     }
 
-    function jr(n, e, t, r) {
+    function Ur(n, e, t, r) {
         for (let i = n.childCount, o = e.childCount;;) {
             if (i == 0 || o == 0) return i == o ? null : {
                 a: t,
                 b: r
             };
             let s = n.child(--i),
                 l = e.child(--o),
@@ -157,15 +157,15 @@
                 for (; c < f && s.text[s.text.length - c - 1] == l.text[l.text.length - c - 1];) c++, t--, r--;
                 return {
                     a: t,
                     b: r
                 }
             }
             if (s.content.size || l.content.size) {
-                let c = jr(s.content, l.content, t - 1, r - 1);
+                let c = Ur(s.content, l.content, t - 1, r - 1);
                 if (c) return c
             }
             t -= a, r -= a
         }
     }
     var y = class n {
         constructor(e, t) {
@@ -258,27 +258,27 @@
         forEach(e) {
             for (let t = 0, r = 0; t < this.content.length; t++) {
                 let i = this.content[t];
                 e(i, r, t), r += i.nodeSize
             }
         }
         findDiffStart(e, t = 0) {
-            return Hr(this, e, t)
+            return _r(this, e, t)
         }
         findDiffEnd(e, t = this.size, r = e.size) {
-            return jr(this, e, t, r)
+            return Ur(this, e, t, r)
         }
         findIndex(e, t = -1) {
-            if (e == 0) return zt(0, e);
-            if (e == this.size) return zt(this.content.length, e);
+            if (e == 0) return Bt(0, e);
+            if (e == this.size) return Bt(this.content.length, e);
             if (e > this.size || e < 0) throw new RangeError(`Position ${e} outside of fragment (${this})`);
             for (let r = 0, i = 0;; r++) {
                 let o = this.child(r),
                     s = i + o.nodeSize;
-                if (s >= e) return s == e || t > 0 ? zt(r + 1, s) : zt(r, i);
+                if (s >= e) return s == e || t > 0 ? Bt(r + 1, s) : Bt(r, i);
                 i = s
             }
         }
         toString() {
             return "<" + this.toStringInner() + ">"
         }
         toStringInner() {
@@ -306,35 +306,35 @@
             if (e instanceof n) return e;
             if (Array.isArray(e)) return this.fromArray(e);
             if (e.attrs) return new n([e], e.nodeSize);
             throw new RangeError("Can not convert " + e + " to a Fragment" + (e.nodesBetween ? " (looks like multiple versions of prosemirror-model were loaded)" : ""))
         }
     };
     y.empty = new y([], 0);
-    var Cn = {
+    var wn = {
         index: 0,
         offset: 0
     };
 
-    function zt(n, e) {
-        return Cn.index = n, Cn.offset = e, Cn
+    function Bt(n, e) {
+        return wn.index = n, wn.offset = e, wn
     }
 
-    function Bt(n, e) {
+    function Ft(n, e) {
         if (n === e) return !0;
         if (!(n && typeof n == "object") || !(e && typeof e == "object")) return !1;
         let t = Array.isArray(n);
         if (Array.isArray(e) != t) return !1;
         if (t) {
             if (n.length != e.length) return !1;
             for (let r = 0; r < n.length; r++)
-                if (!Bt(n[r], e[r])) return !1
+                if (!Ft(n[r], e[r])) return !1
         } else {
             for (let r in n)
-                if (!(r in e) || !Bt(n[r], e[r])) return !1;
+                if (!(r in e) || !Ft(n[r], e[r])) return !1;
             for (let r in e)
                 if (!(r in n)) return !1
         }
         return !0
     }
     var O = class n {
         constructor(e, t) {
@@ -360,15 +360,15 @@
         }
         isInSet(e) {
             for (let t = 0; t < e.length; t++)
                 if (this.eq(e[t])) return !0;
             return !1
         }
         eq(e) {
-            return this == e || this.type == e.type && Bt(this.attrs, e.attrs)
+            return this == e || this.type == e.type && Ft(this.attrs, e.attrs)
         }
         toJSON() {
             let e = {
                 type: this.type.name
             };
             for (let t in this.attrs) {
                 e.attrs = this.attrs;
@@ -393,28 +393,28 @@
             if (!e || Array.isArray(e) && e.length == 0) return n.none;
             if (e instanceof n) return [e];
             let t = e.slice();
             return t.sort((r, i) => r.type.rank - i.type.rank), t
         }
     };
     O.none = [];
-    var Ie = class extends Error {},
+    var Ae = class extends Error {},
         b = class n {
             constructor(e, t, r) {
                 this.content = e, this.openStart = t, this.openEnd = r
             }
             get size() {
                 return this.content.size - this.openStart - this.openEnd
             }
             insertAt(e, t) {
-                let r = Ur(this.content, e + this.openStart, t);
+                let r = Yr(this.content, e + this.openStart, t);
                 return r && new n(r, this.openStart, this.openEnd)
             }
             removeBetween(e, t) {
-                return new n(_r(this.content, e + this.openStart, t + this.openStart), this.openStart, this.openEnd)
+                return new n(Gr(this.content, e + this.openStart, t + this.openStart), this.openStart, this.openEnd)
             }
             eq(e) {
                 return this.content.eq(e.content) && this.openStart == e.openStart && this.openEnd == e.openEnd
             }
             toString() {
                 return this.content + "(" + this.openStart + "," + this.openEnd + ")"
             }
@@ -438,74 +438,74 @@
                 for (let o = e.firstChild; o && !o.isLeaf && (t || !o.type.spec.isolating); o = o.firstChild) r++;
                 for (let o = e.lastChild; o && !o.isLeaf && (t || !o.type.spec.isolating); o = o.lastChild) i++;
                 return new n(e, r, i)
             }
         };
     b.empty = new b(y.empty, 0, 0);
 
-    function _r(n, e, t) {
+    function Gr(n, e, t) {
         let {
             index: r,
             offset: i
         } = n.findIndex(e), o = n.maybeChild(r), {
             index: s,
             offset: l
         } = n.findIndex(t);
         if (i == e || o.isText) {
             if (l != t && !n.child(s).isText) throw new RangeError("Removing non-flat range");
             return n.cut(0, e).append(n.cut(t))
         }
         if (r != s) throw new RangeError("Removing non-flat range");
-        return n.replaceChild(r, o.copy(_r(o.content, e - i - 1, t - i - 1)))
+        return n.replaceChild(r, o.copy(Gr(o.content, e - i - 1, t - i - 1)))
     }
 
-    function Ur(n, e, t, r) {
+    function Yr(n, e, t, r) {
         let {
             index: i,
             offset: o
         } = n.findIndex(e), s = n.maybeChild(i);
         if (o == e || s.isText) return r && !r.canReplace(i, i, t) ? null : n.cut(0, e).append(t).append(n.cut(e));
-        let l = Ur(s.content, e - o - 1, t);
+        let l = Yr(s.content, e - o - 1, t);
         return l && n.replaceChild(i, s.copy(l))
     }
 
-    function Ws(n, e, t) {
-        if (t.openStart > n.depth) throw new Ie("Inserted content deeper than insertion position");
-        if (n.depth - t.openStart != e.depth - t.openEnd) throw new Ie("Inconsistent open depths");
-        return Gr(n, e, t, 0)
+    function Ks(n, e, t) {
+        if (t.openStart > n.depth) throw new Ae("Inserted content deeper than insertion position");
+        if (n.depth - t.openStart != e.depth - t.openEnd) throw new Ae("Inconsistent open depths");
+        return Xr(n, e, t, 0)
     }
 
-    function Gr(n, e, t, r) {
+    function Xr(n, e, t, r) {
         let i = n.index(r),
             o = n.node(r);
         if (i == e.index(r) && r < n.depth - t.openStart) {
-            let s = Gr(n, e, t, r + 1);
+            let s = Xr(n, e, t, r + 1);
             return o.copy(o.content.replaceChild(i, s))
         } else if (t.content.size)
             if (!t.openStart && !t.openEnd && n.depth == r && e.depth == r) {
                 let s = n.parent,
                     l = s.content;
                 return Ee(s, l.cut(0, n.parentOffset).append(t.content).append(l.cut(e.parentOffset)))
             } else {
                 let {
                     start: s,
                     end: l
-                } = qs(t, n);
-                return Ee(o, Xr(n, s, l, e, r))
+                } = $s(t, n);
+                return Ee(o, Zr(n, s, l, e, r))
             }
-        else return Ee(o, Ft(n, e, r))
+        else return Ee(o, Vt(n, e, r))
     }
 
-    function Yr(n, e) {
-        if (!e.type.compatibleContent(n.type)) throw new Ie("Cannot join " + e.type.name + " onto " + n.type.name)
+    function Qr(n, e) {
+        if (!e.type.compatibleContent(n.type)) throw new Ae("Cannot join " + e.type.name + " onto " + n.type.name)
     }
 
-    function Tn(n, e, t) {
+    function En(n, e, t) {
         let r = n.node(t);
-        return Yr(r, e.node(t)), r
+        return Qr(r, e.node(t)), r
     }
 
     function De(n, e) {
         let t = e.length - 1;
         t >= 0 && n.isText && n.sameMarkup(e[t]) ? e[t] = n.withText(e[t].text + n.text) : e.push(n)
     }
 
@@ -518,40 +518,40 @@
         e && e.depth == t && e.textOffset && De(e.nodeBefore, r)
     }
 
     function Ee(n, e) {
         return n.type.checkContent(e), n.copy(e)
     }
 
-    function Xr(n, e, t, r, i) {
-        let o = n.depth > i && Tn(n, e, i + 1),
-            s = r.depth > i && Tn(t, r, i + 1),
+    function Zr(n, e, t, r, i) {
+        let o = n.depth > i && En(n, e, i + 1),
+            s = r.depth > i && En(t, r, i + 1),
             l = [];
-        return lt(null, n, i, l), o && s && e.index(i) == t.index(i) ? (Yr(o, s), De(Ee(o, Xr(n, e, t, r, i + 1)), l)) : (o && De(Ee(o, Ft(n, e, i + 1)), l), lt(e, t, i, l), s && De(Ee(s, Ft(t, r, i + 1)), l)), lt(r, null, i, l), new y(l)
+        return lt(null, n, i, l), o && s && e.index(i) == t.index(i) ? (Qr(o, s), De(Ee(o, Zr(n, e, t, r, i + 1)), l)) : (o && De(Ee(o, Vt(n, e, i + 1)), l), lt(e, t, i, l), s && De(Ee(s, Vt(t, r, i + 1)), l)), lt(r, null, i, l), new y(l)
     }
 
-    function Ft(n, e, t) {
+    function Vt(n, e, t) {
         let r = [];
         if (lt(null, n, t, r), n.depth > t) {
-            let i = Tn(n, e, t + 1);
-            De(Ee(i, Ft(n, e, t + 1)), r)
+            let i = En(n, e, t + 1);
+            De(Ee(i, Vt(n, e, t + 1)), r)
         }
         return lt(e, null, t, r), new y(r)
     }
 
-    function qs(n, e) {
+    function $s(n, e) {
         let t = e.depth - n.openStart,
             i = e.node(t).copy(n.content);
         for (let o = t - 1; o >= 0; o--) i = e.node(o).copy(y.from(i));
         return {
             start: i.resolveNoCache(n.openStart + t),
             end: i.resolveNoCache(i.content.size - n.openEnd - t)
         }
     }
-    var Vt = class n {
+    var Lt = class n {
             constructor(e, t, r) {
                 this.pos = e, this.path = t, this.parentOffset = r, this.depth = t.length / 3 - 1
             }
             resolveDepth(e) {
                 return e == null ? this.depth : e < 0 ? this.depth + e : e
             }
             get parent() {
@@ -633,15 +633,15 @@
                 for (let t = this.depth; t > 0; t--)
                     if (this.start(t) <= e && this.end(t) >= e) return t;
                 return 0
             }
             blockRange(e = this, t) {
                 if (e.pos < this.pos) return e.blockRange(this);
                 for (let r = this.depth - (this.parent.inlineContent || this.pos == e.pos ? 1 : 0); r >= 0; r--)
-                    if (e.pos <= this.end(r) && (!t || t(this.node(r)))) return new Ae(this, e, r);
+                    if (e.pos <= this.end(r) && (!t || t(this.node(r)))) return new Ie(this, e, r);
                 return null
             }
             sameParent(e) {
                 return this.pos - this.parentOffset == e.pos - e.parentOffset
             }
             max(e) {
                 return e.pos > this.pos ? e : this
@@ -666,26 +666,26 @@
                     } = s.content.findIndex(o), c = o - a;
                     if (r.push(s, l, i + a), !c || (s = s.child(l), s.isText)) break;
                     o = c - 1, i += a + 1
                 }
                 return new n(t, r, o)
             }
             static resolveCached(e, t) {
-                for (let i = 0; i < On.length; i++) {
-                    let o = On[i];
+                for (let i = 0; i < Nn.length; i++) {
+                    let o = Nn[i];
                     if (o.pos == t && o.doc == e) return o
                 }
-                let r = On[wn] = n.resolve(e, t);
-                return wn = (wn + 1) % Ks, r
+                let r = Nn[Tn] = n.resolve(e, t);
+                return Tn = (Tn + 1) % Hs, r
             }
         },
-        On = [],
-        wn = 0,
-        Ks = 12,
-        Ae = class {
+        Nn = [],
+        Tn = 0,
+        Hs = 12,
+        Ie = class {
             constructor(e, t, r) {
                 this.$from = e, this.$to = t, this.depth = r
             }
             get start() {
                 return this.$from.before(this.depth + 1)
             }
             get end() {
@@ -697,15 +697,15 @@
             get startIndex() {
                 return this.$from.index(this.depth)
             }
             get endIndex() {
                 return this.$to.indexAfter(this.depth)
             }
         },
-        $s = Object.create(null),
+        js = Object.create(null),
         re = class n {
             constructor(e, t, r, i = O.none) {
                 this.type = e, this.attrs = t, this.marks = i, this.content = r || y.empty
             }
             get nodeSize() {
                 return this.isLeaf ? 1 : 2 + this.content.size
             }
@@ -742,15 +742,15 @@
             eq(e) {
                 return this == e || this.sameMarkup(e) && this.content.eq(e.content)
             }
             sameMarkup(e) {
                 return this.hasMarkup(e.type, e.attrs, e.marks)
             }
             hasMarkup(e, t, r) {
-                return this.type == e && Bt(this.attrs, t || e.defaultAttrs || $s) && O.sameSet(this.marks, r || O.none)
+                return this.type == e && Ft(this.attrs, t || e.defaultAttrs || js) && O.sameSet(this.marks, r || O.none)
             }
             copy(e = null) {
                 return e == this.content ? this : new n(this.type, this.attrs, e, this.marks)
             }
             mark(e) {
                 return e == this.marks ? this : new n(this.type, this.attrs, this.content, e)
             }
@@ -763,15 +763,15 @@
                     o = this.resolve(t),
                     s = r ? 0 : i.sharedDepth(t),
                     l = i.start(s),
                     c = i.node(s).content.cut(i.pos - l, o.pos - l);
                 return new b(c, i.depth - s, o.depth - s)
             }
             replace(e, t, r) {
-                return Ws(this.resolve(e), this.resolve(t), r)
+                return Ks(this.resolve(e), this.resolve(t), r)
             }
             nodeAt(e) {
                 for (let t = this;;) {
                     let {
                         index: r,
                         offset: i
                     } = t.content.findIndex(e);
@@ -810,18 +810,18 @@
                 return {
                     node: i,
                     index: t - 1,
                     offset: r - i.nodeSize
                 }
             }
             resolve(e) {
-                return Vt.resolveCached(this, e)
+                return Lt.resolveCached(this, e)
             }
             resolveNoCache(e) {
-                return Vt.resolve(this, e)
+                return Lt.resolve(this, e)
             }
             rangeHasMark(e, t, r) {
                 let i = !1;
                 return t > e && this.nodesBetween(e, t, o => (r.isInSet(o.marks) && (i = !0), !i)), i
             }
             get isBlock() {
                 return this.type.isBlock
@@ -843,15 +843,15 @@
             }
             get isAtom() {
                 return this.type.isAtom
             }
             toString() {
                 if (this.type.spec.toDebugString) return this.type.spec.toDebugString(this);
                 let e = this.type.name;
-                return this.content.size && (e += "(" + this.content.toStringInner() + ")"), Qr(this.marks, e)
+                return this.content.size && (e += "(" + this.content.toStringInner() + ")"), ei(this.marks, e)
             }
             contentMatchAt(e) {
                 let t = this.type.contentMatch.matchFragment(this.content, 0, e);
                 if (!t) throw new Error("Called contentMatchAt on a node with invalid content");
                 return t
             }
             canReplace(e, t, r = y.empty, i = 0, o = r.childCount) {
@@ -900,21 +900,21 @@
                     return e.text(t.text, r)
                 }
                 let i = y.fromJSON(e, t.content);
                 return e.nodeType(t.type).create(t.attrs, i, r)
             }
         };
     re.prototype.text = void 0;
-    var Dn = class n extends re {
+    var An = class n extends re {
         constructor(e, t, r, i) {
             if (super(e, t, null, i), !r) throw new RangeError("Empty text nodes are not allowed");
             this.text = r
         }
         toString() {
-            return this.type.spec.toDebugString ? this.type.spec.toDebugString(this) : Qr(this.marks, JSON.stringify(this.text))
+            return this.type.spec.toDebugString ? this.type.spec.toDebugString(this) : ei(this.marks, JSON.stringify(this.text))
         }
         get textContent() {
             return this.text
         }
         textBetween(e, t) {
             return this.text.slice(e, t)
         }
@@ -935,29 +935,29 @@
         }
         toJSON() {
             let e = super.toJSON();
             return e.text = this.text, e
         }
     };
 
-    function Qr(n, e) {
+    function ei(n, e) {
         for (let t = n.length - 1; t >= 0; t--) e = n[t].type.name + "(" + e + ")";
         return e
     }
-    var ve = class n {
+    var Re = class n {
         constructor(e) {
             this.validEnd = e, this.next = [], this.wrapCache = []
         }
         static parse(e, t) {
-            let r = new En(e, t);
+            let r = new In(e, t);
             if (r.next == null) return n.empty;
-            let i = Zr(r);
+            let i = ti(r);
             r.next && r.err("Unexpected trailing text");
-            let o = Xs(Ys(i));
-            return Qs(o, r), o
+            let o = Zs(Qs(i));
+            return el(o, r), o
         }
         matchType(e) {
             for (let t = 0; t < this.next.length; t++)
                 if (this.next[t].type == e) return this.next[t].next;
             return null
         }
         matchFragment(e, t = 0, r = e.childCount) {
@@ -1057,115 +1057,115 @@
                 let o = i + (r.validEnd ? "*" : " ") + " ";
                 for (let s = 0; s < r.next.length; s++) o += (s ? ", " : "") + r.next[s].type.name + "->" + e.indexOf(r.next[s].next);
                 return o
             }).join(`
 `)
         }
     };
-    ve.empty = new ve(!0);
-    var En = class {
+    Re.empty = new Re(!0);
+    var In = class {
         constructor(e, t) {
             this.string = e, this.nodeTypes = t, this.inline = null, this.pos = 0, this.tokens = e.split(/\s*(?=\b|\W|$)/), this.tokens[this.tokens.length - 1] == "" && this.tokens.pop(), this.tokens[0] == "" && this.tokens.shift()
         }
         get next() {
             return this.tokens[this.pos]
         }
         eat(e) {
             return this.next == e && (this.pos++ || !0)
         }
         err(e) {
             throw new SyntaxError(e + " (in content expression '" + this.string + "')")
         }
     };
 
-    function Zr(n) {
+    function ti(n) {
         let e = [];
-        do e.push(Hs(n)); while (n.eat("|"));
+        do e.push(_s(n)); while (n.eat("|"));
         return e.length == 1 ? e[0] : {
             type: "choice",
             exprs: e
         }
     }
 
-    function Hs(n) {
+    function _s(n) {
         let e = [];
-        do e.push(js(n)); while (n.next && n.next != ")" && n.next != "|");
+        do e.push(Us(n)); while (n.next && n.next != ")" && n.next != "|");
         return e.length == 1 ? e[0] : {
             type: "seq",
             exprs: e
         }
     }
 
-    function js(n) {
-        let e = Gs(n);
+    function Us(n) {
+        let e = Xs(n);
         for (;;)
             if (n.eat("+")) e = {
                 type: "plus",
                 expr: e
             };
             else if (n.eat("*")) e = {
             type: "star",
             expr: e
         };
         else if (n.eat("?")) e = {
             type: "opt",
             expr: e
         };
-        else if (n.eat("{")) e = _s(n, e);
+        else if (n.eat("{")) e = Gs(n, e);
         else break;
         return e
     }
 
-    function Lr(n) {
+    function Wr(n) {
         /\D/.test(n.next) && n.err("Expected number, got '" + n.next + "'");
         let e = Number(n.next);
         return n.pos++, e
     }
 
-    function _s(n, e) {
-        let t = Lr(n),
+    function Gs(n, e) {
+        let t = Wr(n),
             r = t;
-        return n.eat(",") && (n.next != "}" ? r = Lr(n) : r = -1), n.eat("}") || n.err("Unclosed braced range"), {
+        return n.eat(",") && (n.next != "}" ? r = Wr(n) : r = -1), n.eat("}") || n.err("Unclosed braced range"), {
             type: "range",
             min: t,
             max: r,
             expr: e
         }
     }
 
-    function Us(n, e) {
+    function Ys(n, e) {
         let t = n.nodeTypes,
             r = t[e];
         if (r) return [r];
         let i = [];
         for (let o in t) {
             let s = t[o];
             s.groups.indexOf(e) > -1 && i.push(s)
         }
         return i.length == 0 && n.err("No node type or group '" + e + "' found"), i
     }
 
-    function Gs(n) {
+    function Xs(n) {
         if (n.eat("(")) {
-            let e = Zr(n);
+            let e = ti(n);
             return n.eat(")") || n.err("Missing closing paren"), e
         } else if (/\W/.test(n.next)) n.err("Unexpected token '" + n.next + "'");
         else {
-            let e = Us(n, n.next).map(t => (n.inline == null ? n.inline = t.isInline : n.inline != t.isInline && n.err("Mixing inline and block content"), {
+            let e = Ys(n, n.next).map(t => (n.inline == null ? n.inline = t.isInline : n.inline != t.isInline && n.err("Mixing inline and block content"), {
                 type: "name",
                 value: t
             }));
             return n.pos++, e.length == 1 ? e[0] : {
                 type: "choice",
                 exprs: e
             }
         }
     }
 
-    function Ys(n) {
+    function Qs(n) {
         let e = [
             []
         ];
         return i(o(n, 0), t()), e;
 
         function t() {
             return e.push([]) - 1
@@ -1215,21 +1215,21 @@
                     if (s.type == "name") return [r(l, void 0, s.value)];
                     throw new Error("Unknown expr type")
                 }
             }
         }
     }
 
-    function ei(n, e) {
+    function ni(n, e) {
         return e - n
     }
 
-    function Jr(n, e) {
+    function qr(n, e) {
         let t = [];
-        return r(e), t.sort(ei);
+        return r(e), t.sort(ni);
 
         function r(i) {
             let o = n[i];
             if (o.length == 1 && !o[0].term) return r(o[0].to);
             t.push(i);
             for (let s = 0; s < o.length; s++) {
                 let {
@@ -1237,46 +1237,46 @@
                     to: a
                 } = o[s];
                 !l && t.indexOf(a) == -1 && r(a)
             }
         }
     }
 
-    function Xs(n) {
+    function Zs(n) {
         let e = Object.create(null);
-        return t(Jr(n, 0));
+        return t(qr(n, 0));
 
         function t(r) {
             let i = [];
             r.forEach(s => {
                 n[s].forEach(({
                     term: l,
                     to: a
                 }) => {
                     if (!l) return;
                     let c;
                     for (let f = 0; f < i.length; f++) i[f][0] == l && (c = i[f][1]);
-                    Jr(n, a).forEach(f => {
+                    qr(n, a).forEach(f => {
                         c || i.push([l, c = []]), c.indexOf(f) == -1 && c.push(f)
                     })
                 })
             });
-            let o = e[r.join(",")] = new ve(r.indexOf(n.length - 1) > -1);
+            let o = e[r.join(",")] = new Re(r.indexOf(n.length - 1) > -1);
             for (let s = 0; s < i.length; s++) {
-                let l = i[s][1].sort(ei);
+                let l = i[s][1].sort(ni);
                 o.next.push({
                     type: i[s][0],
                     next: e[l.join(",")] || t(l)
                 })
             }
             return o
         }
     }
 
-    function Qs(n, e) {
+    function el(n, e) {
         for (let t = 0, r = [n]; t < r.length; t++) {
             let i = r[t],
                 o = !i.validEnd,
                 s = [];
             for (let l = 0; l < i.next.length; l++) {
                 let {
                     type: a,
@@ -1284,56 +1284,56 @@
                 } = i.next[l];
                 s.push(a.name), o && !(a.isText || a.hasRequiredAttrs()) && (o = !1), r.indexOf(c) == -1 && r.push(c)
             }
             o && e.err("Only non-generatable nodes (" + s.join(", ") + ") in a required position (see https://prosemirror.net/docs/guide/#generatable)")
         }
     }
 
-    function ti(n) {
+    function ri(n) {
         let e = Object.create(null);
         for (let t in n) {
             let r = n[t];
             if (!r.hasDefault) return null;
             e[t] = r.default
         }
         return e
     }
 
-    function ni(n, e) {
+    function ii(n, e) {
         let t = Object.create(null);
         for (let r in n) {
             let i = e && e[r];
             if (i === void 0) {
                 let o = n[r];
                 if (o.hasDefault) i = o.default;
                 else throw new RangeError("No value supplied for attribute " + r)
             }
             t[r] = i
         }
         return t
     }
 
-    function ri(n) {
+    function oi(n) {
         let e = Object.create(null);
         if (n)
-            for (let t in n) e[t] = new In(n[t]);
+            for (let t in n) e[t] = new Rn(n[t]);
         return e
     }
-    var Lt = class n {
+    var Jt = class n {
             constructor(e, t, r) {
-                this.name = e, this.schema = t, this.spec = r, this.markSet = null, this.groups = r.group ? r.group.split(" ") : [], this.attrs = ri(r.attrs), this.defaultAttrs = ti(this.attrs), this.contentMatch = null, this.inlineContent = null, this.isBlock = !(r.inline || e == "text"), this.isText = e == "text"
+                this.name = e, this.schema = t, this.spec = r, this.markSet = null, this.groups = r.group ? r.group.split(" ") : [], this.attrs = oi(r.attrs), this.defaultAttrs = ri(this.attrs), this.contentMatch = null, this.inlineContent = null, this.isBlock = !(r.inline || e == "text"), this.isText = e == "text"
             }
             get isInline() {
                 return !this.isBlock
             }
             get isTextblock() {
                 return this.isBlock && this.inlineContent
             }
             get isLeaf() {
-                return this.contentMatch == ve.empty
+                return this.contentMatch == Re.empty
             }
             get isAtom() {
                 return this.isLeaf || !!this.spec.atom
             }
             get whitespace() {
                 return this.spec.whitespace || (this.spec.code ? "pre" : "normal")
             }
@@ -1342,15 +1342,15 @@
                     if (this.attrs[e].isRequired) return !0;
                 return !1
             }
             compatibleContent(e) {
                 return this == e || this.contentMatch.compatible(e.contentMatch)
             }
             computeAttrs(e) {
-                return !e && this.defaultAttrs ? this.defaultAttrs : ni(this.attrs, e)
+                return !e && this.defaultAttrs ? this.defaultAttrs : ii(this.attrs, e)
             }
             create(e = null, t, r) {
                 if (this.isText) throw new Error("NodeType.create can't construct text nodes");
                 return new re(this, this.computeAttrs(e), y.from(t), O.setFrom(r))
             }
             createChecked(e = null, t, r) {
                 return t = y.from(t), this.checkContent(t), new re(this, this.computeAttrs(e), t, O.setFrom(r))
@@ -1396,30 +1396,30 @@
                 let i = t.spec.topNode || "doc";
                 if (!r[i]) throw new RangeError("Schema is missing its top node type ('" + i + "')");
                 if (!r.text) throw new RangeError("Every schema needs a 'text' type");
                 for (let o in r.text.attrs) throw new RangeError("The text node type should not have attributes");
                 return r
             }
         },
-        In = class {
+        Rn = class {
             constructor(e) {
                 this.hasDefault = Object.prototype.hasOwnProperty.call(e, "default"), this.default = e.default
             }
             get isRequired() {
                 return !this.hasDefault
             }
         },
         ct = class n {
             constructor(e, t, r, i) {
-                this.name = e, this.rank = t, this.schema = r, this.spec = i, this.attrs = ri(i.attrs), this.excluded = null;
-                let o = ti(this.attrs);
+                this.name = e, this.rank = t, this.schema = r, this.spec = i, this.attrs = oi(i.attrs), this.excluded = null;
+                let o = ri(this.attrs);
                 this.instance = o ? new O(this, o) : null
             }
             create(e = null) {
-                return !e && this.instance ? this.instance : new O(this, ni(this.attrs, e))
+                return !e && this.instance ? this.instance : new O(this, ii(this.attrs, e))
             }
             static compile(e, t) {
                 let r = Object.create(null),
                     i = 0;
                 return e.forEach((o, s) => r[o] = new n(o, i++, t, s)), r
             }
             removeFromSet(e) {
@@ -1430,45 +1430,45 @@
                 for (let t = 0; t < e.length; t++)
                     if (e[t].type == this) return e[t]
             }
             excludes(e) {
                 return this.excluded.indexOf(e) > -1
             }
         },
-        Pe = class {
+        ke = class {
             constructor(e) {
                 this.cached = Object.create(null);
                 let t = this.spec = {};
                 for (let i in e) t[i] = e[i];
-                t.nodes = Mn.from(e.nodes), t.marks = Mn.from(e.marks || {}), this.nodes = Lt.compile(this.spec.nodes, this), this.marks = ct.compile(this.spec.marks, this);
+                t.nodes = On.from(e.nodes), t.marks = On.from(e.marks || {}), this.nodes = Jt.compile(this.spec.nodes, this), this.marks = ct.compile(this.spec.marks, this);
                 let r = Object.create(null);
                 for (let i in this.nodes) {
                     if (i in this.marks) throw new RangeError(i + " can not be both a node and a mark");
                     let o = this.nodes[i],
                         s = o.spec.content || "",
                         l = o.spec.marks;
-                    o.contentMatch = r[s] || (r[s] = ve.parse(s, this.nodes)), o.inlineContent = o.contentMatch.inlineContent, o.markSet = l == "_" ? null : l ? Wr(this, l.split(" ")) : l == "" || !o.inlineContent ? [] : null
+                    o.contentMatch = r[s] || (r[s] = Re.parse(s, this.nodes)), o.inlineContent = o.contentMatch.inlineContent, o.markSet = l == "_" ? null : l ? Kr(this, l.split(" ")) : l == "" || !o.inlineContent ? [] : null
                 }
                 for (let i in this.marks) {
                     let o = this.marks[i],
                         s = o.spec.excludes;
-                    o.excluded = s == null ? [o] : s == "" ? [] : Wr(this, s.split(" "))
+                    o.excluded = s == null ? [o] : s == "" ? [] : Kr(this, s.split(" "))
                 }
                 this.nodeFromJSON = this.nodeFromJSON.bind(this), this.markFromJSON = this.markFromJSON.bind(this), this.topNodeType = this.nodes[this.spec.topNode || "doc"], this.cached.wrappings = Object.create(null)
             }
             node(e, t = null, r, i) {
                 if (typeof e == "string") e = this.nodeType(e);
-                else if (e instanceof Lt) {
+                else if (e instanceof Jt) {
                     if (e.schema != this) throw new RangeError("Node type from different schema used (" + e.name + ")")
                 } else throw new RangeError("Invalid node type: " + e);
                 return e.createChecked(t, r, i)
             }
             text(e, t) {
                 let r = this.nodes.text;
-                return new Dn(r, r.defaultAttrs, e, O.setFrom(t))
+                return new An(r, r.defaultAttrs, e, O.setFrom(t))
             }
             mark(e, t) {
                 return typeof e == "string" && (e = this.marks[e]), e.create(t)
             }
             nodeFromJSON(e) {
                 return re.fromJSON(this, e)
             }
@@ -1478,15 +1478,15 @@
             nodeType(e) {
                 let t = this.nodes[e];
                 if (!t) throw new RangeError("Unknown node type: " + e);
                 return t
             }
         };
 
-    function Wr(n, e) {
+    function Kr(n, e) {
         let t = [];
         for (let r = 0; r < e.length; r++) {
             let i = e[r],
                 o = n.marks[i],
                 s = o;
             if (o) t.push(o);
             else
@@ -1494,36 +1494,44 @@
                     let a = n.marks[l];
                     (i == "_" || a.spec.group && a.spec.group.split(" ").indexOf(i) > -1) && t.push(s = a)
                 }
             if (!s) throw new SyntaxError("Unknown mark type: '" + e[r] + "'")
         }
         return t
     }
-    var Re = class n {
+
+    function tl(n) {
+        return n.tag != null
+    }
+
+    function nl(n) {
+        return n.style != null
+    }
+    var ve = class n {
             constructor(e, t) {
                 this.schema = e, this.rules = t, this.tags = [], this.styles = [], t.forEach(r => {
-                    r.tag ? this.tags.push(r) : r.style && this.styles.push(r)
+                    tl(r) ? this.tags.push(r) : nl(r) && this.styles.push(r)
                 }), this.normalizeLists = !this.tags.some(r => {
                     if (!/^(ul|ol)\b/.test(r.tag) || !r.node) return !1;
                     let i = e.nodes[r.node];
                     return i.contentMatch.matchType(i)
                 })
             }
             parse(e, t = {}) {
-                let r = new qt(this, t, !1);
+                let r = new Kt(this, t, !1);
                 return r.addAll(e, t.from, t.to), r.finish()
             }
             parseSlice(e, t = {}) {
-                let r = new qt(this, t, !0);
+                let r = new Kt(this, t, !0);
                 return r.addAll(e, t.from, t.to), b.maxOpen(r.finish())
             }
             matchTag(e, t, r) {
                 for (let i = r ? this.tags.indexOf(r) + 1 : 0; i < this.tags.length; i++) {
                     let o = this.tags[i];
-                    if (tl(e, o.tag) && (o.namespace === void 0 || e.namespaceURI == o.namespace) && (!o.context || t.matchesContext(o.context))) {
+                    if (ol(e, o.tag) && (o.namespace === void 0 || e.namespaceURI == o.namespace) && (!o.context || t.matchesContext(o.context))) {
                         if (o.getAttrs) {
                             let s = o.getAttrs(e);
                             if (s === !1) continue;
                             o.attrs = s || void 0
                         }
                         return o
                     }
@@ -1554,30 +1562,30 @@
                         if ((l.priority == null ? 50 : l.priority) < o) break
                     }
                     t.splice(s, 0, i)
                 }
                 for (let i in e.marks) {
                     let o = e.marks[i].spec.parseDOM;
                     o && o.forEach(s => {
-                        r(s = Kr(s)), s.mark || s.ignore || s.clearMark || (s.mark = i)
+                        r(s = Hr(s)), s.mark || s.ignore || s.clearMark || (s.mark = i)
                     })
                 }
                 for (let i in e.nodes) {
                     let o = e.nodes[i].spec.parseDOM;
                     o && o.forEach(s => {
-                        r(s = Kr(s)), s.node || s.ignore || s.mark || (s.node = i)
+                        r(s = Hr(s)), s.node || s.ignore || s.mark || (s.node = i)
                     })
                 }
                 return t
             }
             static fromSchema(e) {
                 return e.cached.domParser || (e.cached.domParser = new n(e, n.schemaRules(e)))
             }
         },
-        ii = {
+        si = {
             address: !0,
             article: !0,
             aside: !0,
             blockquote: !0,
             canvas: !0,
             dd: !0,
             div: !0,
@@ -1603,32 +1611,32 @@
             p: !0,
             pre: !0,
             section: !0,
             table: !0,
             tfoot: !0,
             ul: !0
         },
-        Zs = {
+        rl = {
             head: !0,
             noscript: !0,
             object: !0,
             script: !0,
             style: !0,
             title: !0
         },
-        oi = {
+        li = {
             ol: !0,
             ul: !0
         },
-        Jt = 1,
-        Wt = 2,
+        Wt = 1,
+        qt = 2,
         at = 4;
 
-    function qr(n, e, t) {
-        return e != null ? (e ? Jt : 0) | (e === "full" ? Wt : 0) : n && n.whitespace == "pre" ? Jt | Wt : t & ~at
+    function $r(n, e, t) {
+        return e != null ? (e ? Wt : 0) | (e === "full" ? qt : 0) : n && n.whitespace == "pre" ? Wt | qt : t & ~at
     }
     var je = class {
             constructor(e, t, r, i, o, s, l) {
                 this.type = e, this.attrs = t, this.marks = r, this.pendingMarks = i, this.solid = o, this.options = l, this.content = [], this.activeMarks = O.none, this.stashMarks = [], this.match = s || (l & at ? null : e.contentMatch)
             }
             findWrapping(e) {
                 if (!this.match) {
@@ -1640,15 +1648,15 @@
                             i;
                         return (i = r.findWrapping(e.type)) ? (this.match = r, i) : null
                     }
                 }
                 return this.match.findWrapping(e.type)
             }
             finish(e) {
-                if (!(this.options & Jt)) {
+                if (!(this.options & Wt)) {
                     let r = this.content[this.content.length - 1],
                         i;
                     if (r && r.isText && (i = /[ \t\r\n\u000c]+$/.exec(r.text))) {
                         let o = r;
                         r.text.length == i[0].length ? this.content.pop() : this.content[this.content.length - 1] = o.withText(o.text.slice(0, o.text.length - i[0].length))
                     }
                 }
@@ -1658,71 +1666,71 @@
             popFromStashMark(e) {
                 for (let t = this.stashMarks.length - 1; t >= 0; t--)
                     if (e.eq(this.stashMarks[t])) return this.stashMarks.splice(t, 1)[0]
             }
             applyPending(e) {
                 for (let t = 0, r = this.pendingMarks; t < r.length; t++) {
                     let i = r[t];
-                    (this.type ? this.type.allowsMarkType(i.type) : rl(i.type, e)) && !i.isInSet(this.activeMarks) && (this.activeMarks = i.addToSet(this.activeMarks), this.pendingMarks = i.removeFromSet(this.pendingMarks))
+                    (this.type ? this.type.allowsMarkType(i.type) : ll(i.type, e)) && !i.isInSet(this.activeMarks) && (this.activeMarks = i.addToSet(this.activeMarks), this.pendingMarks = i.removeFromSet(this.pendingMarks))
                 }
             }
             inlineContext(e) {
-                return this.type ? this.type.inlineContent : this.content.length ? this.content[0].isInline : e.parentNode && !ii.hasOwnProperty(e.parentNode.nodeName.toLowerCase())
+                return this.type ? this.type.inlineContent : this.content.length ? this.content[0].isInline : e.parentNode && !si.hasOwnProperty(e.parentNode.nodeName.toLowerCase())
             }
         },
-        qt = class {
+        Kt = class {
             constructor(e, t, r) {
                 this.parser = e, this.options = t, this.isOpen = r, this.open = 0;
                 let i = t.topNode,
-                    o, s = qr(null, t.preserveWhitespace, 0) | (r ? at : 0);
+                    o, s = $r(null, t.preserveWhitespace, 0) | (r ? at : 0);
                 i ? o = new je(i.type, i.attrs, O.none, O.none, !0, t.topMatch || i.type.contentMatch, s) : r ? o = new je(null, null, O.none, O.none, !0, null, s) : o = new je(e.schema.topNodeType, null, O.none, O.none, !0, null, s), this.nodes = [o], this.find = t.findPositions, this.needsBlock = !1
             }
             get top() {
                 return this.nodes[this.open]
             }
             addDOM(e) {
                 e.nodeType == 3 ? this.addTextNode(e) : e.nodeType == 1 && this.addElement(e)
             }
             withStyleRules(e, t) {
                 let r = e.getAttribute("style");
                 if (!r) return t();
-                let i = this.readStyles(nl(r));
+                let i = this.readStyles(sl(r));
                 if (!i) return;
                 let [o, s] = i, l = this.top;
                 for (let a = 0; a < s.length; a++) this.removePendingMark(s[a], l);
                 for (let a = 0; a < o.length; a++) this.addPendingMark(o[a]);
                 t();
                 for (let a = 0; a < o.length; a++) this.removePendingMark(o[a], l);
                 for (let a = 0; a < s.length; a++) this.addPendingMark(s[a])
             }
             addTextNode(e) {
                 let t = e.nodeValue,
                     r = this.top;
-                if (r.options & Wt || r.inlineContext(e) || /[^ \t\r\n\u000c]/.test(t)) {
-                    if (r.options & Jt) r.options & Wt ? t = t.replace(/\r\n?/g, `
+                if (r.options & qt || r.inlineContext(e) || /[^ \t\r\n\u000c]/.test(t)) {
+                    if (r.options & Wt) r.options & qt ? t = t.replace(/\r\n?/g, `
 `) : t = t.replace(/\r?\n|\r/g, " ");
                     else if (t = t.replace(/[ \t\r\n\u000c]+/g, " "), /^[ \t\r\n\u000c]/.test(t) && this.open == this.nodes.length - 1) {
                         let i = r.content[r.content.length - 1],
                             o = e.previousSibling;
                         (!i || o && o.nodeName == "BR" || i.isText && /[ \t\r\n\u000c]$/.test(i.text)) && (t = t.slice(1))
                     }
                     t && this.insertNode(this.parser.schema.text(t)), this.findInText(e)
                 } else this.findInside(e)
             }
             addElement(e, t) {
                 let r = e.nodeName.toLowerCase(),
                     i;
-                oi.hasOwnProperty(r) && this.parser.normalizeLists && el(e);
+                li.hasOwnProperty(r) && this.parser.normalizeLists && il(e);
                 let o = this.options.ruleFromNode && this.options.ruleFromNode(e) || (i = this.parser.matchTag(e, this, t));
-                if (o ? o.ignore : Zs.hasOwnProperty(r)) this.findInside(e), this.ignoreFallback(e);
+                if (o ? o.ignore : rl.hasOwnProperty(r)) this.findInside(e), this.ignoreFallback(e);
                 else if (!o || o.skip || o.closeParent) {
                     o && o.closeParent ? this.open = Math.max(0, this.open - 1) : o && o.skip.nodeType && (e = o.skip);
                     let s, l = this.top,
                         a = this.needsBlock;
-                    if (ii.hasOwnProperty(r)) l.content.length && l.content[0].isInline && this.open && (this.open--, l = this.top), s = !0, l.type || (this.needsBlock = !0);
+                    if (si.hasOwnProperty(r)) l.content.length && l.content[0].isInline && this.open && (this.open--, l = this.top), s = !0, l.type || (this.needsBlock = !0);
                     else if (!e.firstChild) {
                         this.leafFallback(e);
                         return
                     }
                     o && o.skip ? this.addAll(e) : this.withStyleRules(e, () => this.addAll(e)), s && this.sync(l), this.needsBlock = a
                 } else this.withStyleRules(e, () => {
                     this.addElementByRule(e, o, o.consuming === !1 ? i : void 0)
@@ -1799,15 +1807,15 @@
                 let i = this.findPlace(e.create(t));
                 return i && this.enterInner(e, t, !0, r), i
             }
             enterInner(e, t = null, r = !1, i) {
                 this.closeExtra();
                 let o = this.top;
                 o.applyPending(e), o.match = o.match && o.match.matchType(e);
-                let s = qr(e, i, o.options);
+                let s = $r(e, i, o.options);
                 o.options & at && o.content.length == 0 && (s |= at), this.nodes.push(new je(e, t, o.activeMarks, o.pendingMarks, r, null, s)), this.open++
             }
             closeExtra(e = !1) {
                 let t = this.nodes.length - 1;
                 if (t > this.open) {
                     for (; t > this.open; t--) this.nodes[t - 1].content.push(this.nodes[t].finish(e));
                     this.nodes.length = this.open + 1
@@ -1880,15 +1888,15 @@
                     }
                 for (let t in this.parser.schema.nodes) {
                     let r = this.parser.schema.nodes[t];
                     if (r.isTextblock && r.defaultAttrs) return r
                 }
             }
             addPendingMark(e) {
-                let t = il(e, this.top.pendingMarks);
+                let t = al(e, this.top.pendingMarks);
                 t && this.top.stashMarks.push(t), this.top.pendingMarks = e.addToSet(this.top.pendingMarks)
             }
             removePendingMark(e, t) {
                 for (let r = this.open; r >= 0; r--) {
                     let i = this.nodes[r];
                     if (i.pendingMarks.lastIndexOf(e) > -1) i.pendingMarks = e.removeFromSet(i.pendingMarks);
                     else {
@@ -1897,39 +1905,39 @@
                         s && i.type && i.type.allowsMarkType(s.type) && (i.activeMarks = s.addToSet(i.activeMarks))
                     }
                     if (i == t) break
                 }
             }
         };
 
-    function el(n) {
+    function il(n) {
         for (let e = n.firstChild, t = null; e; e = e.nextSibling) {
             let r = e.nodeType == 1 ? e.nodeName.toLowerCase() : null;
-            r && oi.hasOwnProperty(r) && t ? (t.appendChild(e), e = t) : r == "li" ? t = e : r && (t = null)
+            r && li.hasOwnProperty(r) && t ? (t.appendChild(e), e = t) : r == "li" ? t = e : r && (t = null)
         }
     }
 
-    function tl(n, e) {
+    function ol(n, e) {
         return (n.matches || n.msMatchesSelector || n.webkitMatchesSelector || n.mozMatchesSelector).call(n, e)
     }
 
-    function nl(n) {
+    function sl(n) {
         let e = /\s*([\w-]+)\s*:\s*([^;]+)/g,
             t, r = [];
         for (; t = e.exec(n);) r.push(t[1], t[2].trim());
         return r
     }
 
-    function Kr(n) {
+    function Hr(n) {
         let e = {};
         for (let t in n) e[t] = n[t];
         return e
     }
 
-    function rl(n, e) {
+    function ll(n, e) {
         let t = e.schema.nodes;
         for (let r in t) {
             let i = t[r];
             if (!i.allowsMarkType(n)) continue;
             let o = [],
                 s = l => {
                     o.push(l);
@@ -1941,24 +1949,24 @@
                         if (c == e || o.indexOf(f) < 0 && s(f)) return !0
                     }
                 };
             if (s(i.contentMatch)) return !0
         }
     }
 
-    function il(n, e) {
+    function al(n, e) {
         for (let t = 0; t < e.length; t++)
             if (n.eq(e[t])) return e[t]
     }
     var ie = class n {
         constructor(e, t) {
             this.nodes = e, this.marks = t
         }
         serializeFragment(e, t = {}, r) {
-            r || (r = Nn(t).createDocumentFragment());
+            r || (r = Dn(t).createDocumentFragment());
             let i = r,
                 o = [];
             return e.forEach(s => {
                 if (o.length || s.marks.length) {
                     let l = 0,
                         a = 0;
                     for (; l < o.length && a < s.marks.length;) {
@@ -1980,15 +1988,15 @@
                 i.appendChild(this.serializeNodeInner(s, t))
             }), r
         }
         serializeNodeInner(e, t) {
             let {
                 dom: r,
                 contentDOM: i
-            } = n.renderSpec(Nn(t), this.nodes[e.type.name](e));
+            } = n.renderSpec(Dn(t), this.nodes[e.type.name](e));
             if (i) {
                 if (e.isLeaf) throw new RangeError("Content hole not allowed in a leaf node spec");
                 this.serializeFragment(e.content, t, i)
             }
             return r
         }
         serializeNode(e, t = {}) {
@@ -1997,15 +2005,15 @@
                 let o = this.serializeMark(e.marks[i], e.isInline, t);
                 o && ((o.contentDOM || o.dom).appendChild(r), r = o.dom)
             }
             return r
         }
         serializeMark(e, t, r = {}) {
             let i = this.marks[e.type.name];
-            return i && n.renderSpec(Nn(r), i(e, t))
+            return i && n.renderSpec(Dn(r), i(e, t))
         }
         static renderSpec(e, t, r = null) {
             if (typeof t == "string") return {
                 dom: e.createTextNode(t)
             };
             if (t.nodeType != null) return {
                 dom: t
@@ -2049,79 +2057,79 @@
                 contentDOM: s
             }
         }
         static fromSchema(e) {
             return e.cached.domSerializer || (e.cached.domSerializer = new n(this.nodesFromSchema(e), this.marksFromSchema(e)))
         }
         static nodesFromSchema(e) {
-            let t = $r(e.nodes);
+            let t = jr(e.nodes);
             return t.text || (t.text = r => r.text), t
         }
         static marksFromSchema(e) {
-            return $r(e.marks)
+            return jr(e.marks)
         }
     };
 
-    function $r(n) {
+    function jr(n) {
         let e = {};
         for (let t in n) {
             let r = n[t].spec.toDOM;
             r && (e[t] = r)
         }
         return e
     }
 
-    function Nn(n) {
+    function Dn(n) {
         return n.document || window.document
     }
-    var ai = 65535,
-        ci = Math.pow(2, 16);
+    var fi = 65535,
+        hi = Math.pow(2, 16);
 
-    function ol(n, e) {
-        return n + e * ci
+    function cl(n, e) {
+        return n + e * hi
     }
 
-    function si(n) {
-        return n & ai
+    function ai(n) {
+        return n & fi
     }
 
-    function sl(n) {
-        return (n - (n & ai)) / ci
+    function fl(n) {
+        return (n - (n & fi)) / hi
     }
-    var fi = 1,
-        hi = 2,
-        Kt = 4,
-        di = 8,
+    var di = 1,
+        ui = 2,
+        $t = 4,
+        pi = 8,
         dt = class {
             constructor(e, t, r) {
                 this.pos = e, this.delInfo = t, this.recover = r
             }
             get deleted() {
-                return (this.delInfo & di) > 0
+                return (this.delInfo & pi) > 0
             }
             get deletedBefore() {
-                return (this.delInfo & (fi | Kt)) > 0
+                return (this.delInfo & (di | $t)) > 0
             }
             get deletedAfter() {
-                return (this.delInfo & (hi | Kt)) > 0
+                return (this.delInfo & (ui | $t)) > 0
             }
             get deletedAcross() {
-                return (this.delInfo & Kt) > 0
+                return (this.delInfo & $t) > 0
             }
         },
         he = class n {
             constructor(e, t = !1) {
                 if (this.ranges = e, this.inverted = t, !e.length && n.empty) return n.empty
             }
             recover(e) {
                 let t = 0,
-                    r = si(e);
+                    r = ai(e);
                 if (!this.inverted)
                     for (let i = 0; i < r; i++) t += this.ranges[i * 3 + 2] - this.ranges[i * 3 + 1];
-                return this.ranges[r * 3] + t + sl(e)
+                return this.ranges[r * 3] + t + fl(e)
             }
             mapResult(e, t = 1) {
                 return this._map(e, t, !1)
             }
             map(e, t = 1) {
                 return this._map(e, t, !0)
             }
@@ -2135,25 +2143,25 @@
                     let c = this.ranges[l + o],
                         f = this.ranges[l + s],
                         h = a + c;
                     if (e <= h) {
                         let d = c ? e == a ? -1 : e == h ? 1 : t : t,
                             p = a + i + (d < 0 ? 0 : f);
                         if (r) return p;
-                        let u = e == (t < 0 ? a : h) ? null : ol(l / 3, e - a),
-                            m = e == a ? hi : e == h ? fi : Kt;
-                        return (t < 0 ? e != a : e != h) && (m |= di), new dt(p, m, u)
+                        let u = e == (t < 0 ? a : h) ? null : cl(l / 3, e - a),
+                            m = e == a ? ui : e == h ? di : $t;
+                        return (t < 0 ? e != a : e != h) && (m |= pi), new dt(p, m, u)
                     }
                     i += f - c
                 }
                 return r ? e + i : new dt(e + i, 0, null)
             }
             touches(e, t) {
                 let r = 0,
-                    i = si(t),
+                    i = ai(t),
                     o = this.inverted ? 2 : 1,
                     s = this.inverted ? 1 : 2;
                 for (let l = 0; l < this.ranges.length; l += 3) {
                     let a = this.ranges[l] - (this.inverted ? r : 0);
                     if (a > e) break;
                     let c = this.ranges[l + o],
                         f = a + c;
@@ -2244,74 +2252,74 @@
                         }
                     }
                     i |= l.delInfo, e = l.pos
                 }
                 return r ? e : new dt(e, i, null)
             }
         },
-        An = Object.create(null),
+        vn = Object.create(null),
         D = class {
             getMap() {
                 return he.empty
             }
             merge(e) {
                 return null
             }
             static fromJSON(e, t) {
                 if (!t || !t.stepType) throw new RangeError("Invalid input for Step.fromJSON");
-                let r = An[t.stepType];
+                let r = vn[t.stepType];
                 if (!r) throw new RangeError(`No step type ${t.stepType} defined`);
                 return r.fromJSON(e, t)
             }
             static jsonID(e, t) {
-                if (e in An) throw new RangeError("Duplicate use of step JSON ID " + e);
-                return An[e] = t, t.prototype.jsonID = e, t
+                if (e in vn) throw new RangeError("Duplicate use of step JSON ID " + e);
+                return vn[e] = t, t.prototype.jsonID = e, t
             }
         },
-        I = class n {
+        A = class n {
             constructor(e, t) {
                 this.doc = e, this.failed = t
             }
             static ok(e) {
                 return new n(e, null)
             }
             static fail(e) {
                 return new n(null, e)
             }
             static fromReplace(e, t, r, i) {
                 try {
                     return n.ok(e.replace(t, r, i))
                 } catch (o) {
-                    if (o instanceof Ie) return n.fail(o.message);
+                    if (o instanceof Ae) return n.fail(o.message);
                     throw o
                 }
             }
         };
 
-    function Bn(n, e, t) {
+    function Vn(n, e, t) {
         let r = [];
         for (let i = 0; i < n.childCount; i++) {
             let o = n.child(i);
-            o.content.size && (o = o.copy(Bn(o.content, e, o))), o.isInline && (o = e(o, t, i)), r.push(o)
+            o.content.size && (o = o.copy(Vn(o.content, e, o))), o.isInline && (o = e(o, t, i)), r.push(o)
         }
         return y.fromArray(r)
     }
     var pt = class n extends D {
         constructor(e, t, r) {
             super(), this.from = e, this.to = t, this.mark = r
         }
         apply(e) {
             let t = e.slice(this.from, this.to),
                 r = e.resolve(this.from),
                 i = r.node(r.sharedDepth(this.to)),
-                o = new b(Bn(t.content, (s, l) => !s.isAtom || !l.type.allowsMarkType(this.mark.type) ? s : s.mark(this.mark.addToSet(s.marks)), i), t.openStart, t.openEnd);
-            return I.fromReplace(e, this.from, this.to, o)
+                o = new b(Vn(t.content, (s, l) => !s.isAtom || !l.type.allowsMarkType(this.mark.type) ? s : s.mark(this.mark.addToSet(s.marks)), i), t.openStart, t.openEnd);
+            return A.fromReplace(e, this.from, this.to, o)
         }
         invert() {
-            return new ze(this.from, this.to, this.mark)
+            return new Pe(this.from, this.to, this.mark)
         }
         map(e) {
             let t = e.mapResult(this.from, 1),
                 r = e.mapResult(this.to, -1);
             return t.deleted && r.deleted || t.pos >= r.pos ? null : new n(t.pos, r.pos, this.mark)
         }
         merge(e) {
@@ -2327,22 +2335,22 @@
         }
         static fromJSON(e, t) {
             if (typeof t.from != "number" || typeof t.to != "number") throw new RangeError("Invalid input for AddMarkStep.fromJSON");
             return new n(t.from, t.to, e.markFromJSON(t.mark))
         }
     };
     D.jsonID("addMark", pt);
-    var ze = class n extends D {
+    var Pe = class n extends D {
         constructor(e, t, r) {
             super(), this.from = e, this.to = t, this.mark = r
         }
         apply(e) {
             let t = e.slice(this.from, this.to),
-                r = new b(Bn(t.content, i => i.mark(this.mark.removeFromSet(i.marks)), e), t.openStart, t.openEnd);
-            return I.fromReplace(e, this.from, this.to, r)
+                r = new b(Vn(t.content, i => i.mark(this.mark.removeFromSet(i.marks)), e), t.openStart, t.openEnd);
+            return A.fromReplace(e, this.from, this.to, r)
         }
         invert() {
             return new pt(this.from, this.to, this.mark)
         }
         map(e) {
             let t = e.mapResult(this.from, 1),
                 r = e.mapResult(this.to, -1);
@@ -2360,24 +2368,24 @@
             }
         }
         static fromJSON(e, t) {
             if (typeof t.from != "number" || typeof t.to != "number") throw new RangeError("Invalid input for RemoveMarkStep.fromJSON");
             return new n(t.from, t.to, e.markFromJSON(t.mark))
         }
     };
-    D.jsonID("removeMark", ze);
+    D.jsonID("removeMark", Pe);
     var mt = class n extends D {
         constructor(e, t) {
             super(), this.pos = e, this.mark = t
         }
         apply(e) {
             let t = e.nodeAt(this.pos);
-            if (!t) return I.fail("No node at mark step's position");
+            if (!t) return A.fail("No node at mark step's position");
             let r = t.type.create(t.attrs, null, this.mark.addToSet(t.marks));
-            return I.fromReplace(e, this.pos, this.pos + 1, new b(y.from(r), 0, t.isLeaf ? 0 : 1))
+            return A.fromReplace(e, this.pos, this.pos + 1, new b(y.from(r), 0, t.isLeaf ? 0 : 1))
         }
         invert(e) {
             let t = e.nodeAt(this.pos);
             if (t) {
                 let r = this.mark.addToSet(t.marks);
                 if (r.length == t.marks.length) {
                     for (let i = 0; i < t.marks.length; i++)
@@ -2406,17 +2414,17 @@
     D.jsonID("addNodeMark", mt);
     var gt = class n extends D {
         constructor(e, t) {
             super(), this.pos = e, this.mark = t
         }
         apply(e) {
             let t = e.nodeAt(this.pos);
-            if (!t) return I.fail("No node at mark step's position");
+            if (!t) return A.fail("No node at mark step's position");
             let r = t.type.create(t.attrs, null, this.mark.removeFromSet(t.marks));
-            return I.fromReplace(e, this.pos, this.pos + 1, new b(y.from(r), 0, t.isLeaf ? 0 : 1))
+            return A.fromReplace(e, this.pos, this.pos + 1, new b(y.from(r), 0, t.isLeaf ? 0 : 1))
         }
         invert(e) {
             let t = e.nodeAt(this.pos);
             return !t || !this.mark.isInSet(t.marks) ? this : new mt(this.pos, this.mark)
         }
         map(e) {
             let t = e.mapResult(this.pos, 1);
@@ -2436,15 +2444,15 @@
     };
     D.jsonID("removeNodeMark", gt);
     var K = class n extends D {
         constructor(e, t, r, i = !1) {
             super(), this.from = e, this.to = t, this.slice = r, this.structure = i
         }
         apply(e) {
-            return this.structure && Rn(e, this.from, this.to) ? I.fail("Structure replace would overwrite content") : I.fromReplace(e, this.from, this.to, this.slice)
+            return this.structure && Bn(e, this.from, this.to) ? A.fail("Structure replace would overwrite content") : A.fromReplace(e, this.from, this.to, this.slice)
         }
         getMap() {
             return new he([this.from, this.to - this.from, this.slice.size])
         }
         invert(e) {
             return new n(this.from, this.from + this.slice.size, e.slice(this.from, this.to))
         }
@@ -2478,19 +2486,19 @@
     };
     D.jsonID("replace", K);
     var E = class n extends D {
         constructor(e, t, r, i, o, s, l = !1) {
             super(), this.from = e, this.to = t, this.gapFrom = r, this.gapTo = i, this.slice = o, this.insert = s, this.structure = l
         }
         apply(e) {
-            if (this.structure && (Rn(e, this.from, this.gapFrom) || Rn(e, this.gapTo, this.to))) return I.fail("Structure gap-replace would overwrite content");
+            if (this.structure && (Bn(e, this.from, this.gapFrom) || Bn(e, this.gapTo, this.to))) return A.fail("Structure gap-replace would overwrite content");
             let t = e.slice(this.gapFrom, this.gapTo);
-            if (t.openStart || t.openEnd) return I.fail("Gap is not a flat range");
+            if (t.openStart || t.openEnd) return A.fail("Gap is not a flat range");
             let r = this.slice.insertAt(this.insert, t.content);
-            return r ? I.fromReplace(e, this.from, this.to, r) : I.fail("Content does not fit in gap")
+            return r ? A.fromReplace(e, this.from, this.to, r) : A.fail("Content does not fit in gap")
         }
         getMap() {
             return new he([this.from, this.gapFrom - this.from, this.insert, this.gapTo, this.to - this.gapTo, this.slice.size - this.insert])
         }
         invert(e) {
             let t = this.gapTo - this.gapFrom;
             return new n(this.from, this.from + this.slice.size + t, this.from + this.insert, this.from + this.insert + t, e.slice(this.from, this.to).removeBetween(this.gapFrom - this.from, this.gapTo - this.from), this.gapFrom - this.from, this.structure)
@@ -2516,47 +2524,47 @@
         static fromJSON(e, t) {
             if (typeof t.from != "number" || typeof t.to != "number" || typeof t.gapFrom != "number" || typeof t.gapTo != "number" || typeof t.insert != "number") throw new RangeError("Invalid input for ReplaceAroundStep.fromJSON");
             return new n(t.from, t.to, t.gapFrom, t.gapTo, b.fromJSON(e, t.slice), t.insert, !!t.structure)
         }
     };
     D.jsonID("replaceAround", E);
 
-    function Rn(n, e, t) {
+    function Bn(n, e, t) {
         let r = n.resolve(e),
             i = t - e,
             o = r.depth;
         for (; i > 0 && o > 0 && r.indexAfter(o) == r.node(o).childCount;) o--, i--;
         if (i > 0) {
             let s = r.node(o).maybeChild(r.indexAfter(o));
             for (; i > 0;) {
                 if (!s || s.isLeaf) return !0;
                 s = s.firstChild, i--
             }
         }
         return !1
     }
 
-    function ll(n, e, t, r) {
+    function hl(n, e, t, r) {
         let i = [],
             o = [],
             s, l;
         n.doc.nodesBetween(e, t, (a, c, f) => {
             if (!a.isInline) return;
             let h = a.marks;
             if (!r.isInSet(h) && f.type.allowsMarkType(r.type)) {
                 let d = Math.max(c, e),
                     p = Math.min(c + a.nodeSize, t),
                     u = r.addToSet(h);
-                for (let m = 0; m < h.length; m++) h[m].isInSet(u) || (s && s.to == d && s.mark.eq(h[m]) ? s.to = p : i.push(s = new ze(d, p, h[m])));
+                for (let m = 0; m < h.length; m++) h[m].isInSet(u) || (s && s.to == d && s.mark.eq(h[m]) ? s.to = p : i.push(s = new Pe(d, p, h[m])));
                 l && l.to == d ? l.to = p : o.push(l = new pt(d, p, r))
             }
         }), i.forEach(a => n.step(a)), o.forEach(a => n.step(a))
     }
 
-    function al(n, e, t, r) {
+    function dl(n, e, t, r) {
         let i = [],
             o = 0;
         n.doc.nodesBetween(e, t, (s, l) => {
             if (!s.isInline) return;
             o++;
             let a = null;
             if (r instanceof ct) {
@@ -2577,29 +2585,29 @@
                         style: h,
                         from: Math.max(l, e),
                         to: c,
                         step: o
                     })
                 }
             }
-        }), i.forEach(s => n.step(new ze(s.from, s.to, s.style)))
+        }), i.forEach(s => n.step(new Pe(s.from, s.to, s.style)))
     }
 
-    function cl(n, e, t, r = t.contentMatch) {
+    function ul(n, e, t, r = t.contentMatch) {
         let i = n.doc.nodeAt(e),
             o = [],
             s = e + 1;
         for (let l = 0; l < i.childCount; l++) {
             let a = i.child(l),
                 c = s + a.nodeSize,
                 f = r.matchType(a.type);
             if (!f) o.push(new K(s, c, b.empty));
             else {
                 r = f;
-                for (let h = 0; h < a.marks.length; h++) t.allowsMarkType(a.marks[h].type) || n.step(new ze(s, c, a.marks[h]));
+                for (let h = 0; h < a.marks.length; h++) t.allowsMarkType(a.marks[h].type) || n.step(new Pe(s, c, a.marks[h]));
                 if (a.isText && !t.spec.code) {
                     let h, d = /\r?\n|\r/g,
                         p;
                     for (; h = d.exec(a.text);) p || (p = new b(y.from(t.schema.text(" ", t.allowedMarks(a.marks))), 0, 0)), o.push(new K(s + h.index, s + h.index + h[0].length, p))
                 }
             }
             s = c
@@ -2607,127 +2615,127 @@
         if (!r.validEnd) {
             let l = r.fillBefore(y.empty, !0);
             n.replace(s, s, new b(l, 0, 0))
         }
         for (let l = o.length - 1; l >= 0; l--) n.step(o[l])
     }
 
-    function fl(n, e, t) {
+    function pl(n, e, t) {
         return (e == 0 || n.canReplace(e, n.childCount)) && (t == n.childCount || n.canReplace(0, t))
     }
 
     function Ue(n) {
         let t = n.parent.content.cutByIndex(n.startIndex, n.endIndex);
         for (let r = n.depth;; --r) {
             let i = n.$from.node(r),
                 o = n.$from.index(r),
                 s = n.$to.indexAfter(r);
             if (r < n.depth && i.canReplace(o, s, t)) return r;
-            if (r == 0 || i.type.spec.isolating || !fl(i, o, s)) break
+            if (r == 0 || i.type.spec.isolating || !pl(i, o, s)) break
         }
         return null
     }
 
-    function hl(n, e, t) {
+    function ml(n, e, t) {
         let {
             $from: r,
             $to: i,
             depth: o
         } = e, s = r.before(o + 1), l = i.after(o + 1), a = s, c = l, f = y.empty, h = 0;
         for (let u = o, m = !1; u > t; u--) m || r.index(u) > 0 ? (m = !0, f = y.from(r.node(u).copy(f)), h++) : a--;
         let d = y.empty,
             p = 0;
         for (let u = o, m = !1; u > t; u--) m || i.after(u + 1) < i.end(u) ? (m = !0, d = y.from(i.node(u).copy(d)), p++) : c++;
         n.step(new E(a, c, s, l, new b(f.append(d), h, p), f.size - h, !0))
     }
 
-    function _t(n, e, t = null, r = n) {
-        let i = dl(n, e),
-            o = i && ul(r, e);
-        return o ? i.map(li).concat({
+    function Ut(n, e, t = null, r = n) {
+        let i = gl(n, e),
+            o = i && yl(r, e);
+        return o ? i.map(ci).concat({
             type: e,
             attrs: t
-        }).concat(o.map(li)) : null
+        }).concat(o.map(ci)) : null
     }
 
-    function li(n) {
+    function ci(n) {
         return {
             type: n,
             attrs: null
         }
     }
 
-    function dl(n, e) {
+    function gl(n, e) {
         let {
             parent: t,
             startIndex: r,
             endIndex: i
         } = n, o = t.contentMatchAt(r).findWrapping(e);
         if (!o) return null;
         let s = o.length ? o[0] : e;
         return t.canReplaceWith(r, i, s) ? o : null
     }
 
-    function ul(n, e) {
+    function yl(n, e) {
         let {
             parent: t,
             startIndex: r,
             endIndex: i
         } = n, o = t.child(r), s = e.contentMatch.findWrapping(o.type);
         if (!s) return null;
         let a = (s.length ? s[s.length - 1] : e).contentMatch;
         for (let c = r; a && c < i; c++) a = a.matchType(t.child(c).type);
         return !a || !a.validEnd ? null : s
     }
 
-    function pl(n, e, t) {
+    function bl(n, e, t) {
         let r = y.empty;
         for (let s = t.length - 1; s >= 0; s--) {
             if (r.size) {
                 let l = t[s].type.contentMatch.matchFragment(r);
                 if (!l || !l.validEnd) throw new RangeError("Wrapper type given to Transform.wrap does not form valid content of its parent wrapper")
             }
             r = y.from(t[s].type.create(t[s].attrs, r))
         }
         let i = e.start,
             o = e.end;
         n.step(new E(i, o, i, o, new b(r, 0, 0), t.length, !0))
     }
 
-    function ml(n, e, t, r, i) {
+    function kl(n, e, t, r, i) {
         if (!r.isTextblock) throw new RangeError("Type given to setBlockType should be a textblock");
         let o = n.steps.length;
         n.doc.nodesBetween(e, t, (s, l) => {
-            if (s.isTextblock && !s.hasMarkup(r, i) && gl(n.doc, n.mapping.slice(o).map(l), r)) {
+            if (s.isTextblock && !s.hasMarkup(r, i) && xl(n.doc, n.mapping.slice(o).map(l), r)) {
                 n.clearIncompatible(n.mapping.slice(o).map(l, 1), r);
                 let a = n.mapping.slice(o),
                     c = a.map(l, 1),
                     f = a.map(l + s.nodeSize, 1);
                 return n.step(new E(c, f, c + 1, f - 1, new b(y.from(r.create(i, null, s.marks)), 0, 0), 1, !0)), !1
             }
         })
     }
 
-    function gl(n, e, t) {
+    function xl(n, e, t) {
         let r = n.resolve(e),
             i = r.index();
         return r.parent.canReplaceWith(i, i + 1, t)
     }
 
-    function yl(n, e, t, r, i) {
+    function Sl(n, e, t, r, i) {
         let o = n.doc.nodeAt(e);
         if (!o) throw new RangeError("No node at given position");
         t || (t = o.type);
         let s = t.create(r, null, i || o.marks);
         if (o.isLeaf) return n.replaceWith(e, e + o.nodeSize, s);
         if (!t.validContent(o.content)) throw new RangeError("Invalid content for node type " + t.name);
         n.step(new E(e, e + o.nodeSize, e + 1, e + o.nodeSize - 1, new b(y.from(s), 0, 0), 1, !0))
     }
 
-    function ke(n, e, t = 1, r) {
+    function xe(n, e, t = 1, r) {
         let i = n.resolve(e),
             o = i.depth - t,
             s = r && r[r.length - 1] || i.parent;
         if (o < 0 || i.parent.type.spec.isolating || !i.parent.canReplace(i.index(), i.parent.childCount) || !s.type.validContent(i.parent.content.cutByIndex(i.index(), i.parent.childCount))) return !1;
         for (let c = i.depth - 1, f = t - 2; c > o; c--, f--) {
             let h = i.node(c),
                 d = i.index(c);
@@ -2739,42 +2747,42 @@
             if (!h.canReplace(d + 1, h.childCount) || !m.type.validContent(p)) return !1
         }
         let l = i.indexAfter(o),
             a = r && r[0];
         return i.node(o).canReplaceWith(l, l, a ? a.type : i.node(o + 1).type)
     }
 
-    function bl(n, e, t = 1, r) {
+    function Ml(n, e, t = 1, r) {
         let i = n.doc.resolve(e),
             o = y.empty,
             s = y.empty;
         for (let l = i.depth, a = i.depth - t, c = t - 1; l > a; l--, c--) {
             o = y.from(i.node(l).copy(o));
             let f = r && r[c];
             s = y.from(f ? f.type.create(f.attrs, s) : i.node(l).copy(s))
         }
         n.step(new K(e, e, new b(o.append(s), t, t), !0))
     }
 
     function yt(n, e) {
         let t = n.resolve(e),
             r = t.index();
-        return kl(t.nodeBefore, t.nodeAfter) && t.parent.canReplace(r, r + 1)
+        return Cl(t.nodeBefore, t.nodeAfter) && t.parent.canReplace(r, r + 1)
     }
 
-    function kl(n, e) {
+    function Cl(n, e) {
         return !!(n && e && !n.isLeaf && n.canAppend(e))
     }
 
-    function xl(n, e, t) {
+    function Ol(n, e, t) {
         let r = new K(e - t, e + t, b.empty, !0);
         n.step(r)
     }
 
-    function Sl(n, e, t) {
+    function wl(n, e, t) {
         let r = n.resolve(e);
         if (r.parent.canReplaceWith(r.index(), r.index(), t)) return e;
         if (r.parentOffset == 0)
             for (let i = r.depth - 1; i >= 0; i--) {
                 let o = r.index(i);
                 if (r.node(i).canReplaceWith(o, o, t)) return r.before(i + 1);
                 if (o > 0) return null
@@ -2784,15 +2792,15 @@
                 let o = r.indexAfter(i);
                 if (r.node(i).canReplaceWith(o, o, t)) return r.after(i + 1);
                 if (o < r.node(i).childCount) return null
             }
         return null
     }
 
-    function Ut(n, e, t) {
+    function Gt(n, e, t) {
         let r = n.resolve(e);
         if (!t.content.size) return e;
         let i = t.content;
         for (let o = 0; o < t.openStart; o++) i = i.firstChild.content;
         for (let o = 1; o <= (t.openStart == 0 && t.size ? 2 : 1); o++)
             for (let s = r.depth; s >= 0; s--) {
                 let l = s == r.depth ? 0 : r.pos <= (r.start(s + 1) + r.end(s + 1)) / 2 ? -1 : 1,
@@ -2805,25 +2813,25 @@
                     f = h && c.canReplaceWith(a, a, h[0])
                 }
                 if (f) return l == 0 ? r.pos : l < 0 ? r.before(s + 1) : r.after(s + 1)
             }
         return null
     }
 
-    function Gt(n, e, t = e, r = b.empty) {
+    function Yt(n, e, t = e, r = b.empty) {
         if (e == t && !r.size) return null;
         let i = n.resolve(e),
             o = n.resolve(t);
-        return ui(i, o, r) ? new K(e, t, r) : new zn(i, o, r).fit()
+        return mi(i, o, r) ? new K(e, t, r) : new Fn(i, o, r).fit()
     }
 
-    function ui(n, e, t) {
+    function mi(n, e, t) {
         return !t.openStart && !t.openEnd && n.start() == e.start() && n.parent.canReplace(n.index(), e.index(), t.content)
     }
-    var zn = class {
+    var Fn = class {
         constructor(e, t, r) {
             this.$from = e, this.$to = t, this.unplaced = r, this.frontier = [], this.placed = y.empty;
             for (let i = 0; i <= e.depth; i++) {
                 let o = e.node(i);
                 this.frontier.push({
                     type: o.type,
                     match: o.contentMatchAt(e.indexAfter(i))
@@ -2860,15 +2868,15 @@
                     break
                 }
                 t = o.content
             }
             for (let t = 1; t <= 2; t++)
                 for (let r = t == 1 ? e : this.unplaced.openStart; r >= 0; r--) {
                     let i, o = null;
-                    r ? (o = vn(this.unplaced.content, r - 1).firstChild, i = o.content) : i = this.unplaced.content;
+                    r ? (o = Pn(this.unplaced.content, r - 1).firstChild, i = o.content) : i = this.unplaced.content;
                     let s = i.firstChild;
                     for (let l = this.depth; l >= 0; l--) {
                         let {
                             type: a,
                             match: c
                         } = this.frontier[l], f, h = null;
                         if (t == 1 && (s ? c.matchType(s.type) || (h = c.fillBefore(y.from(s), !1)) : o && a.compatibleContent(o.type))) return {
@@ -2888,23 +2896,23 @@
                 }
         }
         openMore() {
             let {
                 content: e,
                 openStart: t,
                 openEnd: r
-            } = this.unplaced, i = vn(e, t);
+            } = this.unplaced, i = Pn(e, t);
             return !i.childCount || i.firstChild.isLeaf ? !1 : (this.unplaced = new b(e, t + 1, Math.max(r, i.size + t >= e.size - r ? t + 1 : 0)), !0)
         }
         dropNode() {
             let {
                 content: e,
                 openStart: t,
                 openEnd: r
-            } = this.unplaced, i = vn(e, t);
+            } = this.unplaced, i = Pn(e, t);
             if (i.childCount <= 1 && t > 0) {
                 let o = e.size - t <= t + i.size;
                 this.unplaced = new b(ft(e, t - 1, 1), t - 1, o ? t - 1 : r)
             } else this.unplaced = new b(ft(e, t, 1), t, r)
         }
         placeNodes({
             sliceDepth: e,
@@ -2930,15 +2938,15 @@
                 h = h.matchFragment(i)
             }
             let p = l.size + e - (s.content.size - s.openEnd);
             for (; c < l.childCount;) {
                 let m = l.child(c),
                     g = h.matchType(m.type);
                 if (!g) break;
-                c++, (c > 1 || a == 0 || m.content.size) && (h = g, f.push(pi(m.mark(d.allowedMarks(m.marks)), c == 1 ? a : 0, c == l.childCount ? p : -1)))
+                c++, (c > 1 || a == 0 || m.content.size) && (h = g, f.push(gi(m.mark(d.allowedMarks(m.marks)), c == 1 ? a : 0, c == l.childCount ? p : -1)))
             }
             let u = c == l.childCount;
             u || (p = -1), this.placed = ht(this.placed, t, y.from(f)), this.frontier[t].match = h, u && p < 0 && r && r.type == this.frontier[this.depth].type && this.frontier.length > 1 && this.closeFrontierNode();
             for (let m = 0, g = l; m < p; m++) {
                 let k = g.lastChild;
                 this.frontier.push({
                     type: k.type,
@@ -2947,33 +2955,33 @@
             }
             this.unplaced = u ? e == 0 ? b.empty : new b(ft(s.content, e - 1, 1), e - 1, p < 0 ? s.openEnd : e - 1) : new b(ft(s.content, e, c), s.openStart, s.openEnd)
         }
         mustMoveInline() {
             if (!this.$to.parent.isTextblock) return -1;
             let e = this.frontier[this.depth],
                 t;
-            if (!e.type.isTextblock || !Pn(this.$to, this.$to.depth, e.type, e.match, !1) || this.$to.depth == this.depth && (t = this.findCloseLevel(this.$to)) && t.depth == this.depth) return -1;
+            if (!e.type.isTextblock || !zn(this.$to, this.$to.depth, e.type, e.match, !1) || this.$to.depth == this.depth && (t = this.findCloseLevel(this.$to)) && t.depth == this.depth) return -1;
             let {
                 depth: r
             } = this.$to, i = this.$to.after(r);
             for (; r > 1 && i == this.$to.end(--r);) ++i;
             return i
         }
         findCloseLevel(e) {
             e: for (let t = Math.min(this.depth, e.depth); t >= 0; t--) {
                 let {
                     match: r,
                     type: i
-                } = this.frontier[t], o = t < e.depth && e.end(t + 1) == e.pos + (e.depth - (t + 1)), s = Pn(e, t, i, r, o);
+                } = this.frontier[t], o = t < e.depth && e.end(t + 1) == e.pos + (e.depth - (t + 1)), s = zn(e, t, i, r, o);
                 if (s) {
                     for (let l = t - 1; l >= 0; l--) {
                         let {
                             match: a,
                             type: c
-                        } = this.frontier[l], f = Pn(e, l, c, a, !0);
+                        } = this.frontier[l], f = zn(e, l, c, a, !0);
                         if (!f || f.childCount) continue e
                     }
                     return {
                         depth: t,
                         fit: s,
                         move: o ? e.doc.resolve(e.after(t + 1)) : e
                     }
@@ -3009,49 +3017,49 @@
         return e == 0 ? n.cutByIndex(t, n.childCount) : n.replaceChild(0, n.firstChild.copy(ft(n.firstChild.content, e - 1, t)))
     }
 
     function ht(n, e, t) {
         return e == 0 ? n.append(t) : n.replaceChild(n.childCount - 1, n.lastChild.copy(ht(n.lastChild.content, e - 1, t)))
     }
 
-    function vn(n, e) {
+    function Pn(n, e) {
         for (let t = 0; t < e; t++) n = n.firstChild.content;
         return n
     }
 
-    function pi(n, e, t) {
+    function gi(n, e, t) {
         if (e <= 0) return n;
         let r = n.content;
-        return e > 1 && (r = r.replaceChild(0, pi(r.firstChild, e - 1, r.childCount == 1 ? t - 1 : 0))), e > 0 && (r = n.type.contentMatch.fillBefore(r).append(r), t <= 0 && (r = r.append(n.type.contentMatch.matchFragment(r).fillBefore(y.empty, !0)))), n.copy(r)
+        return e > 1 && (r = r.replaceChild(0, gi(r.firstChild, e - 1, r.childCount == 1 ? t - 1 : 0))), e > 0 && (r = n.type.contentMatch.fillBefore(r).append(r), t <= 0 && (r = r.append(n.type.contentMatch.matchFragment(r).fillBefore(y.empty, !0)))), n.copy(r)
     }
 
-    function Pn(n, e, t, r, i) {
+    function zn(n, e, t, r, i) {
         let o = n.node(e),
             s = i ? n.indexAfter(e) : n.index(e);
         if (s == o.childCount && !t.compatibleContent(o.type)) return null;
         let l = r.fillBefore(o.content, !0, s);
-        return l && !Ml(t, o.content, s) ? l : null
+        return l && !Nl(t, o.content, s) ? l : null
     }
 
-    function Ml(n, e, t) {
+    function Nl(n, e, t) {
         for (let r = t; r < e.childCount; r++)
             if (!n.allowsMarks(e.child(r).marks)) return !0;
         return !1
     }
 
-    function Cl(n) {
+    function Tl(n) {
         return n.spec.defining || n.spec.definingForContent
     }
 
-    function Ol(n, e, t, r) {
+    function Dl(n, e, t, r) {
         if (!r.size) return n.deleteRange(e, t);
         let i = n.doc.resolve(e),
             o = n.doc.resolve(t);
-        if (ui(i, o, r)) return n.step(new K(e, t, r));
-        let s = gi(i, n.doc.resolve(t));
+        if (mi(i, o, r)) return n.step(new K(e, t, r));
+        let s = bi(i, n.doc.resolve(t));
         s[s.length - 1] == 0 && s.pop();
         let l = -(i.depth + 1);
         s.unshift(l);
         for (let d = i.depth, p = i.pos - 1; d > 0; d--, p--) {
             let u = i.node(d).type.spec;
             if (u.defining || u.definingAsContext || u.isolating) break;
             s.indexOf(d) > -1 ? l = d : i.before(d) == p && s.splice(1, 0, -d)
@@ -3062,96 +3070,96 @@
         for (let d = r.content, p = 0;; p++) {
             let u = d.firstChild;
             if (c.push(u), p == r.openStart) break;
             d = u.content
         }
         for (let d = f - 1; d >= 0; d--) {
             let p = c[d],
-                u = Cl(p.type);
+                u = Tl(p.type);
             if (u && !p.sameMarkup(i.node(Math.abs(l) - 1))) f = d;
             else if (u || !p.type.isTextblock) break
         }
         for (let d = r.openStart; d >= 0; d--) {
             let p = (d + f + 1) % (r.openStart + 1),
                 u = c[p];
             if (u)
                 for (let m = 0; m < s.length; m++) {
                     let g = s[(m + a) % s.length],
                         k = !0;
                     g < 0 && (k = !1, g = -g);
                     let N = i.node(g - 1),
                         ee = i.index(g - 1);
-                    if (N.canReplaceWith(ee, ee, u.type, u.marks)) return n.replace(i.before(g), k ? o.after(g) : t, new b(mi(r.content, 0, r.openStart, p), p, r.openEnd))
+                    if (N.canReplaceWith(ee, ee, u.type, u.marks)) return n.replace(i.before(g), k ? o.after(g) : t, new b(yi(r.content, 0, r.openStart, p), p, r.openEnd))
                 }
         }
         let h = n.steps.length;
         for (let d = s.length - 1; d >= 0 && (n.replace(e, t, r), !(n.steps.length > h)); d--) {
             let p = s[d];
             p < 0 || (e = i.before(p), t = o.after(p))
         }
     }
 
-    function mi(n, e, t, r, i) {
+    function yi(n, e, t, r, i) {
         if (e < t) {
             let o = n.firstChild;
-            n = n.replaceChild(0, o.copy(mi(o.content, e + 1, t, r, o)))
+            n = n.replaceChild(0, o.copy(yi(o.content, e + 1, t, r, o)))
         }
         if (e > r) {
             let o = i.contentMatchAt(0),
                 s = o.fillBefore(n).append(n);
             n = s.append(o.matchFragment(s).fillBefore(y.empty, !0))
         }
         return n
     }
 
-    function wl(n, e, t, r) {
+    function El(n, e, t, r) {
         if (!r.isInline && e == t && n.doc.resolve(e).parent.content.size) {
-            let i = Sl(n.doc, e, r.type);
+            let i = wl(n.doc, e, r.type);
             i != null && (e = t = i)
         }
         n.replaceRange(e, t, new b(y.from(r), 0, 0))
     }
 
-    function Nl(n, e, t) {
+    function Al(n, e, t) {
         let r = n.doc.resolve(e),
             i = n.doc.resolve(t),
-            o = gi(r, i);
+            o = bi(r, i);
         for (let s = 0; s < o.length; s++) {
             let l = o[s],
                 a = s == o.length - 1;
             if (a && l == 0 || r.node(l).type.contentMatch.validEnd) return n.delete(r.start(l), i.end(l));
             if (l > 0 && (a || r.node(l - 1).canReplace(r.index(l - 1), i.indexAfter(l - 1)))) return n.delete(r.before(l), i.after(l))
         }
         for (let s = 1; s <= r.depth && s <= i.depth; s++)
             if (e - r.start(s) == r.depth - s && t > r.end(s) && i.end(s) - t != i.depth - s) return n.delete(r.before(s), t);
         n.delete(e, t)
     }
 
-    function gi(n, e) {
+    function bi(n, e) {
         let t = [],
             r = Math.min(n.depth, e.depth);
         for (let i = r; i >= 0; i--) {
             let o = n.start(i);
             if (o < n.pos - (n.depth - i) || e.end(i) > e.pos + (e.depth - i) || n.node(i).type.spec.isolating || e.node(i).type.spec.isolating) break;
             (o == e.start(i) || i == n.depth && i == e.depth && n.parent.inlineContent && e.parent.inlineContent && i && e.start(i - 1) == o - 1) && t.push(i)
         }
         return t
     }
-    var $t = class n extends D {
+    var Ht = class n extends D {
         constructor(e, t, r) {
             super(), this.pos = e, this.attr = t, this.value = r
         }
         apply(e) {
             let t = e.nodeAt(this.pos);
-            if (!t) return I.fail("No node at attribute step's position");
+            if (!t) return A.fail("No node at attribute step's position");
             let r = Object.create(null);
             for (let o in t.attrs) r[o] = t.attrs[o];
             r[this.attr] = this.value;
             let i = t.type.create(r, null, t.marks);
-            return I.fromReplace(e, this.pos, this.pos + 1, new b(y.from(i), 0, t.isLeaf ? 0 : 1))
+            return A.fromReplace(e, this.pos, this.pos + 1, new b(y.from(i), 0, t.isLeaf ? 0 : 1))
         }
         getMap() {
             return he.empty
         }
         invert(e) {
             return new n(this.pos, this.attr, e.nodeAt(this.pos).attrs[this.attr])
         }
@@ -3168,25 +3176,25 @@
             }
         }
         static fromJSON(e, t) {
             if (typeof t.pos != "number" || typeof t.attr != "string") throw new RangeError("Invalid input for AttrStep.fromJSON");
             return new n(t.pos, t.attr, t.value)
         }
     };
-    D.jsonID("attr", $t);
-    var Ht = class n extends D {
+    D.jsonID("attr", Ht);
+    var jt = class n extends D {
         constructor(e, t) {
             super(), this.attr = e, this.value = t
         }
         apply(e) {
             let t = Object.create(null);
             for (let i in e.attrs) t[i] = e.attrs[i];
             t[this.attr] = this.value;
             let r = e.type.create(t, e.content, e.marks);
-            return I.ok(r)
+            return A.ok(r)
         }
         getMap() {
             return he.empty
         }
         invert(e) {
             return new n(this.attr, e.attrs[this.attr])
         }
@@ -3201,24 +3209,24 @@
             }
         }
         static fromJSON(e, t) {
             if (typeof t.attr != "string") throw new RangeError("Invalid input for DocAttrStep.fromJSON");
             return new n(t.attr, t.value)
         }
     };
-    D.jsonID("docAttr", Ht);
+    D.jsonID("docAttr", jt);
     var _e = class extends Error {};
     _e = function n(e) {
         let t = Error.call(this, e);
         return t.__proto__ = n.prototype, t
     };
     _e.prototype = Object.create(Error.prototype);
     _e.prototype.constructor = _e;
     _e.prototype.name = "TransformError";
-    var jt = class {
+    var _t = class {
         constructor(e) {
             this.doc = e, this.steps = [], this.docs = [], this.mapping = new ut
         }
         get before() {
             return this.docs.length ? this.docs[0] : this.doc
         }
         step(e) {
@@ -3233,84 +3241,84 @@
         get docChanged() {
             return this.steps.length > 0
         }
         addStep(e, t) {
             this.docs.push(this.doc), this.steps.push(e), this.mapping.appendMap(e.getMap()), this.doc = t
         }
         replace(e, t = e, r = b.empty) {
-            let i = Gt(this.doc, e, t, r);
+            let i = Yt(this.doc, e, t, r);
             return i && this.step(i), this
         }
         replaceWith(e, t, r) {
             return this.replace(e, t, new b(y.from(r), 0, 0))
         }
         delete(e, t) {
             return this.replace(e, t, b.empty)
         }
         insert(e, t) {
             return this.replaceWith(e, e, t)
         }
         replaceRange(e, t, r) {
-            return Ol(this, e, t, r), this
+            return Dl(this, e, t, r), this
         }
         replaceRangeWith(e, t, r) {
-            return wl(this, e, t, r), this
+            return El(this, e, t, r), this
         }
         deleteRange(e, t) {
-            return Nl(this, e, t), this
+            return Al(this, e, t), this
         }
         lift(e, t) {
-            return hl(this, e, t), this
+            return ml(this, e, t), this
         }
         join(e, t = 1) {
-            return xl(this, e, t), this
+            return Ol(this, e, t), this
         }
         wrap(e, t) {
-            return pl(this, e, t), this
+            return bl(this, e, t), this
         }
         setBlockType(e, t = e, r, i = null) {
-            return ml(this, e, t, r, i), this
+            return kl(this, e, t, r, i), this
         }
         setNodeMarkup(e, t, r = null, i) {
-            return yl(this, e, t, r, i), this
+            return Sl(this, e, t, r, i), this
         }
         setNodeAttribute(e, t, r) {
-            return this.step(new $t(e, t, r)), this
+            return this.step(new Ht(e, t, r)), this
         }
         setDocAttribute(e, t) {
-            return this.step(new Ht(e, t)), this
+            return this.step(new jt(e, t)), this
         }
         addNodeMark(e, t) {
             return this.step(new mt(e, t)), this
         }
         removeNodeMark(e, t) {
             if (!(t instanceof O)) {
                 let r = this.doc.nodeAt(e);
                 if (!r) throw new RangeError("No node at position " + e);
                 if (t = t.isInSet(r.marks), !t) return this
             }
             return this.step(new gt(e, t)), this
         }
         split(e, t = 1, r) {
-            return bl(this, e, t, r), this
+            return Ml(this, e, t, r), this
         }
         addMark(e, t, r) {
-            return ll(this, e, t, r), this
+            return hl(this, e, t, r), this
         }
         removeMark(e, t, r) {
-            return al(this, e, t, r), this
+            return dl(this, e, t, r), this
         }
         clearIncompatible(e, t, r) {
-            return cl(this, e, t, r), this
+            return ul(this, e, t, r), this
         }
     };
-    var Fn = Object.create(null),
+    var Ln = Object.create(null),
         S = class {
             constructor(e, t, r) {
-                this.$anchor = e, this.$head = t, this.ranges = r || [new Ln(e.min(t), e.max(t))]
+                this.$anchor = e, this.$head = t, this.ranges = r || [new Wn(e.min(t), e.max(t))]
             }
             get anchor() {
                 return this.$anchor.pos
             }
             get head() {
                 return this.$head.pos
             }
@@ -3342,26 +3350,26 @@
                 let o = e.steps.length,
                     s = this.ranges;
                 for (let l = 0; l < s.length; l++) {
                     let {
                         $from: a,
                         $to: c
                     } = s[l], f = e.mapping.slice(o);
-                    e.replaceRange(f.map(a.pos), f.map(c.pos), l ? b.empty : t), l == 0 && ki(e, o, (r ? r.isInline : i && i.isTextblock) ? -1 : 1)
+                    e.replaceRange(f.map(a.pos), f.map(c.pos), l ? b.empty : t), l == 0 && Si(e, o, (r ? r.isInline : i && i.isTextblock) ? -1 : 1)
                 }
             }
             replaceWith(e, t) {
                 let r = e.steps.length,
                     i = this.ranges;
                 for (let o = 0; o < i.length; o++) {
                     let {
                         $from: s,
                         $to: l
                     } = i[o], a = e.mapping.slice(r), c = a.map(s.pos), f = a.map(l.pos);
-                    o ? e.deleteRange(c, f) : (e.replaceRangeWith(c, f, t), ki(e, r, t.isInline ? -1 : 1))
+                    o ? e.deleteRange(c, f) : (e.replaceRangeWith(c, f, t), Si(e, r, t.isInline ? -1 : 1))
                 }
             }
             static findFrom(e, t, r = !1) {
                 let i = e.parent.inlineContent ? new w(e) : Ge(e.node(0), e.parent, e.pos, e.index(), t, r);
                 if (i) return i;
                 for (let o = e.depth - 1; o >= 0; o--) {
                     let s = t < 0 ? Ge(e.node(0), e.node(o), e.before(o + 1), e.index(o), t, r) : Ge(e.node(0), e.node(o), e.after(o + 1), e.index(o) + 1, t, r);
@@ -3376,40 +3384,40 @@
                 return Ge(e, e, 0, 0, 1) || new $(e)
             }
             static atEnd(e) {
                 return Ge(e, e, e.content.size, e.childCount, -1) || new $(e)
             }
             static fromJSON(e, t) {
                 if (!t || !t.type) throw new RangeError("Invalid input for Selection.fromJSON");
-                let r = Fn[t.type];
+                let r = Ln[t.type];
                 if (!r) throw new RangeError(`No selection type ${t.type} defined`);
                 return r.fromJSON(e, t)
             }
             static jsonID(e, t) {
-                if (e in Fn) throw new RangeError("Duplicate use of selection JSON ID " + e);
-                return Fn[e] = t, t.prototype.jsonID = e, t
+                if (e in Ln) throw new RangeError("Duplicate use of selection JSON ID " + e);
+                return Ln[e] = t, t.prototype.jsonID = e, t
             }
             getBookmark() {
                 return w.between(this.$anchor, this.$head).getBookmark()
             }
         };
     S.prototype.visible = !0;
-    var Ln = class {
+    var Wn = class {
             constructor(e, t) {
                 this.$from = e, this.$to = t
             }
         },
-        yi = !1;
+        ki = !1;
 
-    function bi(n) {
-        !yi && !n.parent.inlineContent && (yi = !0, console.warn("TextSelection endpoint not pointing into a node with inline content (" + n.parent.type.name + ")"))
+    function xi(n) {
+        !ki && !n.parent.inlineContent && (ki = !0, console.warn("TextSelection endpoint not pointing into a node with inline content (" + n.parent.type.name + ")"))
     }
     var w = class n extends S {
         constructor(e, t = e) {
-            bi(e), bi(t), super(e, t)
+            xi(e), xi(t), super(e, t)
         }
         get $cursor() {
             return this.$anchor.pos == this.$head.pos ? this.$head : null
         }
         map(e, t) {
             let r = e.resolve(t.map(this.head));
             if (!r.parent.inlineContent) return S.near(r);
@@ -3422,15 +3430,15 @@
                 r && e.ensureMarks(r)
             }
         }
         eq(e) {
             return e instanceof n && e.anchor == this.anchor && e.head == this.head
         }
         getBookmark() {
-            return new Xt(this.anchor, this.head)
+            return new Qt(this.anchor, this.head)
         }
         toJSON() {
             return {
                 type: "text",
                 anchor: this.anchor,
                 head: this.head
             }
@@ -3450,15 +3458,15 @@
                 if (o) t = o.$head;
                 else return S.near(t, r)
             }
             return e.parent.inlineContent || (i == 0 ? e = t : (e = (S.findFrom(e, -r, !0) || S.findFrom(e, r, !0)).$anchor, e.pos < t.pos != i < 0 && (e = t))), new n(e, t)
         }
     };
     S.jsonID("text", w);
-    var Xt = class n {
+    var Qt = class n {
             constructor(e, t) {
                 this.anchor = e, this.head = t
             }
             map(e) {
                 return new n(e.map(this.anchor), e.map(this.head))
             }
             resolve(e) {
@@ -3487,39 +3495,39 @@
             toJSON() {
                 return {
                     type: "node",
                     anchor: this.anchor
                 }
             }
             getBookmark() {
-                return new Jn(this.anchor)
+                return new qn(this.anchor)
             }
             static fromJSON(e, t) {
                 if (typeof t.anchor != "number") throw new RangeError("Invalid input for NodeSelection.fromJSON");
                 return new n(e.resolve(t.anchor))
             }
             static create(e, t) {
                 return new n(e.resolve(t))
             }
             static isSelectable(e) {
                 return !e.isText && e.type.spec.selectable !== !1
             }
         };
     x.prototype.visible = !1;
     S.jsonID("node", x);
-    var Jn = class n {
+    var qn = class n {
             constructor(e) {
                 this.anchor = e
             }
             map(e) {
                 let {
                     deleted: t,
                     pos: r
                 } = e.mapResult(this.anchor);
-                return t ? new Xt(r, r) : new n(r)
+                return t ? new Qt(r, r) : new n(r)
             }
             resolve(e) {
                 let t = e.resolve(this.anchor),
                     r = t.nodeAfter;
                 return r && x.isSelectable(r) ? new x(t) : S.near(t)
             }
         },
@@ -3545,19 +3553,19 @@
             map(e) {
                 return new n(e)
             }
             eq(e) {
                 return e instanceof n
             }
             getBookmark() {
-                return Tl
+                return Il
             }
         };
     S.jsonID("all", $);
-    var Tl = {
+    var Il = {
         map() {
             return this
         },
         resolve(n) {
             return new $(n)
         }
     };
@@ -3573,59 +3581,59 @@
                 if (a) return a
             }
             t += l.nodeSize * i
         }
         return null
     }
 
-    function ki(n, e, t) {
+    function Si(n, e, t) {
         let r = n.steps.length - 1;
         if (r < e) return;
         let i = n.steps[r];
         if (!(i instanceof K || i instanceof E)) return;
         let o = n.mapping.maps[r],
             s;
         o.forEach((l, a, c, f) => {
             s == null && (s = f)
         }), n.setSelection(S.near(n.doc.resolve(s), t))
     }
-    var xi = 1,
-        Yt = 2,
-        Si = 4,
-        Wn = class extends jt {
+    var Mi = 1,
+        Xt = 2,
+        Ci = 4,
+        Kn = class extends _t {
             constructor(e) {
                 super(e.doc), this.curSelectionFor = 0, this.updated = 0, this.meta = Object.create(null), this.time = Date.now(), this.curSelection = e.selection, this.storedMarks = e.storedMarks
             }
             get selection() {
                 return this.curSelectionFor < this.steps.length && (this.curSelection = this.curSelection.map(this.doc, this.mapping.slice(this.curSelectionFor)), this.curSelectionFor = this.steps.length), this.curSelection
             }
             setSelection(e) {
                 if (e.$from.doc != this.doc) throw new RangeError("Selection passed to setSelection must point at the current document");
-                return this.curSelection = e, this.curSelectionFor = this.steps.length, this.updated = (this.updated | xi) & ~Yt, this.storedMarks = null, this
+                return this.curSelection = e, this.curSelectionFor = this.steps.length, this.updated = (this.updated | Mi) & ~Xt, this.storedMarks = null, this
             }
             get selectionSet() {
-                return (this.updated & xi) > 0
+                return (this.updated & Mi) > 0
             }
             setStoredMarks(e) {
-                return this.storedMarks = e, this.updated |= Yt, this
+                return this.storedMarks = e, this.updated |= Xt, this
             }
             ensureMarks(e) {
                 return O.sameSet(this.storedMarks || this.selection.$from.marks(), e) || this.setStoredMarks(e), this
             }
             addStoredMark(e) {
                 return this.ensureMarks(e.addToSet(this.storedMarks || this.selection.$head.marks()))
             }
             removeStoredMark(e) {
                 return this.ensureMarks(e.removeFromSet(this.storedMarks || this.selection.$head.marks()))
             }
             get storedMarksSet() {
-                return (this.updated & Yt) > 0
+                return (this.updated & Xt) > 0
             }
             addStep(e, t) {
-                super.addStep(e, t), this.updated = this.updated & ~Yt, this.storedMarks = null
+                super.addStep(e, t), this.updated = this.updated & ~Xt, this.storedMarks = null
             }
             setTime(e) {
                 return this.time = e, this
             }
             replaceSelection(e) {
                 return this.selection.replace(this, e), this
             }
@@ -3656,67 +3664,67 @@
                 return this.meta[typeof e == "string" ? e : e.key]
             }
             get isGeneric() {
                 for (let e in this.meta) return !1;
                 return !0
             }
             scrollIntoView() {
-                return this.updated |= Si, this
+                return this.updated |= Ci, this
             }
             get scrolledIntoView() {
-                return (this.updated & Si) > 0
+                return (this.updated & Ci) > 0
             }
         };
 
-    function Mi(n, e) {
+    function Oi(n, e) {
         return !e || !n ? n : n.bind(e)
     }
-    var Be = class {
+    var ze = class {
             constructor(e, t, r) {
-                this.name = e, this.init = Mi(t.init, r), this.apply = Mi(t.apply, r)
+                this.name = e, this.init = Oi(t.init, r), this.apply = Oi(t.apply, r)
             }
         },
-        Dl = [new Be("doc", {
+        Rl = [new ze("doc", {
             init(n) {
                 return n.doc || n.schema.topNodeType.createAndFill()
             },
             apply(n) {
                 return n.doc
             }
-        }), new Be("selection", {
+        }), new ze("selection", {
             init(n, e) {
                 return n.selection || S.atStart(e.doc)
             },
             apply(n) {
                 return n.selection
             }
-        }), new Be("storedMarks", {
+        }), new ze("storedMarks", {
             init(n) {
                 return n.storedMarks || null
             },
             apply(n, e, t, r) {
                 return r.selection.$cursor ? n.storedMarks : null
             }
-        }), new Be("scrollToSelection", {
+        }), new ze("scrollToSelection", {
             init() {
                 return 0
             },
             apply(n, e) {
                 return n.scrolledIntoView ? e + 1 : e
             }
         })],
         bt = class {
             constructor(e, t) {
-                this.schema = e, this.plugins = [], this.pluginsByKey = Object.create(null), this.fields = Dl.slice(), t && t.forEach(r => {
+                this.schema = e, this.plugins = [], this.pluginsByKey = Object.create(null), this.fields = Rl.slice(), t && t.forEach(r => {
                     if (this.pluginsByKey[r.key]) throw new RangeError("Adding different instances of a keyed plugin (" + r.key + ")");
-                    this.plugins.push(r), this.pluginsByKey[r.key] = r, r.spec.state && this.fields.push(new Be(r.key, r.spec.state, r))
+                    this.plugins.push(r), this.pluginsByKey[r.key] = r, r.spec.state && this.fields.push(new ze(r.key, r.spec.state, r))
                 })
             }
         },
-        Qt = class n {
+        Zt = class n {
             constructor(e) {
                 this.config = e
             }
             get schema() {
                 return this.config.schema
             }
             get plugins() {
@@ -3780,15 +3788,15 @@
                 for (let i = 0; i < r.length; i++) {
                     let o = r[i];
                     t[o.name] = o.apply(e, this[o.name], this, t)
                 }
                 return t
             }
             get tr() {
-                return new Wn(this)
+                return new Kn(this)
             }
             static create(e) {
                 let t = new bt(e.doc ? e.doc.type.schema : e.schema, e.plugins),
                     r = new n(t);
                 for (let i = 0; i < t.fields.length; i++) r[t.fields[i].name] = t.fields[i].init(e, r);
                 return r
             }
@@ -3837,66 +3845,66 @@
                             }
                         o[s.name] = s.init(e, o)
                     }
                 }), o
             }
         };
 
-    function Ci(n, e, t) {
+    function wi(n, e, t) {
         for (let r in n) {
             let i = n[r];
-            i instanceof Function ? i = i.bind(e) : r == "handleDOMEvents" && (i = Ci(i, e, {})), t[r] = i
+            i instanceof Function ? i = i.bind(e) : r == "handleDOMEvents" && (i = wi(i, e, {})), t[r] = i
         }
         return t
     }
     var J = class {
             constructor(e) {
-                this.spec = e, this.props = {}, e.props && Ci(e.props, this, this.props), this.key = e.key ? e.key.key : Oi("plugin")
+                this.spec = e, this.props = {}, e.props && wi(e.props, this, this.props), this.key = e.key ? e.key.key : Ni("plugin")
             }
             getState(e) {
                 return e[this.key]
             }
         },
-        Vn = Object.create(null);
+        Jn = Object.create(null);
 
-    function Oi(n) {
-        return n in Vn ? n + "$" + ++Vn[n] : (Vn[n] = 0, n + "$")
+    function Ni(n) {
+        return n in Jn ? n + "$" + ++Jn[n] : (Jn[n] = 0, n + "$")
     }
     var kt = class {
         constructor(e = "key") {
-            this.key = Oi(e)
+            this.key = Ni(e)
         }
         get(e) {
             return e.config.pluginsByKey[this.key]
         }
         getState(e) {
             return e[this.key]
         }
     };
-    var Ni = (n, e) => n.selection.empty ? !1 : (e && e(n.tr.deleteSelection().scrollIntoView()), !0);
+    var Di = (n, e) => n.selection.empty ? !1 : (e && e(n.tr.deleteSelection().scrollIntoView()), !0);
 
-    function El(n, e) {
+    function vl(n, e) {
         let {
             $cursor: t
         } = n.selection;
         return !t || (e ? !e.endOfTextblock("backward", n) : t.parentOffset > 0) ? null : t
     }
-    var Il = (n, e, t) => {
-        let r = El(n, t);
+    var Pl = (n, e, t) => {
+        let r = vl(n, t);
         if (!r) return !1;
-        let i = Ti(r);
+        let i = Ei(r);
         if (!i) {
             let s = r.blockRange(),
                 l = s && Ue(s);
             return l == null ? !1 : (e && e(n.tr.lift(s, l).scrollIntoView()), !0)
         }
         let o = i.nodeBefore;
-        if (!o.type.spec.isolating && Ei(n, i, e)) return !0;
+        if (!o.type.spec.isolating && Ii(n, i, e)) return !0;
         if (r.parent.content.size == 0 && (Ye(o, "end") || x.isSelectable(o))) {
-            let s = Gt(n.doc, r.before(), r.after(), b.empty);
+            let s = Yt(n.doc, r.before(), r.after(), b.empty);
             if (s && s.slice.size < s.to - s.from) {
                 if (e) {
                     let l = n.tr.step(s);
                     l.setSelection(Ye(o, "end") ? S.findFrom(l.doc.resolve(l.mapping.map(i.pos, -1)), -1) : x.create(l.doc, i.pos - o.nodeSize)), e(l.scrollIntoView())
                 }
                 return !0
             }
@@ -3907,198 +3915,198 @@
     function Ye(n, e, t = !1) {
         for (let r = n; r; r = e == "start" ? r.firstChild : r.lastChild) {
             if (r.isTextblock) return !0;
             if (t && r.childCount != 1) return !1
         }
         return !1
     }
-    var Al = (n, e, t) => {
+    var zl = (n, e, t) => {
         let {
             $head: r,
             empty: i
         } = n.selection, o = r;
         if (!i) return !1;
         if (r.parent.isTextblock) {
             if (t ? !t.endOfTextblock("backward", n) : r.parentOffset > 0) return !1;
-            o = Ti(r)
+            o = Ei(r)
         }
         let s = o && o.nodeBefore;
         return !s || !x.isSelectable(s) ? !1 : (e && e(n.tr.setSelection(x.create(n.doc, o.pos - s.nodeSize)).scrollIntoView()), !0)
     };
 
-    function Ti(n) {
+    function Ei(n) {
         if (!n.parent.type.spec.isolating)
             for (let e = n.depth - 1; e >= 0; e--) {
                 if (n.index(e) > 0) return n.doc.resolve(n.before(e + 1));
                 if (n.node(e).type.spec.isolating) break
             }
         return null
     }
 
-    function vl(n, e) {
+    function Bl(n, e) {
         let {
             $cursor: t
         } = n.selection;
         return !t || (e ? !e.endOfTextblock("forward", n) : t.parentOffset < t.parent.content.size) ? null : t
     }
-    var Pl = (n, e, t) => {
-            let r = vl(n, t);
+    var Fl = (n, e, t) => {
+            let r = Bl(n, t);
             if (!r) return !1;
-            let i = Di(r);
+            let i = Ai(r);
             if (!i) return !1;
             let o = i.nodeAfter;
-            if (Ei(n, i, e)) return !0;
+            if (Ii(n, i, e)) return !0;
             if (r.parent.content.size == 0 && (Ye(o, "start") || x.isSelectable(o))) {
-                let s = Gt(n.doc, r.before(), r.after(), b.empty);
+                let s = Yt(n.doc, r.before(), r.after(), b.empty);
                 if (s && s.slice.size < s.to - s.from) {
                     if (e) {
                         let l = n.tr.step(s);
                         l.setSelection(Ye(o, "start") ? S.findFrom(l.doc.resolve(l.mapping.map(i.pos)), 1) : x.create(l.doc, l.mapping.map(i.pos))), e(l.scrollIntoView())
                     }
                     return !0
                 }
             }
             return o.isAtom && i.depth == r.depth - 1 ? (e && e(n.tr.delete(i.pos, i.pos + o.nodeSize).scrollIntoView()), !0) : !1
         },
-        Rl = (n, e, t) => {
+        Vl = (n, e, t) => {
             let {
                 $head: r,
                 empty: i
             } = n.selection, o = r;
             if (!i) return !1;
             if (r.parent.isTextblock) {
                 if (t ? !t.endOfTextblock("forward", n) : r.parentOffset < r.parent.content.size) return !1;
-                o = Di(r)
+                o = Ai(r)
             }
             let s = o && o.nodeAfter;
             return !s || !x.isSelectable(s) ? !1 : (e && e(n.tr.setSelection(x.create(n.doc, o.pos)).scrollIntoView()), !0)
         };
 
-    function Di(n) {
+    function Ai(n) {
         if (!n.parent.type.spec.isolating)
             for (let e = n.depth - 1; e >= 0; e--) {
                 let t = n.node(e);
                 if (n.index(e) + 1 < t.childCount) return n.doc.resolve(n.after(e + 1));
                 if (t.type.spec.isolating) break
             }
         return null
     }
-    var zl = (n, e) => {
+    var Ll = (n, e) => {
         let {
             $head: t,
             $anchor: r
         } = n.selection;
         return !t.parent.type.spec.code || !t.sameParent(r) ? !1 : (e && e(n.tr.insertText(`
 `).scrollIntoView()), !0)
     };
 
-    function Kn(n) {
+    function Hn(n) {
         for (let e = 0; e < n.edgeCount; e++) {
             let {
                 type: t
             } = n.edge(e);
             if (t.isTextblock && !t.hasRequiredAttrs()) return t
         }
         return null
     }
-    var $n = (n, e) => {
+    var jn = (n, e) => {
             let {
                 $head: t,
                 $anchor: r
             } = n.selection;
             if (!t.parent.type.spec.code || !t.sameParent(r)) return !1;
             let i = t.node(-1),
                 o = t.indexAfter(-1),
-                s = Kn(i.contentMatchAt(o));
+                s = Hn(i.contentMatchAt(o));
             if (!s || !i.canReplaceWith(o, o, s)) return !1;
             if (e) {
                 let l = t.after(),
                     a = n.tr.replaceWith(l, l, s.createAndFill());
                 a.setSelection(S.near(a.doc.resolve(l), 1)), e(a.scrollIntoView())
             }
             return !0
         },
-        Bl = (n, e) => {
+        Jl = (n, e) => {
             let t = n.selection,
                 {
                     $from: r,
                     $to: i
                 } = t;
             if (t instanceof $ || r.parent.inlineContent || i.parent.inlineContent) return !1;
-            let o = Kn(i.parent.contentMatchAt(i.indexAfter()));
+            let o = Hn(i.parent.contentMatchAt(i.indexAfter()));
             if (!o || !o.isTextblock) return !1;
             if (e) {
                 let s = (!r.parentOffset && i.index() < i.parent.childCount ? r : i).pos,
                     l = n.tr.insert(s, o.createAndFill());
                 l.setSelection(w.create(l.doc, s + 1)), e(l.scrollIntoView())
             }
             return !0
         },
-        Fl = (n, e) => {
+        Wl = (n, e) => {
             let {
                 $cursor: t
             } = n.selection;
             if (!t || t.parent.content.size) return !1;
             if (t.depth > 1 && t.after() != t.end(-1)) {
                 let o = t.before();
-                if (ke(n.doc, o)) return e && e(n.tr.split(o).scrollIntoView()), !0
+                if (xe(n.doc, o)) return e && e(n.tr.split(o).scrollIntoView()), !0
             }
             let r = t.blockRange(),
                 i = r && Ue(r);
             return i == null ? !1 : (e && e(n.tr.lift(r, i).scrollIntoView()), !0)
         };
 
-    function Vl(n) {
+    function ql(n) {
         return (e, t) => {
             let {
                 $from: r,
                 $to: i
             } = e.selection;
-            if (e.selection instanceof x && e.selection.node.isBlock) return !r.parentOffset || !ke(e.doc, r.pos) ? !1 : (t && t(e.tr.split(r.pos).scrollIntoView()), !0);
+            if (e.selection instanceof x && e.selection.node.isBlock) return !r.parentOffset || !xe(e.doc, r.pos) ? !1 : (t && t(e.tr.split(r.pos).scrollIntoView()), !0);
             if (!r.parent.isBlock) return !1;
             if (t) {
                 let o = i.parentOffset == i.parent.content.size,
                     s = e.tr;
                 (e.selection instanceof w || e.selection instanceof $) && s.deleteSelection();
-                let l = r.depth == 0 ? null : Kn(r.node(-1).contentMatchAt(r.indexAfter(-1))),
+                let l = r.depth == 0 ? null : Hn(r.node(-1).contentMatchAt(r.indexAfter(-1))),
                     a = n && n(i.parent, o),
                     c = a ? [a] : o && l ? [{
                         type: l
                     }] : void 0,
-                    f = ke(s.doc, s.mapping.map(r.pos), 1, c);
-                if (!c && !f && ke(s.doc, s.mapping.map(r.pos), 1, l ? [{
+                    f = xe(s.doc, s.mapping.map(r.pos), 1, c);
+                if (!c && !f && xe(s.doc, s.mapping.map(r.pos), 1, l ? [{
                         type: l
                     }] : void 0) && (l && (c = [{
                         type: l
                     }]), f = !0), f && (s.split(s.mapping.map(r.pos), 1, c), !o && !r.parentOffset && r.parent.type != l)) {
                     let h = s.mapping.map(r.before()),
                         d = s.doc.resolve(h);
                     l && r.node(-1).canReplaceWith(d.index(), d.index() + 1, l) && s.setNodeMarkup(s.mapping.map(r.before()), l)
                 }
                 t(s.scrollIntoView())
             }
             return !0
         }
     }
-    var Ll = Vl();
-    var Jl = (n, e) => (e && e(n.tr.setSelection(new $(n.doc))), !0);
+    var Kl = ql();
+    var $l = (n, e) => (e && e(n.tr.setSelection(new $(n.doc))), !0);
 
-    function Wl(n, e, t) {
+    function Hl(n, e, t) {
         let r = e.nodeBefore,
             i = e.nodeAfter,
             o = e.index();
         return !r || !i || !r.type.compatibleContent(i.type) ? !1 : !r.content.size && e.parent.canReplace(o - 1, o) ? (t && t(n.tr.delete(e.pos - r.nodeSize, e.pos).scrollIntoView()), !0) : !e.parent.canReplace(o, o + 1) || !(i.isTextblock || yt(n.doc, e.pos)) ? !1 : (t && t(n.tr.clearIncompatible(e.pos, r.type, r.contentMatchAt(r.childCount)).join(e.pos).scrollIntoView()), !0)
     }
 
-    function Ei(n, e, t) {
+    function Ii(n, e, t) {
         let r = e.nodeBefore,
             i = e.nodeAfter,
             o, s;
         if (r.type.spec.isolating || i.type.spec.isolating) return !1;
-        if (Wl(n, e, t)) return !0;
+        if (Hl(n, e, t)) return !0;
         let l = e.parent.canReplace(e.index(), e.index() + 1);
         if (l && (o = (s = r.contentMatchAt(r.childCount)).findWrapping(i.type)) && s.matchType(o[0] || i.type).validEnd) {
             if (t) {
                 let h = e.pos + i.nodeSize,
                     d = y.empty;
                 for (let m = o.length - 1; m >= 0; m--) d = y.from(o[m].create(null, d));
                 d = y.from(r.copy(d));
@@ -4128,40 +4136,40 @@
                 }
                 return !0
             }
         }
         return !1
     }
 
-    function Ii(n) {
+    function Ri(n) {
         return function(e, t) {
             let r = e.selection,
                 i = n < 0 ? r.$from : r.$to,
                 o = i.depth;
             for (; i.node(o).isInline;) {
                 if (!o) return !1;
                 o--
             }
             return i.node(o).isTextblock ? (t && t(e.tr.setSelection(w.create(e.doc, n < 0 ? i.start(o) : i.end(o)))), !0) : !1
         }
     }
-    var ql = Ii(-1),
-        Kl = Ii(1);
+    var jl = Ri(-1),
+        _l = Ri(1);
 
-    function Zt(n, e = null) {
+    function en(n, e = null) {
         return function(t, r) {
             let {
                 $from: i,
                 $to: o
-            } = t.selection, s = i.blockRange(o), l = s && _t(s, n, e);
+            } = t.selection, s = i.blockRange(o), l = s && Ut(s, n, e);
             return l ? (r && r(t.tr.wrap(s, l).scrollIntoView()), !0) : !1
         }
     }
 
-    function Hn(n, e = null) {
+    function _n(n, e = null) {
         return function(t, r) {
             let i = !1;
             for (let o = 0; o < t.selection.ranges.length && !i; o++) {
                 let {
                     $from: {
                         pos: s
                     },
@@ -4196,15 +4204,15 @@
                 }
                 r(o.scrollIntoView())
             }
             return !0
         }
     }
 
-    function $l(n, e, t) {
+    function Ul(n, e, t) {
         for (let r = 0; r < e.length; r++) {
             let {
                 $from: i,
                 $to: o
             } = e[r], s = i.depth == 0 ? n.inlineContent && n.type.allowsMarkType(t) : !1;
             if (n.nodesBetween(i.pos, o.pos, l => {
                     if (s) return !1;
@@ -4217,15 +4225,15 @@
     function G(n, e = null) {
         return function(t, r) {
             let {
                 empty: i,
                 $cursor: o,
                 ranges: s
             } = t.selection;
-            if (i && !o || !$l(t.doc, s, n)) return !1;
+            if (i && !o || !Ul(t.doc, s, n)) return !1;
             if (r)
                 if (o) n.isInSet(t.storedMarks || o.marks()) ? r(t.tr.removeStoredMark(n)) : r(t.tr.addStoredMark(n.create(e)));
                 else {
                     let l = !1,
                         a = t.tr;
                     for (let c = 0; !l && c < s.length; c++) {
                         let {
@@ -4258,48 +4266,48 @@
     function xt(...n) {
         return function(e, t, r) {
             for (let i = 0; i < n.length; i++)
                 if (n[i](e, t, r)) return !0;
             return !1
         }
     }
-    var qn = xt(Ni, Il, Al),
-        wi = xt(Ni, Pl, Rl),
+    var $n = xt(Di, Pl, zl),
+        Ti = xt(Di, Fl, Vl),
         de = {
-            Enter: xt(zl, Bl, Fl, Ll),
-            "Mod-Enter": $n,
-            Backspace: qn,
-            "Mod-Backspace": qn,
-            "Shift-Backspace": qn,
-            Delete: wi,
-            "Mod-Delete": wi,
-            "Mod-a": Jl
+            Enter: xt(Ll, Jl, Wl, Kl),
+            "Mod-Enter": jn,
+            Backspace: $n,
+            "Mod-Backspace": $n,
+            "Shift-Backspace": $n,
+            Delete: Ti,
+            "Mod-Delete": Ti,
+            "Mod-a": $l
         },
-        Ai = {
+        vi = {
             "Ctrl-h": de.Backspace,
             "Alt-Backspace": de["Mod-Backspace"],
             "Ctrl-d": de.Delete,
             "Ctrl-Alt-Backspace": de["Mod-Delete"],
             "Alt-Delete": de["Mod-Delete"],
             "Alt-d": de["Mod-Delete"],
-            "Ctrl-a": ql,
-            "Ctrl-e": Kl
+            "Ctrl-a": jl,
+            "Ctrl-e": _l
         };
-    for (let n in de) Ai[n] = de[n];
-    var Hl = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : typeof os != "undefined" && os.platform ? os.platform() == "darwin" : !1,
-        vi = Hl ? Ai : de;
+    for (let n in de) vi[n] = de[n];
+    var Gl = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : typeof os != "undefined" && os.platform ? os.platform() == "darwin" : !1,
+        Pi = Gl ? vi : de;
 
-    function Pi(n = {}) {
+    function zi(n = {}) {
         return new J({
             view(e) {
-                return new jn(e, n)
+                return new Un(e, n)
             }
         })
     }
-    var jn = class {
+    var Un = class {
         constructor(e, t) {
             var r;
             this.editorView = e, this.cursorPos = null, this.element = null, this.timeout = -1, this.width = (r = t.width) !== null && r !== void 0 ? r : 1, this.color = t.color === !1 ? void 0 : t.color || "black", this.class = t.class, this.handlers = ["dragover", "dragend", "drop", "dragleave"].map(i => {
                 let o = s => {
                     this[i](s)
                 };
                 return e.dom.addEventListener(i, o), {
@@ -4371,15 +4379,15 @@
                 }),
                 r = t && t.inside >= 0 && this.editorView.state.doc.nodeAt(t.inside),
                 i = r && r.type.spec.disableDropCursor,
                 o = typeof i == "function" ? i(this.editorView, t, e) : i;
             if (t && !o) {
                 let s = t.pos;
                 if (this.editorView.dragging && this.editorView.dragging.slice) {
-                    let l = Ut(this.editorView.state.doc, s, this.editorView.dragging.slice);
+                    let l = Gt(this.editorView.state.doc, s, this.editorView.dragging.slice);
                     l != null && (s = l)
                 }
                 this.setCursor(s), this.scheduleRemoval(5e3)
             }
         }
         dragend() {
             this.scheduleRemoval(20)
@@ -4441,15 +4449,15 @@
             191: "/",
             192: "`",
             219: "[",
             220: "\\",
             221: "]",
             222: "'"
         },
-        tn = {
+        nn = {
             48: ")",
             49: "!",
             50: "@",
             51: "#",
             52: "$",
             53: "%",
             54: "^",
@@ -4467,152 +4475,177 @@
             191: "?",
             192: "~",
             219: "{",
             220: "|",
             221: "}",
             222: '"'
         },
-        jl = typeof navigator != "undefined" && /Mac/.test(navigator.platform),
-        _l = typeof navigator != "undefined" && /MSIE \d|Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(navigator.userAgent);
+        Yl = typeof navigator != "undefined" && /Mac/.test(navigator.platform),
+        Xl = typeof navigator != "undefined" && /MSIE \d|Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(navigator.userAgent);
     for (T = 0; T < 10; T++) ue[48 + T] = ue[96 + T] = String(T);
     var T;
     for (T = 1; T <= 24; T++) ue[T + 111] = "F" + T;
     var T;
-    for (T = 65; T <= 90; T++) ue[T] = String.fromCharCode(T + 32), tn[T] = String.fromCharCode(T);
+    for (T = 65; T <= 90; T++) ue[T] = String.fromCharCode(T + 32), nn[T] = String.fromCharCode(T);
     var T;
-    for (en in ue) tn.hasOwnProperty(en) || (tn[en] = ue[en]);
-    var en;
+    for (tn in ue) nn.hasOwnProperty(tn) || (nn[tn] = ue[tn]);
+    var tn;
 
-    function Ri(n) {
-        var e = jl && n.metaKey && n.shiftKey && !n.ctrlKey && !n.altKey || _l && n.shiftKey && n.key && n.key.length == 1 || n.key == "Unidentified",
-            t = !e && n.key || (n.shiftKey ? tn : ue)[n.keyCode] || n.key || "Unidentified";
+    function Bi(n) {
+        var e = Yl && n.metaKey && n.shiftKey && !n.ctrlKey && !n.altKey || Xl && n.shiftKey && n.key && n.key.length == 1 || n.key == "Unidentified",
+            t = !e && n.key || (n.shiftKey ? nn : ue)[n.keyCode] || n.key || "Unidentified";
         return t == "Esc" && (t = "Escape"), t == "Del" && (t = "Delete"), t == "Left" && (t = "ArrowLeft"), t == "Up" && (t = "ArrowUp"), t == "Right" && (t = "ArrowRight"), t == "Down" && (t = "ArrowDown"), t
     }
-    var Ul = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : !1;
+    var Ql = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : !1;
 
-    function Gl(n) {
+    function Zl(n) {
         let e = n.split(/-(?!$)/),
             t = e[e.length - 1];
         t == "Space" && (t = " ");
         let r, i, o, s;
         for (let l = 0; l < e.length - 1; l++) {
             let a = e[l];
             if (/^(cmd|meta|m)$/i.test(a)) s = !0;
             else if (/^a(lt)?$/i.test(a)) r = !0;
             else if (/^(c|ctrl|control)$/i.test(a)) i = !0;
             else if (/^s(hift)?$/i.test(a)) o = !0;
-            else if (/^mod$/i.test(a)) Ul ? s = !0 : i = !0;
+            else if (/^mod$/i.test(a)) Ql ? s = !0 : i = !0;
             else throw new Error("Unrecognized modifier name: " + a)
         }
         return r && (t = "Alt-" + t), i && (t = "Ctrl-" + t), s && (t = "Meta-" + t), o && (t = "Shift-" + t), t
     }
 
-    function Yl(n) {
+    function ea(n) {
         let e = Object.create(null);
-        for (let t in n) e[Gl(t)] = n[t];
+        for (let t in n) e[Zl(t)] = n[t];
         return e
     }
 
-    function _n(n, e, t = !0) {
+    function Gn(n, e, t = !0) {
         return e.altKey && (n = "Alt-" + n), e.ctrlKey && (n = "Ctrl-" + n), e.metaKey && (n = "Meta-" + n), t && e.shiftKey && (n = "Shift-" + n), n
     }
 
-    function Un(n) {
+    function Yn(n) {
         return new J({
             props: {
-                handleKeyDown: Gn(n)
+                handleKeyDown: Xn(n)
             }
         })
     }
 
-    function Gn(n) {
-        let e = Yl(n);
+    function Xn(n) {
+        let e = ea(n);
         return function(t, r) {
-            let i = Ri(r),
-                o, s = e[_n(i, r)];
+            let i = Bi(r),
+                o, s = e[Gn(i, r)];
             if (s && s(t.state, t.dispatch, t)) return !0;
             if (i.length == 1 && i != " ") {
                 if (r.shiftKey) {
-                    let l = e[_n(i, r, !1)];
+                    let l = e[Gn(i, r, !1)];
                     if (l && l(t.state, t.dispatch, t)) return !0
                 }
                 if ((r.shiftKey || r.altKey || r.metaKey || i.charCodeAt(0) > 127) && (o = ue[r.keyCode]) && o != i) {
-                    let l = e[_n(o, r)];
+                    let l = e[Gn(o, r)];
                     if (l && l(t.state, t.dispatch, t)) return !0
                 }
             }
             return !1
         }
     }
-    var B = function(n) {
+    var I = function(n) {
             for (var e = 0;; e++)
                 if (n = n.previousSibling, !n) return e
         },
         Ot = function(n) {
             let e = n.assignedSlot || n.parentNode;
             return e && e.nodeType == 11 ? e.host : e
         },
-        zi = null,
+        nr = null,
         me = function(n, e, t) {
-            let r = zi || (zi = document.createRange());
+            let r = nr || (nr = document.createRange());
             return r.setEnd(n, t == null ? n.nodeValue.length : t), r.setStart(n, e || 0), r
         },
-        Ke = function(n, e, t, r) {
-            return t && (Bi(n, e, t, r, -1) || Bi(n, e, t, r, 1))
+        ta = function() {
+            nr = null
         },
-        Xl = /^(img|br|input|textarea|hr)$/i;
+        qe = function(n, e, t, r) {
+            return t && (Fi(n, e, t, r, -1) || Fi(n, e, t, r, 1))
+        },
+        na = /^(img|br|input|textarea|hr)$/i;
 
-    function Bi(n, e, t, r, i) {
+    function Fi(n, e, t, r, i) {
         for (;;) {
             if (n == t && e == r) return !0;
             if (e == (i < 0 ? 0 : oe(n))) {
                 let o = n.parentNode;
-                if (!o || o.nodeType != 1 || br(n) || Xl.test(n.nodeName) || n.contentEditable == "false") return !1;
-                e = B(n) + (i < 0 ? 0 : 1), n = o
+                if (!o || o.nodeType != 1 || Dt(n) || na.test(n.nodeName) || n.contentEditable == "false") return !1;
+                e = I(n) + (i < 0 ? 0 : 1), n = o
             } else if (n.nodeType == 1) {
                 if (n = n.childNodes[e + (i < 0 ? -1 : 0)], n.contentEditable == "false") return !1;
                 e = i < 0 ? oe(n) : 0
             } else return !1
         }
     }
 
     function oe(n) {
         return n.nodeType == 3 ? n.nodeValue.length : n.childNodes.length
     }
 
-    function Ql(n, e, t) {
+    function ra(n, e) {
+        for (;;) {
+            if (n.nodeType == 3 && e) return n;
+            if (n.nodeType == 1 && e > 0) {
+                if (n.contentEditable == "false") return null;
+                n = n.childNodes[e - 1], e = oe(n)
+            } else if (n.parentNode && !Dt(n)) e = I(n), n = n.parentNode;
+            else return null
+        }
+    }
+
+    function ia(n, e) {
+        for (;;) {
+            if (n.nodeType == 3 && e < n.nodeValue.length) return n;
+            if (n.nodeType == 1 && e < n.childNodes.length) {
+                if (n.contentEditable == "false") return null;
+                n = n.childNodes[e], e = 0
+            } else if (n.parentNode && !Dt(n)) e = I(n) + 1, n = n.parentNode;
+            else return null
+        }
+    }
+
+    function oa(n, e, t) {
         for (let r = e == 0, i = e == oe(n); r || i;) {
             if (n == t) return !0;
-            let o = B(n);
+            let o = I(n);
             if (n = n.parentNode, !n) return !1;
             r = r && o == 0, i = i && o == oe(n)
         }
     }
 
-    function br(n) {
+    function Dt(n) {
         let e;
         for (let t = n; t && !(e = t.pmViewDesc); t = t.parentNode);
         return e && e.node && e.node.isBlock && (e.dom == n || e.contentDOM == n)
     }
-    var un = function(n) {
-        return n.focusNode && Ke(n.focusNode, n.focusOffset, n.anchorNode, n.anchorOffset)
+    var pn = function(n) {
+        return n.focusNode && qe(n.focusNode, n.focusOffset, n.anchorNode, n.anchorOffset)
     };
 
-    function Fe(n, e) {
+    function Be(n, e) {
         let t = document.createEvent("Event");
         return t.initEvent("keydown", !0, !0), t.keyCode = n, t.key = t.code = e, t
     }
 
-    function Zl(n) {
+    function sa(n) {
         let e = n.activeElement;
         for (; e && e.shadowRoot;) e = e.shadowRoot.activeElement;
         return e
     }
 
-    function ea(n, e, t) {
+    function la(n, e, t) {
         if (n.caretPositionFromPoint) try {
             let r = n.caretPositionFromPoint(e, t);
             if (r) return {
                 node: r.offsetNode,
                 offset: r.offset
             }
         } catch (r) {}
@@ -4621,68 +4654,74 @@
             if (r) return {
                 node: r.startContainer,
                 offset: r.startOffset
             }
         }
     }
     var se = typeof navigator != "undefined" ? navigator : null,
-        Fi = typeof document != "undefined" ? document : null,
-        we = se && se.userAgent || "",
-        er = /Edge\/(\d+)/.exec(we),
-        mo = /MSIE \d/.exec(we),
-        tr = /Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(we),
-        W = !!(mo || tr || er),
-        Ce = mo ? document.documentMode : tr ? +tr[1] : er ? +er[1] : 0,
-        X = !W && /gecko\/(\d+)/i.test(we);
-    X && +(/Firefox\/(\d+)/.exec(we) || [0, 0])[1];
-    var nr = !W && /Chrome\/(\d+)/.exec(we),
-        R = !!nr,
-        ta = nr ? +nr[1] : 0,
+        Vi = typeof document != "undefined" ? document : null,
+        Ne = se && se.userAgent || "",
+        rr = /Edge\/(\d+)/.exec(Ne),
+        yo = /MSIE \d/.exec(Ne),
+        ir = /Trident\/(?:[7-9]|\d{2,})\..*rv:(\d+)/.exec(Ne),
+        W = !!(yo || ir || rr),
+        Oe = yo ? document.documentMode : ir ? +ir[1] : rr ? +rr[1] : 0,
+        X = !W && /gecko\/(\d+)/i.test(Ne);
+    X && +(/Firefox\/(\d+)/.exec(Ne) || [0, 0])[1];
+    var or = !W && /Chrome\/(\d+)/.exec(Ne),
+        B = !!or,
+        aa = or ? +or[1] : 0,
         F = !W && !!se && /Apple Computer/.test(se.vendor),
-        tt = F && (/Mobile\/\w+/.test(we) || !!se && se.maxTouchPoints > 2),
+        tt = F && (/Mobile\/\w+/.test(Ne) || !!se && se.maxTouchPoints > 2),
         j = tt || (se ? /Mac/.test(se.platform) : !1),
-        na = se ? /Win/.test(se.platform) : !1,
-        Y = /Android \d/.test(we),
-        Dt = !!Fi && "webkitFontSmoothing" in Fi.documentElement.style,
-        ra = Dt ? +(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent) || [0, 0])[1] : 0;
-
-    function ia(n) {
-        return {
+        ca = se ? /Win/.test(se.platform) : !1,
+        Y = /Android \d/.test(Ne),
+        Et = !!Vi && "webkitFontSmoothing" in Vi.documentElement.style,
+        fa = Et ? +(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent) || [0, 0])[1] : 0;
+
+    function ha(n) {
+        let e = n.defaultView && n.defaultView.visualViewport;
+        return e ? {
+            left: 0,
+            right: e.width,
+            top: 0,
+            bottom: e.height
+        } : {
             left: 0,
             right: n.documentElement.clientWidth,
             top: 0,
             bottom: n.documentElement.clientHeight
         }
     }
 
     function pe(n, e) {
         return typeof n == "number" ? n : n[e]
     }
 
-    function oa(n) {
+    function da(n) {
         let e = n.getBoundingClientRect(),
             t = e.width / n.offsetWidth || 1,
             r = e.height / n.offsetHeight || 1;
         return {
             left: e.left,
             right: e.left + n.clientWidth * t,
             top: e.top,
             bottom: e.top + n.clientHeight * r
         }
     }
 
-    function Vi(n, e, t) {
+    function Li(n, e, t) {
         let r = n.someProp("scrollThreshold") || 0,
             i = n.someProp("scrollMargin") || 5,
             o = n.dom.ownerDocument;
         for (let s = t || n.dom; s; s = Ot(s)) {
             if (s.nodeType != 1) continue;
             let l = s,
                 a = l == o.body,
-                c = a ? ia(o) : oa(l),
+                c = a ? ha(o) : da(l),
                 f = 0,
                 h = 0;
             if (e.top < c.top + pe(r, "top") ? h = -(c.top - e.top + pe(i, "top")) : e.bottom > c.bottom - pe(r, "bottom") && (h = e.bottom - e.top > c.bottom - c.top ? e.top + pe(i, "top") - c.top : e.bottom - c.bottom + pe(i, "bottom")), e.left < c.left + pe(r, "left") ? f = -(c.left - e.left + pe(i, "left")) : e.right > c.right - pe(r, "right") && (f = e.right - c.right + pe(i, "right")), f || h)
                 if (a) o.defaultView.scrollBy(f, h);
                 else {
                     let d = l.scrollLeft,
                         p = l.scrollTop;
@@ -4695,15 +4734,15 @@
                         right: e.right - u,
                         bottom: e.bottom - m
                     }
                 } if (a || /^(fixed|sticky)$/.test(getComputedStyle(s).position)) break
         }
     }
 
-    function sa(n) {
+    function ua(n) {
         let e = n.dom.getBoundingClientRect(),
             t = Math.max(0, e.top),
             r, i;
         for (let o = (e.left + e.right) / 2, s = t + 1; s < Math.min(innerHeight, e.bottom); s += 5) {
             let l = n.root.elementFromPoint(o, s);
             if (!l || l == n.dom || !n.dom.contains(l)) continue;
             let a = l.getBoundingClientRect();
@@ -4711,64 +4750,64 @@
                 r = l, i = a.top;
                 break
             }
         }
         return {
             refDOM: r,
             refTop: i,
-            stack: go(n.dom)
+            stack: bo(n.dom)
         }
     }
 
-    function go(n) {
+    function bo(n) {
         let e = [],
             t = n.ownerDocument;
         for (let r = n; r && (e.push({
                 dom: r,
                 top: r.scrollTop,
                 left: r.scrollLeft
             }), n != t); r = Ot(r));
         return e
     }
 
-    function la({
+    function pa({
         refDOM: n,
         refTop: e,
         stack: t
     }) {
         let r = n ? n.getBoundingClientRect().top : 0;
-        yo(t, r == 0 ? 0 : r - e)
+        ko(t, r == 0 ? 0 : r - e)
     }
 
-    function yo(n, e) {
+    function ko(n, e) {
         for (let t = 0; t < n.length; t++) {
             let {
                 dom: r,
                 top: i,
                 left: o
             } = n[t];
             r.scrollTop != i + e && (r.scrollTop = i + e), r.scrollLeft != o && (r.scrollLeft = o)
         }
     }
     var Xe = null;
 
-    function aa(n) {
+    function ma(n) {
         if (n.setActive) return n.setActive();
         if (Xe) return n.focus(Xe);
-        let e = go(n);
+        let e = bo(n);
         n.focus(Xe == null ? {
             get preventScroll() {
                 return Xe = {
                     preventScroll: !0
                 }, !0
             }
-        } : void 0), Xe || (Xe = !1, yo(e, 0))
+        } : void 0), Xe || (Xe = !1, ko(e, 0))
     }
 
-    function bo(n, e) {
+    function xo(n, e) {
         let t, r = 2e8,
             i, o = 0,
             s = e.top,
             l = e.top,
             a, c;
         for (let f = n.firstChild, h = 0; f; f = f.nextSibling, h++) {
             let d;
@@ -4790,258 +4829,258 @@
                 } else u.top > e.top && !a && u.left <= e.left && u.right >= e.left && (a = f, c = {
                     left: Math.max(u.left, Math.min(u.right, e.left)),
                     top: u.top
                 });
                 !t && (e.left >= u.right && e.top >= u.top || e.left >= u.left && e.top >= u.bottom) && (o = h + 1)
             }
         }
-        return !t && a && (t = a, i = c, r = 0), t && t.nodeType == 3 ? ca(t, i) : !t || r && t.nodeType == 1 ? {
+        return !t && a && (t = a, i = c, r = 0), t && t.nodeType == 3 ? ga(t, i) : !t || r && t.nodeType == 1 ? {
             node: n,
             offset: o
-        } : bo(t, i)
+        } : xo(t, i)
     }
 
-    function ca(n, e) {
+    function ga(n, e) {
         let t = n.nodeValue.length,
             r = document.createRange();
         for (let i = 0; i < t; i++) {
             r.setEnd(n, i + 1), r.setStart(n, i);
-            let o = xe(r, 1);
-            if (o.top != o.bottom && kr(e, o)) return {
+            let o = Se(r, 1);
+            if (o.top != o.bottom && Sr(e, o)) return {
                 node: n,
                 offset: i + (e.left >= (o.left + o.right) / 2 ? 1 : 0)
             }
         }
         return {
             node: n,
             offset: 0
         }
     }
 
-    function kr(n, e) {
+    function Sr(n, e) {
         return n.left >= e.left - 1 && n.left <= e.right + 1 && n.top >= e.top - 1 && n.top <= e.bottom + 1
     }
 
-    function fa(n, e) {
+    function ya(n, e) {
         let t = n.parentNode;
         return t && /^li$/i.test(t.nodeName) && e.left < n.getBoundingClientRect().left ? t : n
     }
 
-    function ha(n, e, t) {
+    function ba(n, e, t) {
         let {
             node: r,
             offset: i
-        } = bo(e, t), o = -1;
+        } = xo(e, t), o = -1;
         if (r.nodeType == 1 && !r.firstChild) {
             let s = r.getBoundingClientRect();
             o = s.left != s.right && t.left > (s.left + s.right) / 2 ? 1 : -1
         }
         return n.docView.posFromDOM(r, i, o)
     }
 
-    function da(n, e, t, r) {
+    function ka(n, e, t, r) {
         let i = -1;
         for (let o = e, s = !1; o != n.dom;) {
             let l = n.docView.nearestDesc(o, !0);
             if (!l) return null;
             if (l.dom.nodeType == 1 && (l.node.isBlock && l.parent && !s || !l.contentDOM)) {
                 let a = l.dom.getBoundingClientRect();
                 if (l.node.isBlock && l.parent && !s && (s = !0, a.left > r.left || a.top > r.top ? i = l.posBefore : (a.right < r.left || a.bottom < r.top) && (i = l.posAfter)), !l.contentDOM && i < 0 && !l.node.isText) return (l.node.isBlock ? r.top < (a.top + a.bottom) / 2 : r.left < (a.left + a.right) / 2) ? l.posBefore : l.posAfter
             }
             o = l.dom.parentNode
         }
         return i > -1 ? i : n.docView.posFromDOM(e, t, -1)
     }
 
-    function ko(n, e, t) {
+    function So(n, e, t) {
         let r = n.childNodes.length;
         if (r && t.top < t.bottom)
             for (let i = Math.max(0, Math.min(r - 1, Math.floor(r * (e.top - t.top) / (t.bottom - t.top)) - 2)), o = i;;) {
                 let s = n.childNodes[o];
                 if (s.nodeType == 1) {
                     let l = s.getClientRects();
                     for (let a = 0; a < l.length; a++) {
                         let c = l[a];
-                        if (kr(e, c)) return ko(s, e, c)
+                        if (Sr(e, c)) return So(s, e, c)
                     }
                 }
                 if ((o = (o + 1) % r) == i) break
             }
         return n
     }
 
-    function ua(n, e) {
+    function xa(n, e) {
         let t = n.dom.ownerDocument,
             r, i = 0,
-            o = ea(t, e.left, e.top);
+            o = la(t, e.left, e.top);
         o && ({
             node: r,
             offset: i
         } = o);
         let s = (n.root.elementFromPoint ? n.root : t).elementFromPoint(e.left, e.top),
             l;
         if (!s || !n.dom.contains(s.nodeType != 1 ? s.parentNode : s)) {
             let c = n.dom.getBoundingClientRect();
-            if (!kr(e, c) || (s = ko(n.dom, e, c), !s)) return null
+            if (!Sr(e, c) || (s = So(n.dom, e, c), !s)) return null
         }
         if (F)
             for (let c = s; r && c; c = Ot(c)) c.draggable && (r = void 0);
-        if (s = fa(s, e), r) {
+        if (s = ya(s, e), r) {
             if (X && r.nodeType == 1 && (i = Math.min(i, r.childNodes.length), i < r.childNodes.length)) {
                 let f = r.childNodes[i],
                     h;
                 f.nodeName == "IMG" && (h = f.getBoundingClientRect()).right <= e.left && h.bottom > e.top && i++
             }
             let c;
-            Dt && i && r.nodeType == 1 && (c = r.childNodes[i - 1]).nodeType == 1 && c.contentEditable == "false" && c.getBoundingClientRect().top >= e.top && i--, r == n.dom && i == r.childNodes.length - 1 && r.lastChild.nodeType == 1 && e.top > r.lastChild.getBoundingClientRect().bottom ? l = n.state.doc.content.size : (i == 0 || r.nodeType != 1 || r.childNodes[i - 1].nodeName != "BR") && (l = da(n, r, i, e))
+            Et && i && r.nodeType == 1 && (c = r.childNodes[i - 1]).nodeType == 1 && c.contentEditable == "false" && c.getBoundingClientRect().top >= e.top && i--, r == n.dom && i == r.childNodes.length - 1 && r.lastChild.nodeType == 1 && e.top > r.lastChild.getBoundingClientRect().bottom ? l = n.state.doc.content.size : (i == 0 || r.nodeType != 1 || r.childNodes[i - 1].nodeName != "BR") && (l = ka(n, r, i, e))
         }
-        l == null && (l = ha(n, s, e));
+        l == null && (l = ba(n, s, e));
         let a = n.docView.nearestDesc(s, !0);
         return {
             pos: l,
             inside: a ? a.posAtStart - a.border : -1
         }
     }
 
-    function Li(n) {
+    function Ji(n) {
         return n.top < n.bottom || n.left < n.right
     }
 
-    function xe(n, e) {
+    function Se(n, e) {
         let t = n.getClientRects();
         if (t.length) {
             let r = t[e < 0 ? 0 : t.length - 1];
-            if (Li(r)) return r
+            if (Ji(r)) return r
         }
-        return Array.prototype.find.call(t, Li) || n.getBoundingClientRect()
+        return Array.prototype.find.call(t, Ji) || n.getBoundingClientRect()
     }
-    var pa = /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/;
+    var Sa = /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/;
 
-    function xo(n, e, t) {
+    function Mo(n, e, t) {
         let {
             node: r,
             offset: i,
             atom: o
-        } = n.docView.domFromPos(e, t < 0 ? -1 : 1), s = Dt || X;
+        } = n.docView.domFromPos(e, t < 0 ? -1 : 1), s = Et || X;
         if (r.nodeType == 3)
-            if (s && (pa.test(r.nodeValue) || (t < 0 ? !i : i == r.nodeValue.length))) {
-                let a = xe(me(r, i, i), t);
+            if (s && (Sa.test(r.nodeValue) || (t < 0 ? !i : i == r.nodeValue.length))) {
+                let a = Se(me(r, i, i), t);
                 if (X && i && /\s/.test(r.nodeValue[i - 1]) && i < r.nodeValue.length) {
-                    let c = xe(me(r, i - 1, i - 1), -1);
+                    let c = Se(me(r, i - 1, i - 1), -1);
                     if (c.top == a.top) {
-                        let f = xe(me(r, i, i + 1), -1);
+                        let f = Se(me(r, i, i + 1), -1);
                         if (f.top != a.top) return St(f, f.left < c.left)
                     }
                 }
                 return a
             } else {
                 let a = i,
                     c = i,
                     f = t < 0 ? 1 : -1;
-                return t < 0 && !i ? (c++, f = -1) : t >= 0 && i == r.nodeValue.length ? (a--, f = 1) : t < 0 ? a-- : c++, St(xe(me(r, a, c), f), f < 0)
+                return t < 0 && !i ? (c++, f = -1) : t >= 0 && i == r.nodeValue.length ? (a--, f = 1) : t < 0 ? a-- : c++, St(Se(me(r, a, c), f), f < 0)
             } if (!n.state.doc.resolve(e - (o || 0)).parent.inlineContent) {
             if (o == null && i && (t < 0 || i == oe(r))) {
                 let a = r.childNodes[i - 1];
-                if (a.nodeType == 1) return Yn(a.getBoundingClientRect(), !1)
+                if (a.nodeType == 1) return Qn(a.getBoundingClientRect(), !1)
             }
             if (o == null && i < oe(r)) {
                 let a = r.childNodes[i];
-                if (a.nodeType == 1) return Yn(a.getBoundingClientRect(), !0)
+                if (a.nodeType == 1) return Qn(a.getBoundingClientRect(), !0)
             }
-            return Yn(r.getBoundingClientRect(), t >= 0)
+            return Qn(r.getBoundingClientRect(), t >= 0)
         }
         if (o == null && i && (t < 0 || i == oe(r))) {
             let a = r.childNodes[i - 1],
                 c = a.nodeType == 3 ? me(a, oe(a) - (s ? 0 : 1)) : a.nodeType == 1 && (a.nodeName != "BR" || !a.nextSibling) ? a : null;
-            if (c) return St(xe(c, 1), !1)
+            if (c) return St(Se(c, 1), !1)
         }
         if (o == null && i < oe(r)) {
             let a = r.childNodes[i];
             for (; a.pmViewDesc && a.pmViewDesc.ignoreForCoords;) a = a.nextSibling;
             let c = a ? a.nodeType == 3 ? me(a, 0, s ? 0 : 1) : a.nodeType == 1 ? a : null : null;
-            if (c) return St(xe(c, -1), !0)
+            if (c) return St(Se(c, -1), !0)
         }
-        return St(xe(r.nodeType == 3 ? me(r) : r, -t), t >= 0)
+        return St(Se(r.nodeType == 3 ? me(r) : r, -t), t >= 0)
     }
 
     function St(n, e) {
         if (n.width == 0) return n;
         let t = e ? n.left : n.right;
         return {
             top: n.top,
             bottom: n.bottom,
             left: t,
             right: t
         }
     }
 
-    function Yn(n, e) {
+    function Qn(n, e) {
         if (n.height == 0) return n;
         let t = e ? n.top : n.bottom;
         return {
             top: t,
             bottom: t,
             left: n.left,
             right: n.right
         }
     }
 
-    function So(n, e, t) {
+    function Co(n, e, t) {
         let r = n.state,
             i = n.root.activeElement;
         r != e && n.updateState(e), i != n.dom && n.focus();
         try {
             return t()
         } finally {
             r != e && n.updateState(r), i != n.dom && i && i.focus()
         }
     }
 
-    function ma(n, e, t) {
+    function Ma(n, e, t) {
         let r = e.selection,
             i = t == "up" ? r.$from : r.$to;
-        return So(n, e, () => {
+        return Co(n, e, () => {
             let {
                 node: o
             } = n.docView.domFromPos(i.pos, t == "up" ? -1 : 1);
             for (;;) {
                 let l = n.docView.nearestDesc(o, !0);
                 if (!l) break;
                 if (l.node.isBlock) {
                     o = l.contentDOM || l.dom;
                     break
                 }
                 o = l.dom.parentNode
             }
-            let s = xo(n, i.pos, 1);
+            let s = Mo(n, i.pos, 1);
             for (let l = o.firstChild; l; l = l.nextSibling) {
                 let a;
                 if (l.nodeType == 1) a = l.getClientRects();
                 else if (l.nodeType == 3) a = me(l, 0, l.nodeValue.length).getClientRects();
                 else continue;
                 for (let c = 0; c < a.length; c++) {
                     let f = a[c];
                     if (f.bottom > f.top + 1 && (t == "up" ? s.top - f.top > (f.bottom - s.top) * 2 : f.bottom - s.bottom > (s.bottom - f.top) * 2)) return !1
                 }
             }
             return !0
         })
     }
-    var ga = /[\u0590-\u08ac]/;
+    var Ca = /[\u0590-\u08ac]/;
 
-    function ya(n, e, t) {
+    function Oa(n, e, t) {
         let {
             $head: r
         } = e.selection;
         if (!r.parent.isTextblock) return !1;
         let i = r.parentOffset,
             o = !i,
             s = i == r.parent.content.size,
             l = n.domSelection();
-        return !ga.test(r.parent.textContent) || !l.modify ? t == "left" || t == "backward" ? o : s : So(n, e, () => {
+        return !Ca.test(r.parent.textContent) || !l.modify ? t == "left" || t == "backward" ? o : s : Co(n, e, () => {
             let {
                 focusNode: a,
                 focusOffset: c,
                 anchorNode: f,
                 anchorOffset: h
             } = n.domSelectionRange(), d = l.caretBidiLevel;
             l.modify("move", t, "character");
@@ -5053,26 +5092,26 @@
                 g = u && !p.contains(u.nodeType == 1 ? u : u.parentNode) || a == u && c == m;
             try {
                 l.collapse(f, h), a && (a != f || c != h) && l.extend && l.extend(a, c)
             } catch (k) {}
             return d != null && (l.caretBidiLevel = d), g
         })
     }
-    var Ji = null,
-        Wi = null,
-        qi = !1;
+    var Wi = null,
+        qi = null,
+        Ki = !1;
 
-    function ba(n, e, t) {
-        return Ji == e && Wi == t ? qi : (Ji = e, Wi = t, qi = t == "up" || t == "down" ? ma(n, e, t) : ya(n, e, t))
+    function wa(n, e, t) {
+        return Wi == e && qi == t ? Ki : (Wi = e, qi = t, Ki = t == "up" || t == "down" ? Ma(n, e, t) : Oa(n, e, t))
     }
     var _ = 0,
-        Ki = 1,
-        Ve = 2,
+        $i = 1,
+        Fe = 2,
         le = 3,
-        $e = class {
+        Ke = class {
             constructor(e, t, r, i) {
                 this.parent = e, this.children = t, this.dom = r, this.contentDOM = i, this.dirty = _, r.pmViewDesc = this
             }
             matchesWidget(e) {
                 return !1
             }
             matchesMark(e) {
@@ -5138,15 +5177,15 @@
                         else {
                             for (; e.parentNode != this.contentDOM;) e = e.parentNode;
                             o = e.nextSibling
                         }
                         for (; o && !((s = o.pmViewDesc) && s.parent == this);) o = o.nextSibling;
                         return o ? this.posBeforeChild(s) : this.posAtEnd
                     } let i;
-                if (e == this.dom && this.contentDOM) i = t > B(this.contentDOM);
+                if (e == this.dom && this.contentDOM) i = t > I(this.contentDOM);
                 else if (this.contentDOM && this.contentDOM != this.dom && this.dom.contains(this.contentDOM)) i = e.compareDocumentPosition(this.contentDOM) & 2;
                 else if (this.dom.firstChild) {
                     if (t == 0)
                         for (let o = e;; o = o.parentNode) {
                             if (o == this.dom) {
                                 i = !1;
                                 break
@@ -5204,35 +5243,35 @@
                     atom: e + 1
                 };
                 let r = 0,
                     i = 0;
                 for (let o = 0; r < this.children.length; r++) {
                     let s = this.children[r],
                         l = o + s.size;
-                    if (l > e || s instanceof on) {
+                    if (l > e || s instanceof sn) {
                         i = e - o;
                         break
                     }
                     o = l
                 }
                 if (i) return this.children[r].domFromPos(i - this.children[r].border, t);
-                for (let o; r && !(o = this.children[r - 1]).size && o instanceof nn && o.side >= 0; r--);
+                for (let o; r && !(o = this.children[r - 1]).size && o instanceof rn && o.side >= 0; r--);
                 if (t <= 0) {
                     let o, s = !0;
                     for (; o = r ? this.children[r - 1] : null, !(!o || o.dom.parentNode == this.contentDOM); r--, s = !1);
                     return o && t && s && !o.border && !o.domAtom ? o.domFromPos(o.size, t) : {
                         node: this.contentDOM,
-                        offset: o ? B(o.dom) + 1 : 0
+                        offset: o ? I(o.dom) + 1 : 0
                     }
                 } else {
                     let o, s = !0;
                     for (; o = r < this.children.length ? this.children[r] : null, !(!o || o.dom.parentNode == this.contentDOM); r++, s = !1);
                     return o && s && !o.border && !o.domAtom ? o.domFromPos(0, t) : {
                         node: this.contentDOM,
-                        offset: o ? B(o.dom) : this.contentDOM.childNodes.length
+                        offset: o ? I(o.dom) : this.contentDOM.childNodes.length
                     }
                 }
             }
             parseRange(e, t, r = 0) {
                 if (this.children.length == 0) return {
                     node: this.contentDOM,
                     from: e,
@@ -5248,27 +5287,27 @@
                     if (i == -1 && e <= c) {
                         let f = s + a.border;
                         if (e >= f && t <= c - a.border && a.node && a.contentDOM && this.contentDOM.contains(a.contentDOM)) return a.parseRange(e, t, f);
                         e = s;
                         for (let h = l; h > 0; h--) {
                             let d = this.children[h - 1];
                             if (d.size && d.dom.parentNode == this.contentDOM && !d.emptyChildAt(1)) {
-                                i = B(d.dom) + 1;
+                                i = I(d.dom) + 1;
                                 break
                             }
                             e -= d.size
                         }
                         i == -1 && (i = 0)
                     }
                     if (i > -1 && (c > t || l == this.children.length - 1)) {
                         t = c;
                         for (let f = l + 1; f < this.children.length; f++) {
                             let h = this.children[f];
                             if (h.size && h.dom.parentNode == this.contentDOM && !h.emptyChildAt(-1)) {
-                                o = B(h.dom);
+                                o = I(h.dom);
                                 break
                             }
                             t += h.size
                         }
                         o == -1 && (o = this.contentDOM.childNodes.length);
                         break
                     }
@@ -5316,15 +5355,15 @@
                     if (d.nodeType == 3) {
                         if (f = !!(p && d.nodeValue[p - 1] == `
 `), f && p == d.nodeValue.length)
                             for (let u = d, m; u; u = u.parentNode) {
                                 if (m = u.nextSibling) {
                                     m.nodeName == "BR" && (l = a = {
                                         node: m.parentNode,
-                                        offset: B(m) + 1
+                                        offset: I(m) + 1
                                     });
                                     break
                                 }
                                 let g = u.pmViewDesc;
                                 if (g && g.node && g.node.isBlock) break
                             }
                     } else {
@@ -5332,15 +5371,15 @@
                         f = u && (u.nodeName == "BR" || u.contentEditable == "false")
                     }
                 }
                 if (X && c.focusNode && c.focusNode != a.node && c.focusNode.nodeType == 1) {
                     let d = c.focusNode.childNodes[c.focusOffset];
                     d && d.contentEditable == "false" && (i = !0)
                 }
-                if (!(i || f && F) && Ke(l.node, l.offset, c.anchorNode, c.anchorOffset) && Ke(a.node, a.offset, c.focusNode, c.focusOffset)) return;
+                if (!(i || f && F) && qe(l.node, l.offset, c.anchorNode, c.anchorOffset) && qe(a.node, a.offset, c.focusNode, c.focusOffset)) return;
                 let h = !1;
                 if ((c.extend || e == t) && !f) {
                     c.collapse(l.node, l.offset);
                     try {
                         e != t && c.extend(a.node, a.offset), h = !0
                     } catch (d) {}
                 }
@@ -5363,37 +5402,40 @@
                 for (let r = 0, i = 0; i < this.children.length; i++) {
                     let o = this.children[i],
                         s = r + o.size;
                     if (r == s ? e <= s && t >= r : e < s && t > r) {
                         let l = r + o.border,
                             a = s - o.border;
                         if (e >= l && t <= a) {
-                            this.dirty = e == r || t == s ? Ve : Ki, e == l && t == a && (o.contentLost || o.dom.parentNode != this.contentDOM) ? o.dirty = le : o.markDirty(e - l, t - l);
+                            this.dirty = e == r || t == s ? Fe : $i, e == l && t == a && (o.contentLost || o.dom.parentNode != this.contentDOM) ? o.dirty = le : o.markDirty(e - l, t - l);
                             return
-                        } else o.dirty = o.dom == o.contentDOM && o.dom.parentNode == this.contentDOM && !o.children.length ? Ve : le
+                        } else o.dirty = o.dom == o.contentDOM && o.dom.parentNode == this.contentDOM && !o.children.length ? Fe : le
                     }
                     r = s
                 }
-                this.dirty = Ve
+                this.dirty = Fe
             }
             markParentsDirty() {
                 let e = 1;
                 for (let t = this.parent; t; t = t.parent, e++) {
-                    let r = e == 1 ? Ve : Ki;
+                    let r = e == 1 ? Fe : $i;
                     t.dirty < r && (t.dirty = r)
                 }
             }
             get domAtom() {
                 return !1
             }
             get ignoreForCoords() {
                 return !1
             }
+            isText(e) {
+                return !1
+            }
         },
-        nn = class extends $e {
+        rn = class extends Ke {
             constructor(e, t, r, i) {
                 let o, s = t.type.toDOM;
                 if (typeof s == "function" && (s = s(r, () => {
                         if (!o) return i;
                         if (o.parent) return o.parent.posBeforeChild(o)
                     })), !t.type.spec.raw) {
                     if (s.nodeType != 1) {
@@ -5425,15 +5467,15 @@
             get domAtom() {
                 return !0
             }
             get side() {
                 return this.widget.type.side
             }
         },
-        rr = class extends $e {
+        sr = class extends Ke {
             constructor(e, t, r, i) {
                 super(e, [], t, null), this.textDOM = r, this.text = i
             }
             get size() {
                 return this.text.length
             }
             localPosFromDOM(e, t) {
@@ -5445,15 +5487,15 @@
                     offset: e
                 }
             }
             ignoreMutation(e) {
                 return e.type === "characterData" && e.target.nodeValue == e.oldValue
             }
         },
-        nt = class n extends $e {
+        nt = class n extends Ke {
             constructor(e, t, r, i) {
                 super(e, [], r, i), this.mark = t
             }
             static create(e, t, r, i) {
                 let o = i.nodeViews[t.type.name],
                     s = o && o(t, i, r);
                 return (!s || !s.dom) && (s = ie.renderSpec(document, t.type.spec.toDOM(t, r))), new n(e, t, s.dom, s.contentDOM || s.dom)
@@ -5475,20 +5517,20 @@
                     r.dirty < this.dirty && (r.dirty = this.dirty), this.dirty = _
                 }
             }
             slice(e, t, r) {
                 let i = n.create(this.parent, this.mark, !0, r),
                     o = this.children,
                     s = this.size;
-                t < s && (o = ar(o, t, s, r)), e > 0 && (o = ar(o, 0, e, r));
+                t < s && (o = hr(o, t, s, r)), e > 0 && (o = hr(o, 0, e, r));
                 for (let l = 0; l < o.length; l++) o[l].parent = i;
                 return i.children = o, i
             }
         },
-        Oe = class n extends $e {
+        we = class n extends Ke {
             constructor(e, t, r, i, o, s, l, a, c) {
                 super(e, [], o, s), this.node = t, this.outerDeco = r, this.innerDeco = i, this.nodeDOM = l
             }
             static create(e, t, r, i, o, s) {
                 let l = o.nodeViews[t.type.name],
                     a, c = l && l(t, o, () => {
                         if (!a) return s;
@@ -5501,15 +5543,15 @@
                     else if (f.nodeType != 3) throw new RangeError("Text must be rendered as a DOM text node")
                 } else f || ({
                     dom: f,
                     contentDOM: h
                 } = ie.renderSpec(document, t.type.spec.toDOM(t)));
                 !h && !t.isText && f.nodeName != "BR" && (f.hasAttribute("contenteditable") || (f.contentEditable = "false"), t.type.spec.draggable && (f.draggable = !0));
                 let d = f;
-                return f = Oo(f, r, t), c ? a = new ir(e, t, r, i, f, h || null, d, c, o, s + 1) : t.isText ? new rn(e, t, r, i, f, d, o) : new n(e, t, r, i, f, h || null, d, o, s + 1)
+                return f = No(f, r, t), c ? a = new lr(e, t, r, i, f, h || null, d, c, o, s + 1) : t.isText ? new on(e, t, r, i, f, d, o) : new n(e, t, r, i, f, h || null, d, o, s + 1)
             }
             parseRule() {
                 if (this.node.type.spec.reparseInView) return null;
                 let e = {
                     node: this.node.type.name,
                     attrs: this.node.attrs
                 };
@@ -5524,56 +5566,55 @@
                         }
                     }
                     e.contentElement || (e.getContent = () => y.empty)
                 }
                 return e
             }
             matchesNode(e, t, r) {
-                return this.dirty == _ && e.eq(this.node) && sr(t, this.outerDeco) && r.eq(this.innerDeco)
+                return this.dirty == _ && e.eq(this.node) && cr(t, this.outerDeco) && r.eq(this.innerDeco)
             }
             get size() {
                 return this.node.nodeSize
             }
             get border() {
                 return this.node.isLeaf ? 0 : 1
             }
             updateChildren(e, t) {
                 let r = this.node.inlineContent,
                     i = t,
                     o = e.composing ? this.localCompositionInfo(e, t) : null,
                     s = o && o.pos > -1 ? o : null,
                     l = o && o.pos < 0,
-                    a = new lr(this, s && s.node, e);
-                Ma(this.node, this.innerDeco, (c, f, h) => {
+                    a = new fr(this, s && s.node, e);
+                Ea(this.node, this.innerDeco, (c, f, h) => {
                     c.spec.marks ? a.syncToMarks(c.spec.marks, r, e) : c.type.side >= 0 && !h && a.syncToMarks(f == this.node.childCount ? O.none : this.node.child(f).marks, r, e), a.placeWidget(c, e, i)
                 }, (c, f, h, d) => {
                     a.syncToMarks(c.marks, r, e);
                     let p;
                     a.findNodeMatch(c, f, h, d) || l && e.state.selection.from > i && e.state.selection.to < i + c.nodeSize && (p = a.findIndexWithChild(o.node)) > -1 && a.updateNodeAt(c, f, h, p, e) || a.updateNextNode(c, f, h, e, d, i) || a.addNode(c, f, h, e, i), i += c.nodeSize
-                }), a.syncToMarks([], r, e), this.node.isTextblock && a.addTextblockHacks(), a.destroyRest(), (a.changed || this.dirty == Ve) && (s && this.protectLocalComposition(e, s), Mo(this.contentDOM, this.children, e), tt && Ca(this.dom))
+                }), a.syncToMarks([], r, e), this.node.isTextblock && a.addTextblockHacks(), a.destroyRest(), (a.changed || this.dirty == Fe) && (s && this.protectLocalComposition(e, s), Oo(this.contentDOM, this.children, e), tt && Aa(this.dom))
             }
             localCompositionInfo(e, t) {
                 let {
                     from: r,
                     to: i
                 } = e.state.selection;
                 if (!(e.state.selection instanceof w) || r < t || i > t + this.node.content.size) return null;
-                let o = e.domSelectionRange(),
-                    s = Oa(o.focusNode, o.focusOffset);
-                if (!s || !this.dom.contains(s.parentNode)) return null;
+                let o = e.input.compositionNode;
+                if (!o || !this.dom.contains(o.parentNode)) return null;
                 if (this.node.inlineContent) {
-                    let l = s.nodeValue,
-                        a = wa(this.node.content, l, r - t, i - t);
-                    return a < 0 ? null : {
-                        node: s,
-                        pos: a,
-                        text: l
+                    let s = o.nodeValue,
+                        l = Ia(this.node.content, s, r - t, i - t);
+                    return l < 0 ? null : {
+                        node: o,
+                        pos: l,
+                        text: s
                     }
                 } else return {
-                    node: s,
+                    node: o,
                     pos: -1,
                     text: ""
                 }
             }
             protectLocalComposition(e, {
                 node: t,
                 pos: r,
@@ -5582,46 +5623,46 @@
                 if (this.getDesc(t)) return;
                 let o = t;
                 for (; o.parentNode != this.contentDOM; o = o.parentNode) {
                     for (; o.previousSibling;) o.parentNode.removeChild(o.previousSibling);
                     for (; o.nextSibling;) o.parentNode.removeChild(o.nextSibling);
                     o.pmViewDesc && (o.pmViewDesc = void 0)
                 }
-                let s = new rr(this, o, t, i);
-                e.input.compositionNodes.push(s), this.children = ar(this.children, r, r + i.length, e, s)
+                let s = new sr(this, o, t, i);
+                e.input.compositionNodes.push(s), this.children = hr(this.children, r, r + i.length, e, s)
             }
             update(e, t, r, i) {
                 return this.dirty == le || !e.sameMarkup(this.node) ? !1 : (this.updateInner(e, t, r, i), !0)
             }
             updateInner(e, t, r, i) {
                 this.updateOuterDeco(t), this.node = e, this.innerDeco = r, this.contentDOM && this.updateChildren(i, this.posAtStart), this.dirty = _
             }
             updateOuterDeco(e) {
-                if (sr(e, this.outerDeco)) return;
+                if (cr(e, this.outerDeco)) return;
                 let t = this.nodeDOM.nodeType != 1,
                     r = this.dom;
-                this.dom = Co(this.dom, this.nodeDOM, or(this.outerDeco, this.node, t), or(e, this.node, t)), this.dom != r && (r.pmViewDesc = void 0, this.dom.pmViewDesc = this), this.outerDeco = e
+                this.dom = wo(this.dom, this.nodeDOM, ar(this.outerDeco, this.node, t), ar(e, this.node, t)), this.dom != r && (r.pmViewDesc = void 0, this.dom.pmViewDesc = this), this.outerDeco = e
             }
             selectNode() {
                 this.nodeDOM.nodeType == 1 && this.nodeDOM.classList.add("ProseMirror-selectednode"), (this.contentDOM || !this.node.type.spec.draggable) && (this.dom.draggable = !0)
             }
             deselectNode() {
                 this.nodeDOM.nodeType == 1 && this.nodeDOM.classList.remove("ProseMirror-selectednode"), (this.contentDOM || !this.node.type.spec.draggable) && this.dom.removeAttribute("draggable")
             }
             get domAtom() {
                 return this.node.isAtom
             }
         };
 
-    function $i(n, e, t, r, i) {
-        Oo(r, e, n);
-        let o = new Oe(void 0, n, e, t, r, r, r, i, 0);
+    function Hi(n, e, t, r, i) {
+        No(r, e, n);
+        let o = new we(void 0, n, e, t, r, r, r, i, 0);
         return o.contentDOM && o.updateChildren(i, 0), o
     }
-    var rn = class n extends Oe {
+    var on = class n extends we {
             constructor(e, t, r, i, o, s, l) {
                 super(e, t, r, i, o, null, s, l, 0)
             }
             parseRule() {
                 let e = this.nodeDOM.parentNode;
                 for (; e && e != this.dom && !e.pmIsDeco;) e = e.parentNode;
                 return {
@@ -5656,16 +5697,19 @@
             }
             markDirty(e, t) {
                 super.markDirty(e, t), this.dom != this.nodeDOM && (e == 0 || t == this.nodeDOM.nodeValue.length) && (this.dirty = le)
             }
             get domAtom() {
                 return !1
             }
+            isText(e) {
+                return this.node.text == e
+            }
         },
-        on = class extends $e {
+        sn = class extends Ke {
             parseRule() {
                 return {
                     ignore: !0
                 }
             }
             matchesHack(e) {
                 return this.dirty == _ && this.dom.nodeName == e
@@ -5673,15 +5717,15 @@
             get domAtom() {
                 return !0
             }
             get ignoreForCoords() {
                 return this.dom.nodeName == "IMG"
             }
         },
-        ir = class extends Oe {
+        lr = class extends we {
             constructor(e, t, r, i, o, s, l, a, c, f) {
                 super(e, t, r, i, o, s, l, c, f), this.spec = a
             }
             update(e, t, r, i) {
                 if (this.dirty == le) return !1;
                 if (this.spec.update) {
                     let o = this.spec.update(e, t, r);
@@ -5704,71 +5748,71 @@
                 return this.spec.stopEvent ? this.spec.stopEvent(e) : !1
             }
             ignoreMutation(e) {
                 return this.spec.ignoreMutation ? this.spec.ignoreMutation(e) : super.ignoreMutation(e)
             }
         };
 
-    function Mo(n, e, t) {
+    function Oo(n, e, t) {
         let r = n.firstChild,
             i = !1;
         for (let o = 0; o < e.length; o++) {
             let s = e[o],
                 l = s.dom;
             if (l.parentNode == n) {
-                for (; l != r;) r = Hi(r), i = !0;
+                for (; l != r;) r = ji(r), i = !0;
                 r = r.nextSibling
             } else i = !0, n.insertBefore(l, r);
             if (s instanceof nt) {
                 let a = r ? r.previousSibling : n.lastChild;
-                Mo(s.contentDOM, s.children, t), r = a ? a.nextSibling : n.firstChild
+                Oo(s.contentDOM, s.children, t), r = a ? a.nextSibling : n.firstChild
             }
         }
-        for (; r;) r = Hi(r), i = !0;
+        for (; r;) r = ji(r), i = !0;
         i && t.trackWrites == n && (t.trackWrites = null)
     }
     var Mt = function(n) {
         n && (this.nodeName = n)
     };
     Mt.prototype = Object.create(null);
-    var Le = [new Mt];
+    var Ve = [new Mt];
 
-    function or(n, e, t) {
-        if (n.length == 0) return Le;
-        let r = t ? Le[0] : new Mt,
+    function ar(n, e, t) {
+        if (n.length == 0) return Ve;
+        let r = t ? Ve[0] : new Mt,
             i = [r];
         for (let o = 0; o < n.length; o++) {
             let s = n[o].type.attrs;
             if (s) {
                 s.nodeName && i.push(r = new Mt(s.nodeName));
                 for (let l in s) {
                     let a = s[l];
                     a != null && (t && i.length == 1 && i.push(r = new Mt(e.isInline ? "span" : "div")), l == "class" ? r.class = (r.class ? r.class + " " : "") + a : l == "style" ? r.style = (r.style ? r.style + ";" : "") + a : l != "nodeName" && (r[l] = a))
                 }
             }
         }
         return i
     }
 
-    function Co(n, e, t, r) {
-        if (t == Le && r == Le) return e;
+    function wo(n, e, t, r) {
+        if (t == Ve && r == Ve) return e;
         let i = e;
         for (let o = 0; o < r.length; o++) {
             let s = r[o],
                 l = t[o];
             if (o) {
                 let a;
-                l && l.nodeName == s.nodeName && i != n && (a = i.parentNode) && a.nodeName.toLowerCase() == s.nodeName || (a = document.createElement(s.nodeName), a.pmIsDeco = !0, a.appendChild(i), l = Le[0]), i = a
+                l && l.nodeName == s.nodeName && i != n && (a = i.parentNode) && a.nodeName.toLowerCase() == s.nodeName || (a = document.createElement(s.nodeName), a.pmIsDeco = !0, a.appendChild(i), l = Ve[0]), i = a
             }
-            ka(i, l || Le[0], s)
+            Na(i, l || Ve[0], s)
         }
         return i
     }
 
-    function ka(n, e, t) {
+    function Na(n, e, t) {
         for (let r in e) r != "class" && r != "style" && r != "nodeName" && !(r in t) && n.removeAttribute(r);
         for (let r in t) r != "class" && r != "style" && r != "nodeName" && t[r] != e[r] && n.setAttribute(r, t[r]);
         if (e.class != t.class) {
             let r = e.class ? e.class.split(" ").filter(Boolean) : [],
                 i = t.class ? t.class.split(" ").filter(Boolean) : [];
             for (let o = 0; o < r.length; o++) i.indexOf(r[o]) == -1 && n.classList.remove(r[o]);
             for (let o = 0; o < i.length; o++) r.indexOf(i[o]) == -1 && n.classList.add(i[o]);
@@ -5780,32 +5824,32 @@
                     i;
                 for (; i = r.exec(e.style);) n.style.removeProperty(i[1])
             }
             t.style && (n.style.cssText += t.style)
         }
     }
 
-    function Oo(n, e, t) {
-        return Co(n, n, Le, or(e, t, n.nodeType != 1))
+    function No(n, e, t) {
+        return wo(n, n, Ve, ar(e, t, n.nodeType != 1))
     }
 
-    function sr(n, e) {
+    function cr(n, e) {
         if (n.length != e.length) return !1;
         for (let t = 0; t < n.length; t++)
             if (!n[t].type.eq(e[t].type)) return !1;
         return !0
     }
 
-    function Hi(n) {
+    function ji(n) {
         let e = n.nextSibling;
         return n.parentNode.removeChild(n), e
     }
-    var lr = class {
+    var fr = class {
         constructor(e, t, r) {
-            this.lock = t, this.view = r, this.index = 0, this.stack = [], this.changed = !1, this.top = e, this.preMatch = xa(e.node.content, e)
+            this.lock = t, this.view = r, this.index = 0, this.stack = [], this.changed = !1, this.top = e, this.preMatch = Ta(e.node.content, e)
         }
         destroyBetween(e, t) {
             if (e != t) {
                 for (let r = e; r < t; r++) this.top.children[r].destroy();
                 this.top.children.splice(e, t - e), this.changed = !0
             }
         }
@@ -5848,15 +5892,15 @@
                         break
                     }
                 }
             return o < 0 ? !1 : (this.destroyBetween(this.index, o), this.index++, !0)
         }
         updateNodeAt(e, t, r, i, o) {
             let s = this.top.children[i];
-            return s.dirty == le && s.dom == s.contentDOM && (s.dirty = Ve), s.update(e, t, r, o) ? (this.destroyBetween(this.index, i), this.index++, !0) : !1
+            return s.dirty == le && s.dom == s.contentDOM && (s.dirty = Fe), s.update(e, t, r, o) ? (this.destroyBetween(this.index, i), this.index++, !0) : !1
         }
         findIndexWithChild(e) {
             for (;;) {
                 let t = e.parentNode;
                 if (!t) return -1;
                 if (t == this.top.contentDOM) {
                     let r = e.pmViewDesc;
@@ -5868,68 +5912,68 @@
                 }
                 e = t
             }
         }
         updateNextNode(e, t, r, i, o, s) {
             for (let l = this.index; l < this.top.children.length; l++) {
                 let a = this.top.children[l];
-                if (a instanceof Oe) {
+                if (a instanceof we) {
                     let c = this.preMatch.matched.get(a);
                     if (c != null && c != o) return !1;
                     let f = a.dom,
-                        h, d = this.isLocked(f) && !(e.isText && a.node && a.node.isText && a.nodeDOM.nodeValue == e.text && a.dirty != le && sr(t, a.outerDeco));
+                        h, d = this.isLocked(f) && !(e.isText && a.node && a.node.isText && a.nodeDOM.nodeValue == e.text && a.dirty != le && cr(t, a.outerDeco));
                     if (!d && a.update(e, t, r, i)) return this.destroyBetween(this.index, l), a.dom != f && (this.changed = !0), this.index++, !0;
-                    if (!d && (h = this.recreateWrapper(a, e, t, r, i, s))) return this.top.children[this.index] = h, h.contentDOM && (h.dirty = Ve, h.updateChildren(i, s + 1), h.dirty = _), this.changed = !0, this.index++, !0;
+                    if (!d && (h = this.recreateWrapper(a, e, t, r, i, s))) return this.top.children[this.index] = h, h.contentDOM && (h.dirty = Fe, h.updateChildren(i, s + 1), h.dirty = _), this.changed = !0, this.index++, !0;
                     break
                 }
             }
             return !1
         }
         recreateWrapper(e, t, r, i, o, s) {
             if (e.dirty || t.isAtom || !e.children.length || !e.node.content.eq(t.content)) return null;
-            let l = Oe.create(this.top, t, r, i, o, s);
+            let l = we.create(this.top, t, r, i, o, s);
             if (l.contentDOM) {
                 l.children = e.children, e.children = [];
                 for (let a of l.children) a.parent = l
             }
             return e.destroy(), l
         }
         addNode(e, t, r, i, o) {
-            let s = Oe.create(this.top, e, t, r, i, o);
+            let s = we.create(this.top, e, t, r, i, o);
             s.contentDOM && s.updateChildren(i, o + 1), this.top.children.splice(this.index++, 0, s), this.changed = !0
         }
         placeWidget(e, t, r) {
             let i = this.index < this.top.children.length ? this.top.children[this.index] : null;
             if (i && i.matchesWidget(e) && (e == i.widget || !i.widget.type.toDOM.parentNode)) this.index++;
             else {
-                let o = new nn(this.top, e, t, r);
+                let o = new rn(this.top, e, t, r);
                 this.top.children.splice(this.index++, 0, o), this.changed = !0
             }
         }
         addTextblockHacks() {
             let e = this.top.children[this.index - 1],
                 t = this.top;
             for (; e instanceof nt;) t = e, e = t.children[t.children.length - 1];
-            (!e || !(e instanceof rn) || /\n$/.test(e.node.text) || this.view.requiresGeckoHackNode && /\s$/.test(e.node.text)) && ((F || R) && e && e.dom.contentEditable == "false" && this.addHackNode("IMG", t), this.addHackNode("BR", this.top))
+            (!e || !(e instanceof on) || /\n$/.test(e.node.text) || this.view.requiresGeckoHackNode && /\s$/.test(e.node.text)) && ((F || B) && e && e.dom.contentEditable == "false" && this.addHackNode("IMG", t), this.addHackNode("BR", this.top))
         }
         addHackNode(e, t) {
             if (t == this.top && this.index < t.children.length && t.children[this.index].matchesHack(e)) this.index++;
             else {
                 let r = document.createElement(e);
                 e == "IMG" && (r.className = "ProseMirror-separator", r.alt = ""), e == "BR" && (r.className = "ProseMirror-trailingBreak");
-                let i = new on(this.top, [], r, null);
+                let i = new sn(this.top, [], r, null);
                 t != this.top ? t.children.push(i) : t.children.splice(this.index++, 0, i), this.changed = !0
             }
         }
         isLocked(e) {
             return this.lock && (e == this.lock || e.nodeType == 1 && e.contains(this.lock.parentNode))
         }
     };
 
-    function xa(n, e) {
+    function Ta(n, e) {
         let t = e,
             r = t.children.length,
             i = n.childCount,
             o = new Map,
             s = [];
         e: for (; i > 0;) {
             let l;
@@ -5953,19 +5997,19 @@
         return {
             index: i,
             matched: o,
             matches: s.reverse()
         }
     }
 
-    function Sa(n, e) {
+    function Da(n, e) {
         return n.type.side - e.type.side
     }
 
-    function Ma(n, e, t, r) {
+    function Ea(n, e, t, r) {
         let i = e.locals(n),
             o = 0;
         if (i.length == 0) {
             for (let c = 0; c < n.childCount; c++) {
                 let f = n.child(c);
                 r(f, i, e.forChild(o, f), c), o += f.nodeSize
             }
@@ -5978,15 +6022,15 @@
             let f, h;
             for (; s < i.length && i[s].to == o;) {
                 let g = i[s++];
                 g.widget && (f ? (h || (h = [f])).push(g) : f = g)
             }
             if (f)
                 if (h) {
-                    h.sort(Sa);
+                    h.sort(Da);
                     for (let g = 0; g < h.length; g++) t(h[g], c, !!a)
                 } else t(f, c, !!a);
             let d, p;
             if (a) p = -1, d = a, a = null;
             else if (c < n.childCount) p = c, d = n.child(c++);
             else break;
             for (let g = 0; g < l.length; g++) l[g].to <= o && l.splice(g--, 1);
@@ -6000,33 +6044,22 @@
             } else
                 for (; s < i.length && i[s].to < u;) s++;
             let m = d.isInline && !d.isLeaf ? l.filter(g => !g.inline) : l.slice();
             r(d, m, e.forChild(o, d), p), o = u
         }
     }
 
-    function Ca(n) {
+    function Aa(n) {
         if (n.nodeName == "UL" || n.nodeName == "OL") {
             let e = n.style.cssText;
             n.style.cssText = e + "; list-style: square !important", window.getComputedStyle(n).listStyle, n.style.cssText = e
         }
     }
 
-    function Oa(n, e) {
-        for (;;) {
-            if (n.nodeType == 3) return n;
-            if (n.nodeType == 1 && e > 0) {
-                if (n.childNodes.length > e && n.childNodes[e].nodeType == 3) return n.childNodes[e];
-                n = n.childNodes[e - 1], e = oe(n)
-            } else if (n.nodeType == 1 && e < n.childNodes.length) n = n.childNodes[e], e = 0;
-            else return null
-        }
-    }
-
-    function wa(n, e, t, r) {
+    function Ia(n, e, t, r) {
         for (let i = 0, o = 0; i < n.childCount && o <= r;) {
             let s = n.child(i++),
                 l = o;
             if (o += s.nodeSize, !s.isText) continue;
             let a = s.text;
             for (; i < n.childCount;) {
                 let c = n.child(i++);
@@ -6039,318 +6072,318 @@
                 if (c >= 0 && c + e.length + l >= t) return l + c;
                 if (t == r && a.length >= r + e.length - l && a.slice(r - l, r - l + e.length) == e) return r
             }
         }
         return -1
     }
 
-    function ar(n, e, t, r, i) {
+    function hr(n, e, t, r, i) {
         let o = [];
         for (let s = 0, l = 0; s < n.length; s++) {
             let a = n[s],
                 c = l,
                 f = l += a.size;
             c >= t || f <= e ? o.push(a) : (c < e && o.push(a.slice(0, e - c, r)), i && (o.push(i), i = void 0), f > t && o.push(a.slice(t - c, a.size, r)))
         }
         return o
     }
 
-    function xr(n, e = null) {
+    function Mr(n, e = null) {
         let t = n.domSelectionRange(),
             r = n.state.doc;
         if (!t.focusNode) return null;
         let i = n.docView.nearestDesc(t.focusNode),
             o = i && i.size == 0,
             s = n.docView.posFromDOM(t.focusNode, t.focusOffset, 1);
         if (s < 0) return null;
         let l = r.resolve(s),
             a, c;
-        if (un(t)) {
+        if (pn(t)) {
             for (a = l; i && !i.node;) i = i.parent;
             let f = i.node;
-            if (i && f.isAtom && x.isSelectable(f) && i.parent && !(f.isInline && Ql(t.focusNode, t.focusOffset, i.dom))) {
+            if (i && f.isAtom && x.isSelectable(f) && i.parent && !(f.isInline && oa(t.focusNode, t.focusOffset, i.dom))) {
                 let h = i.posBefore;
                 c = new x(s == h ? l : r.resolve(h))
             }
         } else {
             let f = n.docView.posFromDOM(t.anchorNode, t.anchorOffset, 1);
             if (f < 0) return null;
             a = r.resolve(f)
         }
         if (!c) {
             let f = e == "pointer" || n.state.selection.head < l.pos && !o ? 1 : -1;
-            c = Sr(n, a, l, f)
+            c = Cr(n, a, l, f)
         }
         return c
     }
 
-    function wo(n) {
-        return n.editable ? n.hasFocus() : To(n) && document.activeElement && document.activeElement.contains(n.dom)
+    function To(n) {
+        return n.editable ? n.hasFocus() : Eo(n) && document.activeElement && document.activeElement.contains(n.dom)
     }
 
     function ge(n, e = !1) {
         let t = n.state.selection;
-        if (No(n, t), !!wo(n)) {
-            if (!e && n.input.mouseDown && n.input.mouseDown.allowDefault && R) {
+        if (Do(n, t), !!To(n)) {
+            if (!e && n.input.mouseDown && n.input.mouseDown.allowDefault && B) {
                 let r = n.domSelectionRange(),
                     i = n.domObserver.currentSelection;
-                if (r.anchorNode && i.anchorNode && Ke(r.anchorNode, r.anchorOffset, i.anchorNode, i.anchorOffset)) {
+                if (r.anchorNode && i.anchorNode && qe(r.anchorNode, r.anchorOffset, i.anchorNode, i.anchorOffset)) {
                     n.input.mouseDown.delayedSelectionSync = !0, n.domObserver.setCurSelection();
                     return
                 }
             }
-            if (n.domObserver.disconnectSelection(), n.cursorWrapper) Ta(n);
+            if (n.domObserver.disconnectSelection(), n.cursorWrapper) va(n);
             else {
                 let {
                     anchor: r,
                     head: i
                 } = t, o, s;
-                ji && !(t instanceof w) && (t.$from.parent.inlineContent || (o = _i(n, t.from)), !t.empty && !t.$from.parent.inlineContent && (s = _i(n, t.to))), n.docView.setSelection(r, i, n.root, e), ji && (o && Ui(o), s && Ui(s)), t.visible ? n.dom.classList.remove("ProseMirror-hideselection") : (n.dom.classList.add("ProseMirror-hideselection"), "onselectionchange" in document && Na(n))
+                _i && !(t instanceof w) && (t.$from.parent.inlineContent || (o = Ui(n, t.from)), !t.empty && !t.$from.parent.inlineContent && (s = Ui(n, t.to))), n.docView.setSelection(r, i, n.root, e), _i && (o && Gi(o), s && Gi(s)), t.visible ? n.dom.classList.remove("ProseMirror-hideselection") : (n.dom.classList.add("ProseMirror-hideselection"), "onselectionchange" in document && Ra(n))
             }
             n.domObserver.setCurSelection(), n.domObserver.connectSelection()
         }
     }
-    var ji = F || R && ta < 63;
+    var _i = F || B && aa < 63;
 
-    function _i(n, e) {
+    function Ui(n, e) {
         let {
             node: t,
             offset: r
         } = n.docView.domFromPos(e, 0), i = r < t.childNodes.length ? t.childNodes[r] : null, o = r ? t.childNodes[r - 1] : null;
-        if (F && i && i.contentEditable == "false") return Xn(i);
+        if (F && i && i.contentEditable == "false") return Zn(i);
         if ((!i || i.contentEditable == "false") && (!o || o.contentEditable == "false")) {
-            if (i) return Xn(i);
-            if (o) return Xn(o)
+            if (i) return Zn(i);
+            if (o) return Zn(o)
         }
     }
 
-    function Xn(n) {
+    function Zn(n) {
         return n.contentEditable = "true", F && n.draggable && (n.draggable = !1, n.wasDraggable = !0), n
     }
 
-    function Ui(n) {
+    function Gi(n) {
         n.contentEditable = "false", n.wasDraggable && (n.draggable = !0, n.wasDraggable = null)
     }
 
-    function Na(n) {
+    function Ra(n) {
         let e = n.dom.ownerDocument;
         e.removeEventListener("selectionchange", n.input.hideSelectionGuard);
         let t = n.domSelectionRange(),
             r = t.anchorNode,
             i = t.anchorOffset;
         e.addEventListener("selectionchange", n.input.hideSelectionGuard = () => {
             (t.anchorNode != r || t.anchorOffset != i) && (e.removeEventListener("selectionchange", n.input.hideSelectionGuard), setTimeout(() => {
-                (!wo(n) || n.state.selection.visible) && n.dom.classList.remove("ProseMirror-hideselection")
+                (!To(n) || n.state.selection.visible) && n.dom.classList.remove("ProseMirror-hideselection")
             }, 20))
         })
     }
 
-    function Ta(n) {
+    function va(n) {
         let e = n.domSelection(),
             t = document.createRange(),
             r = n.cursorWrapper.dom,
             i = r.nodeName == "IMG";
-        i ? t.setEnd(r.parentNode, B(r) + 1) : t.setEnd(r, 0), t.collapse(!1), e.removeAllRanges(), e.addRange(t), !i && !n.state.selection.visible && W && Ce <= 11 && (r.disabled = !0, r.disabled = !1)
+        i ? t.setEnd(r.parentNode, I(r) + 1) : t.setEnd(r, 0), t.collapse(!1), e.removeAllRanges(), e.addRange(t), !i && !n.state.selection.visible && W && Oe <= 11 && (r.disabled = !0, r.disabled = !1)
     }
 
-    function No(n, e) {
+    function Do(n, e) {
         if (e instanceof x) {
             let t = n.docView.descAt(e.from);
-            t != n.lastSelectedViewDesc && (Gi(n), t && t.selectNode(), n.lastSelectedViewDesc = t)
-        } else Gi(n)
+            t != n.lastSelectedViewDesc && (Yi(n), t && t.selectNode(), n.lastSelectedViewDesc = t)
+        } else Yi(n)
     }
 
-    function Gi(n) {
+    function Yi(n) {
         n.lastSelectedViewDesc && (n.lastSelectedViewDesc.parent && n.lastSelectedViewDesc.deselectNode(), n.lastSelectedViewDesc = void 0)
     }
 
-    function Sr(n, e, t, r) {
+    function Cr(n, e, t, r) {
         return n.someProp("createSelectionBetween", i => i(n, e, t)) || w.between(e, t, r)
     }
 
-    function Yi(n) {
-        return n.editable && !n.hasFocus() ? !1 : To(n)
+    function Xi(n) {
+        return n.editable && !n.hasFocus() ? !1 : Eo(n)
     }
 
-    function To(n) {
+    function Eo(n) {
         let e = n.domSelectionRange();
         if (!e.anchorNode) return !1;
         try {
             return n.dom.contains(e.anchorNode.nodeType == 3 ? e.anchorNode.parentNode : e.anchorNode) && (n.editable || n.dom.contains(e.focusNode.nodeType == 3 ? e.focusNode.parentNode : e.focusNode))
         } catch (t) {
             return !1
         }
     }
 
-    function Da(n) {
+    function Pa(n) {
         let e = n.docView.domFromPos(n.state.selection.anchor, 0),
             t = n.domSelectionRange();
-        return Ke(e.node, e.offset, t.anchorNode, t.anchorOffset)
+        return qe(e.node, e.offset, t.anchorNode, t.anchorOffset)
     }
 
-    function cr(n, e) {
+    function dr(n, e) {
         let {
             $anchor: t,
             $head: r
         } = n.selection, i = e > 0 ? t.max(r) : t.min(r), o = i.parent.inlineContent ? i.depth ? n.doc.resolve(e > 0 ? i.after() : i.before()) : null : i;
         return o && S.findFrom(o, e)
     }
 
-    function Se(n, e) {
+    function Me(n, e) {
         return n.dispatch(n.state.tr.setSelection(e).scrollIntoView()), !0
     }
 
-    function Xi(n, e, t) {
+    function Qi(n, e, t) {
         let r = n.state.selection;
         if (r instanceof w)
             if (t.indexOf("s") > -1) {
                 let {
                     $head: i
                 } = r, o = i.textOffset ? null : e < 0 ? i.nodeBefore : i.nodeAfter;
                 if (!o || o.isText || !o.isLeaf) return !1;
                 let s = n.state.doc.resolve(i.pos + o.nodeSize * (e < 0 ? -1 : 1));
-                return Se(n, new w(r.$anchor, s))
+                return Me(n, new w(r.$anchor, s))
             } else if (r.empty) {
             if (n.endOfTextblock(e > 0 ? "forward" : "backward")) {
-                let i = cr(n.state, e);
-                return i && i instanceof x ? Se(n, i) : !1
+                let i = dr(n.state, e);
+                return i && i instanceof x ? Me(n, i) : !1
             } else if (!(j && t.indexOf("m") > -1)) {
                 let i = r.$head,
                     o = i.textOffset ? null : e < 0 ? i.nodeBefore : i.nodeAfter,
                     s;
                 if (!o || o.isText) return !1;
                 let l = e < 0 ? i.pos - o.nodeSize : i.pos;
-                return o.isAtom || (s = n.docView.descAt(l)) && !s.contentDOM ? x.isSelectable(o) ? Se(n, new x(e < 0 ? n.state.doc.resolve(i.pos - o.nodeSize) : i)) : Dt ? Se(n, new w(n.state.doc.resolve(e < 0 ? l : l + o.nodeSize))) : !1 : !1
+                return o.isAtom || (s = n.docView.descAt(l)) && !s.contentDOM ? x.isSelectable(o) ? Me(n, new x(e < 0 ? n.state.doc.resolve(i.pos - o.nodeSize) : i)) : Et ? Me(n, new w(n.state.doc.resolve(e < 0 ? l : l + o.nodeSize))) : !1 : !1
             }
         } else return !1;
         else {
-            if (r instanceof x && r.node.isInline) return Se(n, new w(e > 0 ? r.$to : r.$from));
+            if (r instanceof x && r.node.isInline) return Me(n, new w(e > 0 ? r.$to : r.$from));
             {
-                let i = cr(n.state, e);
-                return i ? Se(n, i) : !1
+                let i = dr(n.state, e);
+                return i ? Me(n, i) : !1
             }
         }
     }
 
-    function sn(n) {
+    function ln(n) {
         return n.nodeType == 3 ? n.nodeValue.length : n.childNodes.length
     }
 
     function Ct(n, e) {
         let t = n.pmViewDesc;
         return t && t.size == 0 && (e < 0 || n.nextSibling || n.nodeName != "BR")
     }
 
     function Qe(n, e) {
-        return e < 0 ? Ea(n) : Ia(n)
+        return e < 0 ? za(n) : Ba(n)
     }
 
-    function Ea(n) {
+    function za(n) {
         let e = n.domSelectionRange(),
             t = e.focusNode,
             r = e.focusOffset;
         if (!t) return;
         let i, o, s = !1;
-        for (X && t.nodeType == 1 && r < sn(t) && Ct(t.childNodes[r], -1) && (s = !0);;)
+        for (X && t.nodeType == 1 && r < ln(t) && Ct(t.childNodes[r], -1) && (s = !0);;)
             if (r > 0) {
                 if (t.nodeType != 1) break;
                 {
                     let l = t.childNodes[r - 1];
                     if (Ct(l, -1)) i = t, o = --r;
                     else if (l.nodeType == 3) t = l, r = t.nodeValue.length;
                     else break
                 }
             } else {
-                if (Do(t)) break;
+                if (Ao(t)) break;
                 {
                     let l = t.previousSibling;
-                    for (; l && Ct(l, -1);) i = t.parentNode, o = B(l), l = l.previousSibling;
-                    if (l) t = l, r = sn(t);
+                    for (; l && Ct(l, -1);) i = t.parentNode, o = I(l), l = l.previousSibling;
+                    if (l) t = l, r = ln(t);
                     else {
                         if (t = t.parentNode, t == n.dom) break;
                         r = 0
                     }
                 }
-            } s ? fr(n, t, r) : i && fr(n, i, o)
+            } s ? ur(n, t, r) : i && ur(n, i, o)
     }
 
-    function Ia(n) {
+    function Ba(n) {
         let e = n.domSelectionRange(),
             t = e.focusNode,
             r = e.focusOffset;
         if (!t) return;
-        let i = sn(t),
+        let i = ln(t),
             o, s;
         for (;;)
             if (r < i) {
                 if (t.nodeType != 1) break;
                 let l = t.childNodes[r];
                 if (Ct(l, 1)) o = t, s = ++r;
                 else break
             } else {
-                if (Do(t)) break;
+                if (Ao(t)) break;
                 {
                     let l = t.nextSibling;
-                    for (; l && Ct(l, 1);) o = l.parentNode, s = B(l) + 1, l = l.nextSibling;
-                    if (l) t = l, r = 0, i = sn(t);
+                    for (; l && Ct(l, 1);) o = l.parentNode, s = I(l) + 1, l = l.nextSibling;
+                    if (l) t = l, r = 0, i = ln(t);
                     else {
                         if (t = t.parentNode, t == n.dom) break;
                         r = i = 0
                     }
                 }
-            } o && fr(n, o, s)
+            } o && ur(n, o, s)
     }
 
-    function Do(n) {
+    function Ao(n) {
         let e = n.pmViewDesc;
         return e && e.node && e.node.isBlock
     }
 
-    function Aa(n, e) {
-        for (; n && e == n.childNodes.length && !br(n);) e = B(n) + 1, n = n.parentNode;
+    function Fa(n, e) {
+        for (; n && e == n.childNodes.length && !Dt(n);) e = I(n) + 1, n = n.parentNode;
         for (; n && e < n.childNodes.length;) {
             let t = n.childNodes[e];
             if (t.nodeType == 3) return t;
             if (t.nodeType == 1 && t.contentEditable == "false") break;
             n = t, e = 0
         }
     }
 
-    function va(n, e) {
-        for (; n && !e && !br(n);) e = B(n), n = n.parentNode;
+    function Va(n, e) {
+        for (; n && !e && !Dt(n);) e = I(n), n = n.parentNode;
         for (; n && e;) {
             let t = n.childNodes[e - 1];
             if (t.nodeType == 3) return t;
             if (t.nodeType == 1 && t.contentEditable == "false") break;
             n = t, e = n.childNodes.length
         }
     }
 
-    function fr(n, e, t) {
+    function ur(n, e, t) {
         if (e.nodeType != 3) {
             let o, s;
-            (s = Aa(e, t)) ? (e = s, t = 0) : (o = va(e, t)) && (e = o, t = o.nodeValue.length)
+            (s = Fa(e, t)) ? (e = s, t = 0) : (o = Va(e, t)) && (e = o, t = o.nodeValue.length)
         }
         let r = n.domSelection();
-        if (un(r)) {
+        if (pn(r)) {
             let o = document.createRange();
             o.setEnd(e, t), o.setStart(e, t), r.removeAllRanges(), r.addRange(o)
         } else r.extend && r.extend(e, t);
         n.domObserver.setCurSelection();
         let {
             state: i
         } = n;
         setTimeout(() => {
             n.state == i && ge(n)
         }, 50)
     }
 
-    function Qi(n, e) {
+    function Zi(n, e) {
         let t = n.state.doc.resolve(e);
-        if (!(R || na) && t.parent.inlineContent) {
+        if (!(B || ca) && t.parent.inlineContent) {
             let i = n.coordsAtPos(e);
             if (e > t.start()) {
                 let o = n.coordsAtPos(e - 1),
                     s = (o.top + o.bottom) / 2;
                 if (s > i.top && s < i.bottom && Math.abs(o.left - i.left) > 1) return o.left < i.left ? "ltr" : "rtl"
             }
             if (e < t.end()) {
@@ -6358,34 +6391,34 @@
                     s = (o.top + o.bottom) / 2;
                 if (s > i.top && s < i.bottom && Math.abs(o.left - i.left) > 1) return o.left > i.left ? "ltr" : "rtl"
             }
         }
         return getComputedStyle(n.dom).direction == "rtl" ? "rtl" : "ltr"
     }
 
-    function Zi(n, e, t) {
+    function eo(n, e, t) {
         let r = n.state.selection;
         if (r instanceof w && !r.empty || t.indexOf("s") > -1 || j && t.indexOf("m") > -1) return !1;
         let {
             $from: i,
             $to: o
         } = r;
         if (!i.parent.inlineContent || n.endOfTextblock(e < 0 ? "up" : "down")) {
-            let s = cr(n.state, e);
-            if (s && s instanceof x) return Se(n, s)
+            let s = dr(n.state, e);
+            if (s && s instanceof x) return Me(n, s)
         }
         if (!i.parent.inlineContent) {
             let s = e < 0 ? i : o,
                 l = r instanceof $ ? S.near(s, e) : S.findFrom(s, e);
-            return l ? Se(n, l) : !1
+            return l ? Me(n, l) : !1
         }
         return !1
     }
 
-    function eo(n, e) {
+    function to(n, e) {
         if (!(n.state.selection instanceof w)) return !0;
         let {
             $head: t,
             $anchor: r,
             empty: i
         } = n.state.selection;
         if (!t.sameParent(r)) return !0;
@@ -6395,57 +6428,57 @@
         if (o && !o.isText) {
             let s = n.state.tr;
             return e < 0 ? s.delete(t.pos - o.nodeSize, t.pos) : s.delete(t.pos, t.pos + o.nodeSize), n.dispatch(s), !0
         }
         return !1
     }
 
-    function to(n, e, t) {
+    function no(n, e, t) {
         n.domObserver.stop(), e.contentEditable = t, n.domObserver.start()
     }
 
-    function Pa(n) {
+    function La(n) {
         if (!F || n.state.selection.$head.parentOffset > 0) return !1;
         let {
             focusNode: e,
             focusOffset: t
         } = n.domSelectionRange();
         if (e && e.nodeType == 1 && t == 0 && e.firstChild && e.firstChild.contentEditable == "false") {
             let r = e.firstChild;
-            to(n, r, "true"), setTimeout(() => to(n, r, "false"), 20)
+            no(n, r, "true"), setTimeout(() => no(n, r, "false"), 20)
         }
         return !1
     }
 
-    function Ra(n) {
+    function Ja(n) {
         let e = "";
         return n.ctrlKey && (e += "c"), n.metaKey && (e += "m"), n.altKey && (e += "a"), n.shiftKey && (e += "s"), e
     }
 
-    function za(n, e) {
+    function Wa(n, e) {
         let t = e.keyCode,
-            r = Ra(e);
-        if (t == 8 || j && t == 72 && r == "c") return eo(n, -1) || Qe(n, -1);
-        if (t == 46 && !e.shiftKey || j && t == 68 && r == "c") return eo(n, 1) || Qe(n, 1);
+            r = Ja(e);
+        if (t == 8 || j && t == 72 && r == "c") return to(n, -1) || Qe(n, -1);
+        if (t == 46 && !e.shiftKey || j && t == 68 && r == "c") return to(n, 1) || Qe(n, 1);
         if (t == 13 || t == 27) return !0;
         if (t == 37 || j && t == 66 && r == "c") {
-            let i = t == 37 ? Qi(n, n.state.selection.from) == "ltr" ? -1 : 1 : -1;
-            return Xi(n, i, r) || Qe(n, i)
+            let i = t == 37 ? Zi(n, n.state.selection.from) == "ltr" ? -1 : 1 : -1;
+            return Qi(n, i, r) || Qe(n, i)
         } else if (t == 39 || j && t == 70 && r == "c") {
-            let i = t == 39 ? Qi(n, n.state.selection.from) == "ltr" ? 1 : -1 : 1;
-            return Xi(n, i, r) || Qe(n, i)
+            let i = t == 39 ? Zi(n, n.state.selection.from) == "ltr" ? 1 : -1 : 1;
+            return Qi(n, i, r) || Qe(n, i)
         } else {
-            if (t == 38 || j && t == 80 && r == "c") return Zi(n, -1, r) || Qe(n, -1);
-            if (t == 40 || j && t == 78 && r == "c") return Pa(n) || Zi(n, 1, r) || Qe(n, 1);
+            if (t == 38 || j && t == 80 && r == "c") return eo(n, -1, r) || Qe(n, -1);
+            if (t == 40 || j && t == 78 && r == "c") return La(n) || eo(n, 1, r) || Qe(n, 1);
             if (r == (j ? "m" : "c") && (t == 66 || t == 73 || t == 89 || t == 90)) return !0
         }
         return !1
     }
 
-    function Eo(n, e) {
+    function Io(n, e) {
         n.someProp("transformCopied", p => {
             e = p(e, n)
         });
         let t = [],
             {
                 content: r,
                 openStart: i,
@@ -6453,22 +6486,22 @@
             } = e;
         for (; i > 1 && o > 1 && r.childCount == 1 && r.firstChild.childCount == 1;) {
             i--, o--;
             let p = r.firstChild;
             t.push(p.type.name, p.attrs != p.type.defaultAttrs ? p.attrs : null), r = p.content
         }
         let s = n.someProp("clipboardSerializer") || ie.fromSchema(n.state.schema),
-            l = zo(),
+            l = Fo(),
             a = l.createElement("div");
         a.appendChild(s.serializeFragment(r, {
             document: l
         }));
         let c = a.firstChild,
             f, h = 0;
-        for (; c && c.nodeType == 1 && (f = Ro[c.nodeName.toLowerCase()]);) {
+        for (; c && c.nodeType == 1 && (f = Bo[c.nodeName.toLowerCase()]);) {
             for (let p = f.length - 1; p >= 0; p--) {
                 let u = l.createElement(f[p]);
                 for (; a.firstChild;) u.appendChild(a.firstChild);
                 a.appendChild(u), h++
             }
             c = a.firstChild
         }
@@ -6478,15 +6511,15 @@
 `);
         return {
             dom: a,
             text: d
         }
     }
 
-    function Io(n, e, t, r, i) {
+    function Ro(n, e, t, r, i) {
         let o = i.parent.type.spec.code,
             s, l;
         if (!t && !e) return null;
         let a = e && (r || o || !t);
         if (a) {
             if (n.someProp("transformPastedText", d => {
                     e = d(e, o || r, n)
@@ -6503,133 +6536,133 @@
                 s = document.createElement("div"), e.split(/(?:\r\n?|\n)+/).forEach(m => {
                     let g = s.appendChild(document.createElement("p"));
                     m && g.appendChild(u.serializeNode(p.text(m, d)))
                 })
             }
         } else n.someProp("transformPastedHTML", h => {
             t = h(t, n)
-        }), s = Va(t), Dt && La(s);
+        }), s = $a(t), Et && Ha(s);
         let c = s && s.querySelector("[data-pm-slice]"),
             f = c && /^(\d+) (\d+)(?: -(\d+))? (.*)/.exec(c.getAttribute("data-pm-slice") || "");
         if (f && f[3])
             for (let h = +f[3]; h > 0; h--) {
                 let d = s.firstChild;
                 for (; d && d.nodeType != 1;) d = d.nextSibling;
                 if (!d) break;
                 s = d
             }
-        if (l || (l = (n.someProp("clipboardParser") || n.someProp("domParser") || Re.fromSchema(n.state.schema)).parseSlice(s, {
+        if (l || (l = (n.someProp("clipboardParser") || n.someProp("domParser") || ve.fromSchema(n.state.schema)).parseSlice(s, {
                 preserveWhitespace: !!(a || f),
                 context: i,
                 ruleFromNode(d) {
-                    return d.nodeName == "BR" && !d.nextSibling && d.parentNode && !Ba.test(d.parentNode.nodeName) ? {
+                    return d.nodeName == "BR" && !d.nextSibling && d.parentNode && !qa.test(d.parentNode.nodeName) ? {
                         ignore: !0
                     } : null
                 }
-            })), f) l = Ja(no(l, +f[1], +f[2]), f[4]);
-        else if (l = b.maxOpen(Fa(l.content, i), !0), l.openStart || l.openEnd) {
+            })), f) l = ja(ro(l, +f[1], +f[2]), f[4]);
+        else if (l = b.maxOpen(Ka(l.content, i), !0), l.openStart || l.openEnd) {
             let h = 0,
                 d = 0;
             for (let p = l.content.firstChild; h < l.openStart && !p.type.spec.isolating; h++, p = p.firstChild);
             for (let p = l.content.lastChild; d < l.openEnd && !p.type.spec.isolating; d++, p = p.lastChild);
-            l = no(l, h, d)
+            l = ro(l, h, d)
         }
         return n.someProp("transformPasted", h => {
             l = h(l, n)
         }), l
     }
-    var Ba = /^(a|abbr|acronym|b|cite|code|del|em|i|ins|kbd|label|output|q|ruby|s|samp|span|strong|sub|sup|time|u|tt|var)$/i;
+    var qa = /^(a|abbr|acronym|b|cite|code|del|em|i|ins|kbd|label|output|q|ruby|s|samp|span|strong|sub|sup|time|u|tt|var)$/i;
 
-    function Fa(n, e) {
+    function Ka(n, e) {
         if (n.childCount < 2) return n;
         for (let t = e.depth; t >= 0; t--) {
             let i = e.node(t).contentMatchAt(e.index(t)),
                 o, s = [];
             if (n.forEach(l => {
                     if (!s) return;
                     let a = i.findWrapping(l.type),
                         c;
                     if (!a) return s = null;
-                    if (c = s.length && o.length && vo(a, o, l, s[s.length - 1], 0)) s[s.length - 1] = c;
+                    if (c = s.length && o.length && Po(a, o, l, s[s.length - 1], 0)) s[s.length - 1] = c;
                     else {
-                        s.length && (s[s.length - 1] = Po(s[s.length - 1], o.length));
-                        let f = Ao(l, a);
+                        s.length && (s[s.length - 1] = zo(s[s.length - 1], o.length));
+                        let f = vo(l, a);
                         s.push(f), i = i.matchType(f.type), o = a
                     }
                 }), s) return y.from(s)
         }
         return n
     }
 
-    function Ao(n, e, t = 0) {
+    function vo(n, e, t = 0) {
         for (let r = e.length - 1; r >= t; r--) n = e[r].create(null, y.from(n));
         return n
     }
 
-    function vo(n, e, t, r, i) {
+    function Po(n, e, t, r, i) {
         if (i < n.length && i < e.length && n[i] == e[i]) {
-            let o = vo(n, e, t, r.lastChild, i + 1);
+            let o = Po(n, e, t, r.lastChild, i + 1);
             if (o) return r.copy(r.content.replaceChild(r.childCount - 1, o));
-            if (r.contentMatchAt(r.childCount).matchType(i == n.length - 1 ? t.type : n[i + 1])) return r.copy(r.content.append(y.from(Ao(t, n, i + 1))))
+            if (r.contentMatchAt(r.childCount).matchType(i == n.length - 1 ? t.type : n[i + 1])) return r.copy(r.content.append(y.from(vo(t, n, i + 1))))
         }
     }
 
-    function Po(n, e) {
+    function zo(n, e) {
         if (e == 0) return n;
-        let t = n.content.replaceChild(n.childCount - 1, Po(n.lastChild, e - 1)),
+        let t = n.content.replaceChild(n.childCount - 1, zo(n.lastChild, e - 1)),
             r = n.contentMatchAt(n.childCount).fillBefore(y.empty, !0);
         return n.copy(t.append(r))
     }
 
-    function hr(n, e, t, r, i, o) {
+    function pr(n, e, t, r, i, o) {
         let s = e < 0 ? n.firstChild : n.lastChild,
             l = s.content;
-        return n.childCount > 1 && (o = 0), i < r - 1 && (l = hr(l, e, t, r, i + 1, o)), i >= t && (l = e < 0 ? s.contentMatchAt(0).fillBefore(l, o <= i).append(l) : l.append(s.contentMatchAt(s.childCount).fillBefore(y.empty, !0))), n.replaceChild(e < 0 ? 0 : n.childCount - 1, s.copy(l))
+        return n.childCount > 1 && (o = 0), i < r - 1 && (l = pr(l, e, t, r, i + 1, o)), i >= t && (l = e < 0 ? s.contentMatchAt(0).fillBefore(l, o <= i).append(l) : l.append(s.contentMatchAt(s.childCount).fillBefore(y.empty, !0))), n.replaceChild(e < 0 ? 0 : n.childCount - 1, s.copy(l))
     }
 
-    function no(n, e, t) {
-        return e < n.openStart && (n = new b(hr(n.content, -1, e, n.openStart, 0, n.openEnd), e, n.openEnd)), t < n.openEnd && (n = new b(hr(n.content, 1, t, n.openEnd, 0, 0), n.openStart, t)), n
+    function ro(n, e, t) {
+        return e < n.openStart && (n = new b(pr(n.content, -1, e, n.openStart, 0, n.openEnd), e, n.openEnd)), t < n.openEnd && (n = new b(pr(n.content, 1, t, n.openEnd, 0, 0), n.openStart, t)), n
     }
-    var Ro = {
+    var Bo = {
             thead: ["table"],
             tbody: ["table"],
             tfoot: ["table"],
             caption: ["table"],
             colgroup: ["table"],
             col: ["table", "colgroup"],
             tr: ["table", "tbody"],
             td: ["table", "tbody", "tr"],
             th: ["table", "tbody", "tr"]
         },
-        ro = null;
+        io = null;
 
-    function zo() {
-        return ro || (ro = document.implementation.createHTMLDocument("title"))
+    function Fo() {
+        return io || (io = document.implementation.createHTMLDocument("title"))
     }
 
-    function Va(n) {
+    function $a(n) {
         let e = /^(\s*<meta [^>]*>)*/.exec(n);
         e && (n = n.slice(e[0].length));
-        let t = zo().createElement("div"),
+        let t = Fo().createElement("div"),
             r = /<([a-z][^>\s]+)/i.exec(n),
             i;
-        if ((i = r && Ro[r[1].toLowerCase()]) && (n = i.map(o => "<" + o + ">").join("") + n + i.map(o => "</" + o + ">").reverse().join("")), t.innerHTML = n, i)
+        if ((i = r && Bo[r[1].toLowerCase()]) && (n = i.map(o => "<" + o + ">").join("") + n + i.map(o => "</" + o + ">").reverse().join("")), t.innerHTML = n, i)
             for (let o = 0; o < i.length; o++) t = t.querySelector(i[o]) || t;
         return t
     }
 
-    function La(n) {
-        let e = n.querySelectorAll(R ? "span:not([class]):not([style])" : "span.Apple-converted-space");
+    function Ha(n) {
+        let e = n.querySelectorAll(B ? "span:not([class]):not([style])" : "span.Apple-converted-space");
         for (let t = 0; t < e.length; t++) {
             let r = e[t];
             r.childNodes.length == 1 && r.textContent == "\xA0" && r.parentNode && r.parentNode.replaceChild(n.ownerDocument.createTextNode(" "), r)
         }
     }
 
-    function Ja(n, e) {
+    function ja(n, e) {
         if (!n.size) return n;
         let t = n.content.firstChild.type.schema,
             r;
         try {
             r = JSON.parse(e)
         } catch (l) {
             return n
@@ -6644,137 +6677,137 @@
             if (!a || a.hasRequiredAttrs()) break;
             i = y.from(a.create(r[l + 1], i)), o++, s++
         }
         return new b(i, o, s)
     }
     var V = {},
         L = {},
-        Wa = {
+        _a = {
             touchstart: !0,
             touchmove: !0
         },
-        dr = class {
+        mr = class {
             constructor() {
                 this.shiftKey = !1, this.mouseDown = null, this.lastKeyCode = null, this.lastKeyCodeTime = 0, this.lastClick = {
                     time: 0,
                     x: 0,
                     y: 0,
                     type: ""
-                }, this.lastSelectionOrigin = null, this.lastSelectionTime = 0, this.lastIOSEnter = 0, this.lastIOSEnterFallbackTimeout = -1, this.lastFocus = 0, this.lastTouch = 0, this.lastAndroidDelete = 0, this.composing = !1, this.composingTimeout = -1, this.compositionNodes = [], this.compositionEndedAt = -2e8, this.compositionID = 1, this.compositionPendingChanges = 0, this.domChangeCount = 0, this.eventHandlers = Object.create(null), this.hideSelectionGuard = null
+                }, this.lastSelectionOrigin = null, this.lastSelectionTime = 0, this.lastIOSEnter = 0, this.lastIOSEnterFallbackTimeout = -1, this.lastFocus = 0, this.lastTouch = 0, this.lastAndroidDelete = 0, this.composing = !1, this.compositionNode = null, this.composingTimeout = -1, this.compositionNodes = [], this.compositionEndedAt = -2e8, this.compositionID = 1, this.compositionPendingChanges = 0, this.domChangeCount = 0, this.eventHandlers = Object.create(null), this.hideSelectionGuard = null
             }
         };
 
-    function qa(n) {
+    function Ua(n) {
         for (let e in V) {
             let t = V[e];
             n.dom.addEventListener(e, n.input.eventHandlers[e] = r => {
-                $a(n, r) && !Mr(n, r) && (n.editable || !(r.type in L)) && t(n, r)
-            }, Wa[e] ? {
+                Ya(n, r) && !Or(n, r) && (n.editable || !(r.type in L)) && t(n, r)
+            }, _a[e] ? {
                 passive: !0
             } : void 0)
         }
-        F && n.dom.addEventListener("input", () => null), ur(n)
+        F && n.dom.addEventListener("input", () => null), gr(n)
     }
 
-    function Me(n, e) {
+    function Ce(n, e) {
         n.input.lastSelectionOrigin = e, n.input.lastSelectionTime = Date.now()
     }
 
-    function Ka(n) {
+    function Ga(n) {
         n.domObserver.stop();
         for (let e in n.input.eventHandlers) n.dom.removeEventListener(e, n.input.eventHandlers[e]);
         clearTimeout(n.input.composingTimeout), clearTimeout(n.input.lastIOSEnterFallbackTimeout)
     }
 
-    function ur(n) {
+    function gr(n) {
         n.someProp("handleDOMEvents", e => {
-            for (let t in e) n.input.eventHandlers[t] || n.dom.addEventListener(t, n.input.eventHandlers[t] = r => Mr(n, r))
+            for (let t in e) n.input.eventHandlers[t] || n.dom.addEventListener(t, n.input.eventHandlers[t] = r => Or(n, r))
         })
     }
 
-    function Mr(n, e) {
+    function Or(n, e) {
         return n.someProp("handleDOMEvents", t => {
             let r = t[e.type];
             return r ? r(n, e) || e.defaultPrevented : !1
         })
     }
 
-    function $a(n, e) {
+    function Ya(n, e) {
         if (!e.bubbles) return !0;
         if (e.defaultPrevented) return !1;
         for (let t = e.target; t != n.dom; t = t.parentNode)
             if (!t || t.nodeType == 11 || t.pmViewDesc && t.pmViewDesc.stopEvent(e)) return !1;
         return !0
     }
 
-    function Ha(n, e) {
-        !Mr(n, e) && V[e.type] && (n.editable || !(e.type in L)) && V[e.type](n, e)
+    function Xa(n, e) {
+        !Or(n, e) && V[e.type] && (n.editable || !(e.type in L)) && V[e.type](n, e)
     }
     L.keydown = (n, e) => {
         let t = e;
-        if (n.input.shiftKey = t.keyCode == 16 || t.shiftKey, !Fo(n, t) && (n.input.lastKeyCode = t.keyCode, n.input.lastKeyCodeTime = Date.now(), !(Y && R && t.keyCode == 13)))
+        if (n.input.shiftKey = t.keyCode == 16 || t.shiftKey, !Lo(n, t) && (n.input.lastKeyCode = t.keyCode, n.input.lastKeyCodeTime = Date.now(), !(Y && B && t.keyCode == 13)))
             if (t.keyCode != 229 && n.domObserver.forceFlush(), tt && t.keyCode == 13 && !t.ctrlKey && !t.altKey && !t.metaKey) {
                 let r = Date.now();
                 n.input.lastIOSEnter = r, n.input.lastIOSEnterFallbackTimeout = setTimeout(() => {
-                    n.input.lastIOSEnter == r && (n.someProp("handleKeyDown", i => i(n, Fe(13, "Enter"))), n.input.lastIOSEnter = 0)
+                    n.input.lastIOSEnter == r && (n.someProp("handleKeyDown", i => i(n, Be(13, "Enter"))), n.input.lastIOSEnter = 0)
                 }, 200)
-            } else n.someProp("handleKeyDown", r => r(n, t)) || za(n, t) ? t.preventDefault() : Me(n, "key")
+            } else n.someProp("handleKeyDown", r => r(n, t)) || Wa(n, t) ? t.preventDefault() : Ce(n, "key")
     };
     L.keyup = (n, e) => {
         e.keyCode == 16 && (n.input.shiftKey = !1)
     };
     L.keypress = (n, e) => {
         let t = e;
-        if (Fo(n, t) || !t.charCode || t.ctrlKey && !t.altKey || j && t.metaKey) return;
+        if (Lo(n, t) || !t.charCode || t.ctrlKey && !t.altKey || j && t.metaKey) return;
         if (n.someProp("handleKeyPress", i => i(n, t))) {
             t.preventDefault();
             return
         }
         let r = n.state.selection;
         if (!(r instanceof w) || !r.$from.sameParent(r.$to)) {
             let i = String.fromCharCode(t.charCode);
             !/[\r\n]/.test(i) && !n.someProp("handleTextInput", o => o(n, r.$from.pos, r.$to.pos, i)) && n.dispatch(n.state.tr.insertText(i).scrollIntoView()), t.preventDefault()
         }
     };
 
-    function pn(n) {
+    function mn(n) {
         return {
             left: n.clientX,
             top: n.clientY
         }
     }
 
-    function ja(n, e) {
+    function Qa(n, e) {
         let t = e.x - n.clientX,
             r = e.y - n.clientY;
         return t * t + r * r < 100
     }
 
-    function Cr(n, e, t, r, i) {
+    function wr(n, e, t, r, i) {
         if (r == -1) return !1;
         let o = n.state.doc.resolve(r);
         for (let s = o.depth + 1; s > 0; s--)
             if (n.someProp(e, l => s > o.depth ? l(n, t, o.nodeAfter, o.before(s), i, !0) : l(n, t, o.node(s), o.before(s), i, !1))) return !0;
         return !1
     }
 
     function et(n, e, t) {
         n.focused || n.focus();
         let r = n.state.tr.setSelection(e);
         t == "pointer" && r.setMeta("pointer", !0), n.dispatch(r)
     }
 
-    function _a(n, e) {
+    function Za(n, e) {
         if (e == -1) return !1;
         let t = n.state.doc.resolve(e),
             r = t.nodeAfter;
         return r && r.isAtom && x.isSelectable(r) ? (et(n, new x(t), "pointer"), !0) : !1
     }
 
-    function Ua(n, e) {
+    function ec(n, e) {
         if (e == -1) return !1;
         let t = n.state.selection,
             r, i;
         t instanceof x && (r = t.node);
         let o = n.state.doc.resolve(e);
         for (let s = o.depth + 1; s > 0; s--) {
             let l = s > o.depth ? o.nodeAfter : o.node(s);
@@ -6782,63 +6815,63 @@
                 r && t.$from.depth > 0 && s >= t.$from.depth && o.before(t.$from.depth + 1) == t.$from.pos ? i = o.before(t.$from.depth) : i = o.before(s);
                 break
             }
         }
         return i != null ? (et(n, x.create(n.state.doc, i), "pointer"), !0) : !1
     }
 
-    function Ga(n, e, t, r, i) {
-        return Cr(n, "handleClickOn", e, t, r) || n.someProp("handleClick", o => o(n, e, r)) || (i ? Ua(n, t) : _a(n, t))
+    function tc(n, e, t, r, i) {
+        return wr(n, "handleClickOn", e, t, r) || n.someProp("handleClick", o => o(n, e, r)) || (i ? ec(n, t) : Za(n, t))
     }
 
-    function Ya(n, e, t, r) {
-        return Cr(n, "handleDoubleClickOn", e, t, r) || n.someProp("handleDoubleClick", i => i(n, e, r))
+    function nc(n, e, t, r) {
+        return wr(n, "handleDoubleClickOn", e, t, r) || n.someProp("handleDoubleClick", i => i(n, e, r))
     }
 
-    function Xa(n, e, t, r) {
-        return Cr(n, "handleTripleClickOn", e, t, r) || n.someProp("handleTripleClick", i => i(n, e, r)) || Qa(n, t, r)
+    function rc(n, e, t, r) {
+        return wr(n, "handleTripleClickOn", e, t, r) || n.someProp("handleTripleClick", i => i(n, e, r)) || ic(n, t, r)
     }
 
-    function Qa(n, e, t) {
+    function ic(n, e, t) {
         if (t.button != 0) return !1;
         let r = n.state.doc;
         if (e == -1) return r.inlineContent ? (et(n, w.create(r, 0, r.content.size), "pointer"), !0) : !1;
         let i = r.resolve(e);
         for (let o = i.depth + 1; o > 0; o--) {
             let s = o > i.depth ? i.nodeAfter : i.node(o),
                 l = i.before(o);
             if (s.inlineContent) et(n, w.create(r, l + 1, l + 1 + s.content.size), "pointer");
             else if (x.isSelectable(s)) et(n, x.create(r, l), "pointer");
             else continue;
             return !0
         }
     }
 
-    function Or(n) {
-        return ln(n)
+    function Nr(n) {
+        return an(n)
     }
-    var Bo = j ? "metaKey" : "ctrlKey";
+    var Vo = j ? "metaKey" : "ctrlKey";
     V.mousedown = (n, e) => {
         let t = e;
         n.input.shiftKey = t.shiftKey;
-        let r = Or(n),
+        let r = Nr(n),
             i = Date.now(),
             o = "singleClick";
-        i - n.input.lastClick.time < 500 && ja(t, n.input.lastClick) && !t[Bo] && (n.input.lastClick.type == "singleClick" ? o = "doubleClick" : n.input.lastClick.type == "doubleClick" && (o = "tripleClick")), n.input.lastClick = {
+        i - n.input.lastClick.time < 500 && Qa(t, n.input.lastClick) && !t[Vo] && (n.input.lastClick.type == "singleClick" ? o = "doubleClick" : n.input.lastClick.type == "doubleClick" && (o = "tripleClick")), n.input.lastClick = {
             time: i,
             x: t.clientX,
             y: t.clientY,
             type: o
         };
-        let s = n.posAtCoords(pn(t));
-        s && (o == "singleClick" ? (n.input.mouseDown && n.input.mouseDown.done(), n.input.mouseDown = new pr(n, s, t, !!r)) : (o == "doubleClick" ? Ya : Xa)(n, s.pos, s.inside, t) ? t.preventDefault() : Me(n, "pointer"))
+        let s = n.posAtCoords(mn(t));
+        s && (o == "singleClick" ? (n.input.mouseDown && n.input.mouseDown.done(), n.input.mouseDown = new yr(n, s, t, !!r)) : (o == "doubleClick" ? nc : rc)(n, s.pos, s.inside, t) ? t.preventDefault() : Ce(n, "pointer"))
     };
-    var pr = class {
+    var yr = class {
         constructor(e, t, r, i) {
-            this.view = e, this.pos = t, this.event = r, this.flushed = i, this.delayedSelectionSync = !1, this.mightDrag = null, this.startDoc = e.state.doc, this.selectNode = !!r[Bo], this.allowDefault = r.shiftKey;
+            this.view = e, this.pos = t, this.event = r, this.flushed = i, this.delayedSelectionSync = !1, this.mightDrag = null, this.startDoc = e.state.doc, this.selectNode = !!r[Vo], this.allowDefault = r.shiftKey;
             let o, s;
             if (t.inside > -1) o = e.state.doc.nodeAt(t.inside), s = t.inside;
             else {
                 let f = e.state.doc.resolve(t.pos);
                 o = f.parent, s = f.depth ? f.before() : 0
             }
             let l = i ? null : r.target,
@@ -6850,207 +6883,223 @@
             (r.button == 0 && o.type.spec.draggable && o.type.spec.selectable !== !1 || c instanceof x && c.from <= s && c.to > s) && (this.mightDrag = {
                 node: o,
                 pos: s,
                 addAttr: !!(this.target && !this.target.draggable),
                 setUneditable: !!(this.target && X && !this.target.hasAttribute("contentEditable"))
             }), this.target && this.mightDrag && (this.mightDrag.addAttr || this.mightDrag.setUneditable) && (this.view.domObserver.stop(), this.mightDrag.addAttr && (this.target.draggable = !0), this.mightDrag.setUneditable && setTimeout(() => {
                 this.view.input.mouseDown == this && this.target.setAttribute("contentEditable", "false")
-            }, 20), this.view.domObserver.start()), e.root.addEventListener("mouseup", this.up = this.up.bind(this)), e.root.addEventListener("mousemove", this.move = this.move.bind(this)), Me(e, "pointer")
+            }, 20), this.view.domObserver.start()), e.root.addEventListener("mouseup", this.up = this.up.bind(this)), e.root.addEventListener("mousemove", this.move = this.move.bind(this)), Ce(e, "pointer")
         }
         done() {
             this.view.root.removeEventListener("mouseup", this.up), this.view.root.removeEventListener("mousemove", this.move), this.mightDrag && this.target && (this.view.domObserver.stop(), this.mightDrag.addAttr && this.target.removeAttribute("draggable"), this.mightDrag.setUneditable && this.target.removeAttribute("contentEditable"), this.view.domObserver.start()), this.delayedSelectionSync && setTimeout(() => ge(this.view)), this.view.input.mouseDown = null
         }
         up(e) {
             if (this.done(), !this.view.dom.contains(e.target)) return;
             let t = this.pos;
-            this.view.state.doc != this.startDoc && (t = this.view.posAtCoords(pn(e))), this.updateAllowDefault(e), this.allowDefault || !t ? Me(this.view, "pointer") : Ga(this.view, t.pos, t.inside, e, this.selectNode) ? e.preventDefault() : e.button == 0 && (this.flushed || F && this.mightDrag && !this.mightDrag.node.isAtom || R && !this.view.state.selection.visible && Math.min(Math.abs(t.pos - this.view.state.selection.from), Math.abs(t.pos - this.view.state.selection.to)) <= 2) ? (et(this.view, S.near(this.view.state.doc.resolve(t.pos)), "pointer"), e.preventDefault()) : Me(this.view, "pointer")
+            this.view.state.doc != this.startDoc && (t = this.view.posAtCoords(mn(e))), this.updateAllowDefault(e), this.allowDefault || !t ? Ce(this.view, "pointer") : tc(this.view, t.pos, t.inside, e, this.selectNode) ? e.preventDefault() : e.button == 0 && (this.flushed || F && this.mightDrag && !this.mightDrag.node.isAtom || B && !this.view.state.selection.visible && Math.min(Math.abs(t.pos - this.view.state.selection.from), Math.abs(t.pos - this.view.state.selection.to)) <= 2) ? (et(this.view, S.near(this.view.state.doc.resolve(t.pos)), "pointer"), e.preventDefault()) : Ce(this.view, "pointer")
         }
         move(e) {
-            this.updateAllowDefault(e), Me(this.view, "pointer"), e.buttons == 0 && this.done()
+            this.updateAllowDefault(e), Ce(this.view, "pointer"), e.buttons == 0 && this.done()
         }
         updateAllowDefault(e) {
             !this.allowDefault && (Math.abs(this.event.x - e.clientX) > 4 || Math.abs(this.event.y - e.clientY) > 4) && (this.allowDefault = !0)
         }
     };
     V.touchstart = n => {
-        n.input.lastTouch = Date.now(), Or(n), Me(n, "pointer")
+        n.input.lastTouch = Date.now(), Nr(n), Ce(n, "pointer")
     };
     V.touchmove = n => {
-        n.input.lastTouch = Date.now(), Me(n, "pointer")
+        n.input.lastTouch = Date.now(), Ce(n, "pointer")
     };
-    V.contextmenu = n => Or(n);
+    V.contextmenu = n => Nr(n);
 
-    function Fo(n, e) {
+    function Lo(n, e) {
         return n.composing ? !0 : F && Math.abs(e.timeStamp - n.input.compositionEndedAt) < 500 ? (n.input.compositionEndedAt = -2e8, !0) : !1
     }
-    var Za = Y ? 5e3 : -1;
+    var oc = Y ? 5e3 : -1;
     L.compositionstart = L.compositionupdate = n => {
         if (!n.composing) {
             n.domObserver.flush();
             let {
                 state: e
             } = n, t = e.selection.$from;
-            if (e.selection.empty && (e.storedMarks || !t.textOffset && t.parentOffset && t.nodeBefore.marks.some(r => r.type.spec.inclusive === !1))) n.markCursor = n.state.storedMarks || t.marks(), ln(n, !0), n.markCursor = null;
-            else if (ln(n), X && e.selection.empty && t.parentOffset && !t.textOffset && t.nodeBefore.marks.length) {
+            if (e.selection.empty && (e.storedMarks || !t.textOffset && t.parentOffset && t.nodeBefore.marks.some(r => r.type.spec.inclusive === !1))) n.markCursor = n.state.storedMarks || t.marks(), an(n, !0), n.markCursor = null;
+            else if (an(n), X && e.selection.empty && t.parentOffset && !t.textOffset && t.nodeBefore.marks.length) {
                 let r = n.domSelectionRange();
                 for (let i = r.focusNode, o = r.focusOffset; i && i.nodeType == 1 && o != 0;) {
                     let s = o < 0 ? i.lastChild : i.childNodes[o - 1];
                     if (!s) break;
                     if (s.nodeType == 3) {
                         n.domSelection().collapse(s, s.nodeValue.length);
                         break
                     } else i = s, o = -1
                 }
             }
             n.input.composing = !0
         }
-        Vo(n, Za)
+        Jo(n, oc)
     };
     L.compositionend = (n, e) => {
-        n.composing && (n.input.composing = !1, n.input.compositionEndedAt = e.timeStamp, n.input.compositionPendingChanges = n.domObserver.pendingRecords().length ? n.input.compositionID : 0, n.input.compositionPendingChanges && Promise.resolve().then(() => n.domObserver.flush()), n.input.compositionID++, Vo(n, 20))
+        n.composing && (n.input.composing = !1, n.input.compositionEndedAt = e.timeStamp, n.input.compositionPendingChanges = n.domObserver.pendingRecords().length ? n.input.compositionID : 0, n.input.compositionNode = null, n.input.compositionPendingChanges && Promise.resolve().then(() => n.domObserver.flush()), n.input.compositionID++, Jo(n, 20))
     };
 
-    function Vo(n, e) {
-        clearTimeout(n.input.composingTimeout), e > -1 && (n.input.composingTimeout = setTimeout(() => ln(n), e))
+    function Jo(n, e) {
+        clearTimeout(n.input.composingTimeout), e > -1 && (n.input.composingTimeout = setTimeout(() => an(n), e))
+    }
+
+    function Wo(n) {
+        for (n.composing && (n.input.composing = !1, n.input.compositionEndedAt = lc()); n.input.compositionNodes.length > 0;) n.input.compositionNodes.pop().markParentsDirty()
     }
 
-    function Lo(n) {
-        for (n.composing && (n.input.composing = !1, n.input.compositionEndedAt = ec()); n.input.compositionNodes.length > 0;) n.input.compositionNodes.pop().markParentsDirty()
+    function sc(n) {
+        let e = n.domSelectionRange();
+        if (!e.focusNode) return null;
+        let t = ra(e.focusNode, e.focusOffset),
+            r = ia(e.focusNode, e.focusOffset);
+        if (t && r && t != r) {
+            let i = r.pmViewDesc;
+            if (!i || !i.isText(r.nodeValue)) return r;
+            if (n.input.compositionNode == r) {
+                let o = t.pmViewDesc;
+                if (!(!o || !o.isText(t.nodeValue))) return r
+            }
+        }
+        return t
     }
 
-    function ec() {
+    function lc() {
         let n = document.createEvent("Event");
         return n.initEvent("event", !0, !0), n.timeStamp
     }
 
-    function ln(n, e = !1) {
+    function an(n, e = !1) {
         if (!(Y && n.domObserver.flushingSoon >= 0)) {
-            if (n.domObserver.forceFlush(), Lo(n), e || n.docView && n.docView.dirty) {
-                let t = xr(n);
+            if (n.domObserver.forceFlush(), Wo(n), e || n.docView && n.docView.dirty) {
+                let t = Mr(n);
                 return t && !t.eq(n.state.selection) ? n.dispatch(n.state.tr.setSelection(t)) : n.updateState(n.state), !0
             }
             return !1
         }
     }
 
-    function tc(n, e) {
+    function ac(n, e) {
         if (!n.dom.parentNode) return;
         let t = n.dom.parentNode.appendChild(document.createElement("div"));
         t.appendChild(e), t.style.cssText = "position: fixed; left: -10000px; top: 10px";
         let r = getSelection(),
             i = document.createRange();
         i.selectNodeContents(e), n.dom.blur(), r.removeAllRanges(), r.addRange(i), setTimeout(() => {
             t.parentNode && t.parentNode.removeChild(t), n.focus()
         }, 50)
     }
-    var wt = W && Ce < 15 || tt && ra < 604;
+    var wt = W && Oe < 15 || tt && fa < 604;
     V.copy = L.cut = (n, e) => {
         let t = e,
             r = n.state.selection,
             i = t.type == "cut";
         if (r.empty) return;
         let o = wt ? null : t.clipboardData,
             s = r.content(),
             {
                 dom: l,
                 text: a
-            } = Eo(n, s);
-        o ? (t.preventDefault(), o.clearData(), o.setData("text/html", l.innerHTML), o.setData("text/plain", a)) : tc(n, l), i && n.dispatch(n.state.tr.deleteSelection().scrollIntoView().setMeta("uiEvent", "cut"))
+            } = Io(n, s);
+        o ? (t.preventDefault(), o.clearData(), o.setData("text/html", l.innerHTML), o.setData("text/plain", a)) : ac(n, l), i && n.dispatch(n.state.tr.deleteSelection().scrollIntoView().setMeta("uiEvent", "cut"))
     };
 
-    function nc(n) {
+    function cc(n) {
         return n.openStart == 0 && n.openEnd == 0 && n.content.childCount == 1 ? n.content.firstChild : null
     }
 
-    function rc(n, e) {
+    function fc(n, e) {
         if (!n.dom.parentNode) return;
         let t = n.input.shiftKey || n.state.selection.$from.parent.type.spec.code,
             r = n.dom.parentNode.appendChild(document.createElement(t ? "textarea" : "div"));
         t || (r.contentEditable = "true"), r.style.cssText = "position: fixed; left: -10000px; top: 10px", r.focus();
         let i = n.input.shiftKey && n.input.lastKeyCode != 45;
         setTimeout(() => {
             n.focus(), r.parentNode && r.parentNode.removeChild(r), t ? Nt(n, r.value, null, i, e) : Nt(n, r.textContent, r.innerHTML, i, e)
         }, 50)
     }
 
     function Nt(n, e, t, r, i) {
-        let o = Io(n, e, t, r, n.state.selection.$from);
+        let o = Ro(n, e, t, r, n.state.selection.$from);
         if (n.someProp("handlePaste", a => a(n, i, o || b.empty))) return !0;
         if (!o) return !1;
-        let s = nc(o),
+        let s = cc(o),
             l = s ? n.state.tr.replaceSelectionWith(s, r) : n.state.tr.replaceSelection(o);
         return n.dispatch(l.scrollIntoView().setMeta("paste", !0).setMeta("uiEvent", "paste")), !0
     }
 
-    function Jo(n) {
+    function qo(n) {
         let e = n.getData("text/plain") || n.getData("Text");
         if (e) return e;
         let t = n.getData("text/uri-list");
         return t ? t.replace(/\r?\n/g, " ") : ""
     }
     L.paste = (n, e) => {
         let t = e;
         if (n.composing && !Y) return;
         let r = wt ? null : t.clipboardData,
             i = n.input.shiftKey && n.input.lastKeyCode != 45;
-        r && Nt(n, Jo(r), r.getData("text/html"), i, t) ? t.preventDefault() : rc(n, t)
+        r && Nt(n, qo(r), r.getData("text/html"), i, t) ? t.preventDefault() : fc(n, t)
     };
-    var an = class {
+    var cn = class {
             constructor(e, t, r) {
                 this.slice = e, this.move = t, this.node = r
             }
         },
-        Wo = j ? "altKey" : "ctrlKey";
+        Ko = j ? "altKey" : "ctrlKey";
     V.dragstart = (n, e) => {
         let t = e,
             r = n.input.mouseDown;
         if (r && r.done(), !t.dataTransfer) return;
         let i = n.state.selection,
-            o = i.empty ? null : n.posAtCoords(pn(t)),
+            o = i.empty ? null : n.posAtCoords(mn(t)),
             s;
         if (!(o && o.pos >= i.from && o.pos <= (i instanceof x ? i.to - 1 : i.to))) {
             if (r && r.mightDrag) s = x.create(n.state.doc, r.mightDrag.pos);
             else if (t.target && t.target.nodeType == 1) {
                 let f = n.docView.nearestDesc(t.target, !0);
                 f && f.node.type.spec.draggable && f != n.docView && (s = x.create(n.state.doc, f.posBefore))
             }
         }
         let l = (s || n.state.selection).content(),
             {
                 dom: a,
                 text: c
-            } = Eo(n, l);
-        t.dataTransfer.clearData(), t.dataTransfer.setData(wt ? "Text" : "text/html", a.innerHTML), t.dataTransfer.effectAllowed = "copyMove", wt || t.dataTransfer.setData("text/plain", c), n.dragging = new an(l, !t[Wo], s)
+            } = Io(n, l);
+        t.dataTransfer.clearData(), t.dataTransfer.setData(wt ? "Text" : "text/html", a.innerHTML), t.dataTransfer.effectAllowed = "copyMove", wt || t.dataTransfer.setData("text/plain", c), n.dragging = new cn(l, !t[Ko], s)
     };
     V.dragend = n => {
         let e = n.dragging;
         window.setTimeout(() => {
             n.dragging == e && (n.dragging = null)
         }, 50)
     };
     L.dragover = L.dragenter = (n, e) => e.preventDefault();
     L.drop = (n, e) => {
         let t = e,
             r = n.dragging;
         if (n.dragging = null, !t.dataTransfer) return;
-        let i = n.posAtCoords(pn(t));
+        let i = n.posAtCoords(mn(t));
         if (!i) return;
         let o = n.state.doc.resolve(i.pos),
             s = r && r.slice;
         s ? n.someProp("transformPasted", u => {
             s = u(s, n)
-        }) : s = Io(n, Jo(t.dataTransfer), wt ? null : t.dataTransfer.getData("text/html"), !1, o);
-        let l = !!(r && !t[Wo]);
+        }) : s = Ro(n, qo(t.dataTransfer), wt ? null : t.dataTransfer.getData("text/html"), !1, o);
+        let l = !!(r && !t[Ko]);
         if (n.someProp("handleDrop", u => u(n, t, s || b.empty, l))) {
             t.preventDefault();
             return
         }
         if (!s) return;
         t.preventDefault();
-        let a = s ? Ut(n.state.doc, o.pos, s) : o.pos;
+        let a = s ? Gt(n.state.doc, o.pos, s) : o.pos;
         a == null && (a = o.pos);
         let c = n.state.tr;
         if (l) {
             let {
                 node: u
             } = r;
             u ? u.replace(c) : c.deleteSelection()
@@ -7059,35 +7108,35 @@
             h = s.openStart == 0 && s.openEnd == 0 && s.content.childCount == 1,
             d = c.doc;
         if (h ? c.replaceRangeWith(f, f, s.content.firstChild) : c.replaceRange(f, f, s), c.doc.eq(d)) return;
         let p = c.doc.resolve(f);
         if (h && x.isSelectable(s.content.firstChild) && p.nodeAfter && p.nodeAfter.sameMarkup(s.content.firstChild)) c.setSelection(new x(p));
         else {
             let u = c.mapping.map(a);
-            c.mapping.maps[c.mapping.maps.length - 1].forEach((m, g, k, N) => u = N), c.setSelection(Sr(n, p, c.doc.resolve(u)))
+            c.mapping.maps[c.mapping.maps.length - 1].forEach((m, g, k, N) => u = N), c.setSelection(Cr(n, p, c.doc.resolve(u)))
         }
         n.focus(), n.dispatch(c.setMeta("uiEvent", "drop"))
     };
     V.focus = n => {
         n.input.lastFocus = Date.now(), n.focused || (n.domObserver.stop(), n.dom.classList.add("ProseMirror-focused"), n.domObserver.start(), n.focused = !0, setTimeout(() => {
             n.docView && n.hasFocus() && !n.domObserver.currentSelection.eq(n.domSelectionRange()) && ge(n)
         }, 20))
     };
     V.blur = (n, e) => {
         let t = e;
         n.focused && (n.domObserver.stop(), n.dom.classList.remove("ProseMirror-focused"), n.domObserver.start(), t.relatedTarget && n.dom.contains(t.relatedTarget) && n.domObserver.currentSelection.clear(), n.focused = !1)
     };
     V.beforeinput = (n, e) => {
-        if (R && Y && e.inputType == "deleteContentBackward") {
+        if (B && Y && e.inputType == "deleteContentBackward") {
             n.domObserver.flushSoon();
             let {
                 domChangeCount: r
             } = n.input;
             setTimeout(() => {
-                if (n.input.domChangeCount != r || (n.dom.blur(), n.focus(), n.someProp("handleKeyDown", o => o(n, Fe(8, "Backspace"))))) return;
+                if (n.input.domChangeCount != r || (n.dom.blur(), n.focus(), n.someProp("handleKeyDown", o => o(n, Be(8, "Backspace"))))) return;
                 let {
                     $cursor: i
                 } = n.state.selection;
                 i && i.pos > 0 && n.dispatch(n.state.tr.delete(i.pos - 1, i.pos).scrollIntoView())
             }, 50)
         }
     };
@@ -7097,17 +7146,17 @@
         if (n == e) return !0;
         for (let t in n)
             if (n[t] !== e[t]) return !1;
         for (let t in e)
             if (!(t in n)) return !1;
         return !0
     }
-    var cn = class n {
+    var fn = class n {
             constructor(e, t) {
-                this.toDOM = e, this.spec = t || We, this.side = this.spec.side || 0
+                this.toDOM = e, this.spec = t || Je, this.side = this.spec.side || 0
             }
             map(e, t, r, i) {
                 let {
                     pos: o,
                     deleted: s
                 } = e.mapResult(t.from + i, this.side < 0 ? -1 : 1);
                 return s ? null : new ae(o - r, o - r, this)
@@ -7118,17 +7167,17 @@
             eq(e) {
                 return this == e || e instanceof n && (this.spec.key && this.spec.key == e.spec.key || this.toDOM == e.toDOM && Tt(this.spec, e.spec))
             }
             destroy(e) {
                 this.spec.destroy && this.spec.destroy(e)
             }
         },
-        Je = class n {
+        Le = class n {
             constructor(e, t) {
-                this.attrs = e, this.spec = t || We
+                this.attrs = e, this.spec = t || Je
             }
             map(e, t, r, i) {
                 let o = e.map(t.from + i, this.spec.inclusiveStart ? -1 : 1) - r,
                     s = e.map(t.to + i, this.spec.inclusiveEnd ? 1 : -1) - r;
                 return o >= s ? null : new ae(o, s, this)
             }
             valid(e, t) {
@@ -7138,17 +7187,17 @@
                 return this == e || e instanceof n && Tt(this.attrs, e.attrs) && Tt(this.spec, e.spec)
             }
             static is(e) {
                 return e.type instanceof n
             }
             destroy() {}
         },
-        mr = class n {
+        br = class n {
             constructor(e, t) {
-                this.attrs = e, this.spec = t || We
+                this.attrs = e, this.spec = t || Je
             }
             map(e, t, r, i) {
                 let o = e.mapResult(t.from + i, 1);
                 if (o.deleted) return null;
                 let s = e.mapResult(t.to + i, -1);
                 return s.deleted || s.pos <= o.pos ? null : new ae(o.pos - r, s.pos - r, this)
             }
@@ -7174,40 +7223,40 @@
             eq(e, t = 0) {
                 return this.type.eq(e.type) && this.from + t == e.from && this.to + t == e.to
             }
             map(e, t, r) {
                 return this.type.map(e, this, t, r)
             }
             static widget(e, t, r) {
-                return new n(e, e, new cn(t, r))
+                return new n(e, e, new fn(t, r))
             }
             static inline(e, t, r, i) {
-                return new n(e, t, new Je(r, i))
+                return new n(e, t, new Le(r, i))
             }
             static node(e, t, r, i) {
-                return new n(e, t, new mr(r, i))
+                return new n(e, t, new br(r, i))
             }
             get spec() {
                 return this.type.spec
             }
             get inline() {
-                return this.type instanceof Je
+                return this.type instanceof Le
             }
             get widget() {
-                return this.type instanceof cn
+                return this.type instanceof fn
             }
         },
         Ze = [],
-        We = {},
+        Je = {},
         q = class n {
             constructor(e, t) {
                 this.local = e.length ? e : Ze, this.children = t.length ? t : Ze
             }
             static create(e, t) {
-                return t.length ? hn(t, e, 0, We) : P
+                return t.length ? dn(t, e, 0, Je) : P
             }
             find(e, t, r) {
                 let i = [];
                 return this.findInner(e == null ? 0 : e, t == null ? 1e9 : t, i, 0, r), i
             }
             findInner(e, t, r, i, o) {
                 for (let s = 0; s < this.local.length; s++) {
@@ -7217,40 +7266,40 @@
                 for (let s = 0; s < this.children.length; s += 3)
                     if (this.children[s] < t && this.children[s + 1] > e) {
                         let l = this.children[s] + 1;
                         this.children[s + 2].findInner(e - l, t - l, r, i + l, o)
                     }
             }
             map(e, t, r) {
-                return this == P || e.maps.length == 0 ? this : this.mapInner(e, t, 0, 0, r || We)
+                return this == P || e.maps.length == 0 ? this : this.mapInner(e, t, 0, 0, r || Je)
             }
             mapInner(e, t, r, i, o) {
                 let s;
                 for (let l = 0; l < this.local.length; l++) {
                     let a = this.local[l].map(e, r, i);
                     a && a.type.valid(t, a) ? (s || (s = [])).push(a) : o.onRemove && o.onRemove(this.local[l].spec)
                 }
-                return this.children.length ? ic(this.children, s || [], e, t, r, i, o) : s ? new n(s.sort(qe), Ze) : P
+                return this.children.length ? hc(this.children, s || [], e, t, r, i, o) : s ? new n(s.sort(We), Ze) : P
             }
             add(e, t) {
                 return t.length ? this == P ? n.create(e, t) : this.addInner(e, t, 0) : this
             }
             addInner(e, t, r) {
                 let i, o = 0;
                 e.forEach((l, a) => {
                     let c = a + r,
                         f;
-                    if (f = Ko(t, l, c)) {
+                    if (f = Ho(t, l, c)) {
                         for (i || (i = this.children.slice()); o < i.length && i[o] < a;) o += 3;
-                        i[o] == a ? i[o + 2] = i[o + 2].addInner(l, f, c + 1) : i.splice(o, 0, a, a + l.nodeSize, hn(f, l, c + 1, We)), o += 3
+                        i[o] == a ? i[o + 2] = i[o + 2].addInner(l, f, c + 1) : i.splice(o, 0, a, a + l.nodeSize, dn(f, l, c + 1, Je)), o += 3
                     }
                 });
-                let s = qo(o ? $o(t) : t, -r);
+                let s = $o(o ? jo(t) : t, -r);
                 for (let l = 0; l < s.length; l++) s[l].type.valid(e, s[l]) || s.splice(l--, 1);
-                return new n(s.length ? this.local.concat(s).sort(qe) : this.local, i || this.children)
+                return new n(s.length ? this.local.concat(s).sort(We) : this.local, i || this.children)
             }
             remove(e) {
                 return e.length == 0 || this == P ? this : this.removeInner(e, 0)
             }
             removeInner(e, t) {
                 let r = this.children,
                     i = this.local;
@@ -7278,55 +7327,55 @@
                     if (this.children[l] >= e) {
                         this.children[l] == e && (r = this.children[l + 2]);
                         break
                     } let o = e + 1,
                     s = o + t.content.size;
                 for (let l = 0; l < this.local.length; l++) {
                     let a = this.local[l];
-                    if (a.from < s && a.to > o && a.type instanceof Je) {
+                    if (a.from < s && a.to > o && a.type instanceof Le) {
                         let c = Math.max(o, a.from) - o,
                             f = Math.min(s, a.to) - o;
                         c < f && (i || (i = [])).push(a.copy(c, f))
                     }
                 }
                 if (i) {
-                    let l = new n(i.sort(qe), Ze);
-                    return r ? new fn([l, r]) : l
+                    let l = new n(i.sort(We), Ze);
+                    return r ? new hn([l, r]) : l
                 }
                 return r || P
             }
             eq(e) {
                 if (this == e) return !0;
                 if (!(e instanceof n) || this.local.length != e.local.length || this.children.length != e.children.length) return !1;
                 for (let t = 0; t < this.local.length; t++)
                     if (!this.local[t].eq(e.local[t])) return !1;
                 for (let t = 0; t < this.children.length; t += 3)
                     if (this.children[t] != e.children[t] || this.children[t + 1] != e.children[t + 1] || !this.children[t + 2].eq(e.children[t + 2])) return !1;
                 return !0
             }
             locals(e) {
-                return wr(this.localsInner(e))
+                return Tr(this.localsInner(e))
             }
             localsInner(e) {
                 if (this == P) return Ze;
-                if (e.inlineContent || !this.local.some(Je.is)) return this.local;
+                if (e.inlineContent || !this.local.some(Le.is)) return this.local;
                 let t = [];
-                for (let r = 0; r < this.local.length; r++) this.local[r].type instanceof Je || t.push(this.local[r]);
+                for (let r = 0; r < this.local.length; r++) this.local[r].type instanceof Le || t.push(this.local[r]);
                 return t
             }
         };
     q.empty = new q([], []);
-    q.removeOverlap = wr;
+    q.removeOverlap = Tr;
     var P = q.empty,
-        fn = class n {
+        hn = class n {
             constructor(e) {
                 this.members = e
             }
             map(e, t) {
-                let r = this.members.map(i => i.map(e, t, We));
+                let r = this.members.map(i => i.map(e, t, Je));
                 return n.from(r)
             }
             forChild(e, t) {
                 if (t.isLeaf) return q.empty;
                 let r = [];
                 for (let i = 0; i < this.members.length; i++) {
                     let o = this.members[i].forChild(e, t);
@@ -7347,29 +7396,29 @@
                     if (o.length)
                         if (!t) t = o;
                         else {
                             r && (t = t.slice(), r = !1);
                             for (let s = 0; s < o.length; s++) t.push(o[s])
                         }
                 }
-                return t ? wr(r ? t : t.sort(qe)) : Ze
+                return t ? Tr(r ? t : t.sort(We)) : Ze
             }
             static from(e) {
                 switch (e.length) {
                     case 0:
                         return P;
                     case 1:
                         return e[0];
                     default:
                         return new n(e.every(t => t instanceof q) ? e : e.reduce((t, r) => t.concat(r instanceof q ? r : r.members), []))
                 }
             }
         };
 
-    function ic(n, e, t, r, i, o, s) {
+    function hc(n, e, t, r, i, o, s) {
         let l = n.slice();
         for (let c = 0, f = o; c < t.maps.length; c++) {
             let h = 0;
             t.maps[c].forEach((d, p, u, m) => {
                 let g = m - u - (p - d);
                 for (let k = 0; k < l.length; k += 3) {
                     let N = l[k + 1];
@@ -7401,143 +7450,143 @@
                     } = r.content.findIndex(h),
                     g = r.maybeChild(u);
                 if (g && m == h && m + g.nodeSize == p) {
                     let k = l[c + 2].mapInner(t, g, f + 1, n[c] + o + 1, s);
                     k != P ? (l[c] = h, l[c + 1] = p, l[c + 2] = k) : (l[c + 1] = -2, a = !0)
                 } else a = !0
             } if (a) {
-            let c = oc(l, n, e, t, i, o, s),
-                f = hn(c, r, 0, s);
+            let c = dc(l, n, e, t, i, o, s),
+                f = dn(c, r, 0, s);
             e = f.local;
             for (let h = 0; h < l.length; h += 3) l[h + 1] < 0 && (l.splice(h, 3), h -= 3);
             for (let h = 0, d = 0; h < f.children.length; h += 3) {
                 let p = f.children[h];
                 for (; d < l.length && l[d] < p;) d += 3;
                 l.splice(d, 0, f.children[h], f.children[h + 1], f.children[h + 2])
             }
         }
-        return new q(e.sort(qe), l)
+        return new q(e.sort(We), l)
     }
 
-    function qo(n, e) {
+    function $o(n, e) {
         if (!e || !n.length) return n;
         let t = [];
         for (let r = 0; r < n.length; r++) {
             let i = n[r];
             t.push(new ae(i.from + e, i.to + e, i.type))
         }
         return t
     }
 
-    function oc(n, e, t, r, i, o, s) {
+    function dc(n, e, t, r, i, o, s) {
         function l(a, c) {
             for (let f = 0; f < a.local.length; f++) {
                 let h = a.local[f].map(r, i, c);
                 h ? t.push(h) : s.onRemove && s.onRemove(a.local[f].spec)
             }
             for (let f = 0; f < a.children.length; f += 3) l(a.children[f + 2], a.children[f] + c + 1)
         }
         for (let a = 0; a < n.length; a += 3) n[a + 1] == -1 && l(n[a + 2], e[a] + o + 1);
         return t
     }
 
-    function Ko(n, e, t) {
+    function Ho(n, e, t) {
         if (e.isLeaf) return null;
         let r = t + e.nodeSize,
             i = null;
         for (let o = 0, s; o < n.length; o++)(s = n[o]) && s.from > t && s.to < r && ((i || (i = [])).push(s), n[o] = null);
         return i
     }
 
-    function $o(n) {
+    function jo(n) {
         let e = [];
         for (let t = 0; t < n.length; t++) n[t] != null && e.push(n[t]);
         return e
     }
 
-    function hn(n, e, t, r) {
+    function dn(n, e, t, r) {
         let i = [],
             o = !1;
         e.forEach((l, a) => {
-            let c = Ko(n, l, a + t);
+            let c = Ho(n, l, a + t);
             if (c) {
                 o = !0;
-                let f = hn(c, l, t + a + 1, r);
+                let f = dn(c, l, t + a + 1, r);
                 f != P && i.push(a, a + l.nodeSize, f)
             }
         });
-        let s = qo(o ? $o(n) : n, -t).sort(qe);
+        let s = $o(o ? jo(n) : n, -t).sort(We);
         for (let l = 0; l < s.length; l++) s[l].type.valid(e, s[l]) || (r.onRemove && r.onRemove(s[l].spec), s.splice(l--, 1));
         return s.length || i.length ? new q(s, i) : P
     }
 
-    function qe(n, e) {
+    function We(n, e) {
         return n.from - e.from || n.to - e.to
     }
 
-    function wr(n) {
+    function Tr(n) {
         let e = n;
         for (let t = 0; t < e.length - 1; t++) {
             let r = e[t];
             if (r.from != r.to)
                 for (let i = t + 1; i < e.length; i++) {
                     let o = e[i];
                     if (o.from == r.from) {
-                        o.to != r.to && (e == n && (e = n.slice()), e[i] = o.copy(o.from, r.to), io(e, i + 1, o.copy(r.to, o.to)));
+                        o.to != r.to && (e == n && (e = n.slice()), e[i] = o.copy(o.from, r.to), oo(e, i + 1, o.copy(r.to, o.to)));
                         continue
                     } else {
-                        o.from < r.to && (e == n && (e = n.slice()), e[t] = r.copy(r.from, o.from), io(e, i, r.copy(o.from, r.to)));
+                        o.from < r.to && (e == n && (e = n.slice()), e[t] = r.copy(r.from, o.from), oo(e, i, r.copy(o.from, r.to)));
                         break
                     }
                 }
         }
         return e
     }
 
-    function io(n, e, t) {
-        for (; e < n.length && qe(t, n[e]) > 0;) e++;
+    function oo(n, e, t) {
+        for (; e < n.length && We(t, n[e]) > 0;) e++;
         n.splice(e, 0, t)
     }
 
-    function Qn(n) {
+    function er(n) {
         let e = [];
         return n.someProp("decorations", t => {
             let r = t(n.state);
             r && r != P && e.push(r)
-        }), n.cursorWrapper && e.push(q.create(n.state.doc, [n.cursorWrapper.deco])), fn.from(e)
+        }), n.cursorWrapper && e.push(q.create(n.state.doc, [n.cursorWrapper.deco])), hn.from(e)
     }
-    var sc = {
+    var uc = {
             childList: !0,
             characterData: !0,
             characterDataOldValue: !0,
             attributes: !0,
             attributeOldValue: !0,
             subtree: !0
         },
-        lc = W && Ce <= 11,
-        gr = class {
+        pc = W && Oe <= 11,
+        kr = class {
             constructor() {
                 this.anchorNode = null, this.anchorOffset = 0, this.focusNode = null, this.focusOffset = 0
             }
             set(e) {
                 this.anchorNode = e.anchorNode, this.anchorOffset = e.anchorOffset, this.focusNode = e.focusNode, this.focusOffset = e.focusOffset
             }
             clear() {
                 this.anchorNode = this.focusNode = null
             }
             eq(e) {
                 return e.anchorNode == this.anchorNode && e.anchorOffset == this.anchorOffset && e.focusNode == this.focusNode && e.focusOffset == this.focusOffset
             }
         },
-        yr = class {
+        xr = class {
             constructor(e, t) {
-                this.view = e, this.handleDOMChange = t, this.queue = [], this.flushingSoon = -1, this.observer = null, this.currentSelection = new gr, this.onCharData = null, this.suppressingSelectionUpdates = !1, this.observer = window.MutationObserver && new window.MutationObserver(r => {
+                this.view = e, this.handleDOMChange = t, this.queue = [], this.flushingSoon = -1, this.observer = null, this.currentSelection = new kr, this.onCharData = null, this.suppressingSelectionUpdates = !1, this.observer = window.MutationObserver && new window.MutationObserver(r => {
                     for (let i = 0; i < r.length; i++) this.queue.push(r[i]);
-                    W && Ce <= 11 && r.some(i => i.type == "childList" && i.removedNodes.length || i.type == "characterData" && i.oldValue.length > i.target.nodeValue.length) ? this.flushSoon() : this.flush()
-                }), lc && (this.onCharData = r => {
+                    W && Oe <= 11 && r.some(i => i.type == "childList" && i.removedNodes.length || i.type == "characterData" && i.oldValue.length > i.target.nodeValue.length) ? this.flushSoon() : this.flush()
+                }), pc && (this.onCharData = r => {
                     this.queue.push({
                         target: r.target,
                         type: "characterData",
                         oldValue: r.prevValue
                     }), this.flushSoon()
                 }), this.onSelectionChange = this.onSelectionChange.bind(this)
             }
@@ -7546,15 +7595,15 @@
                     this.flushingSoon = -1, this.flush()
                 }, 20))
             }
             forceFlush() {
                 this.flushingSoon > -1 && (window.clearTimeout(this.flushingSoon), this.flushingSoon = -1, this.flush())
             }
             start() {
-                this.observer && (this.observer.takeRecords(), this.observer.observe(this.view.dom, sc)), this.onCharData && this.view.dom.addEventListener("DOMCharacterDataModified", this.onCharData), this.connectSelection()
+                this.observer && (this.observer.takeRecords(), this.observer.observe(this.view.dom, uc)), this.onCharData && this.view.dom.addEventListener("DOMCharacterDataModified", this.onCharData), this.connectSelection()
             }
             stop() {
                 if (this.observer) {
                     let e = this.observer.takeRecords();
                     if (e.length) {
                         for (let t = 0; t < e.length; t++) this.queue.push(e[t]);
                         window.setTimeout(() => this.flush(), 20)
@@ -7569,19 +7618,19 @@
             disconnectSelection() {
                 this.view.dom.ownerDocument.removeEventListener("selectionchange", this.onSelectionChange)
             }
             suppressSelectionUpdates() {
                 this.suppressingSelectionUpdates = !0, setTimeout(() => this.suppressingSelectionUpdates = !1, 50)
             }
             onSelectionChange() {
-                if (Yi(this.view)) {
+                if (Xi(this.view)) {
                     if (this.suppressingSelectionUpdates) return ge(this.view);
-                    if (W && Ce <= 11 && !this.view.state.selection.empty) {
+                    if (W && Oe <= 11 && !this.view.state.selection.empty) {
                         let e = this.view.domSelectionRange();
-                        if (e.focusNode && Ke(e.focusNode, e.focusOffset, e.anchorNode, e.anchorOffset)) return this.flushSoon()
+                        if (e.focusNode && qe(e.focusNode, e.focusOffset, e.anchorNode, e.anchorOffset)) return this.flushSoon()
                     }
                     this.flush()
                 }
             }
             setCurSelection() {
                 this.currentSelection.set(this.view.domSelectionRange())
             }
@@ -7609,15 +7658,15 @@
                 let {
                     view: e
                 } = this;
                 if (!e.docView || this.flushingSoon > -1) return;
                 let t = this.pendingRecords();
                 t.length && (this.queue = []);
                 let r = e.domSelectionRange(),
-                    i = !this.suppressingSelectionUpdates && !this.currentSelection.eq(r) && Yi(e) && !this.ignoreSelectionChange(r),
+                    i = !this.suppressingSelectionUpdates && !this.currentSelection.eq(r) && Xi(e) && !this.ignoreSelectionChange(r),
                     o = -1,
                     s = -1,
                     l = !1,
                     a = [];
                 if (e.editable)
                     for (let f = 0; f < t.length; f++) {
                         let h = this.registerMutation(t[f], a);
@@ -7628,39 +7677,39 @@
                     if (f.length == 2) {
                         let h = f[0],
                             d = f[1];
                         h.parentNode && h.parentNode.parentNode == d.parentNode ? d.remove() : h.remove()
                     }
                 }
                 let c = null;
-                o < 0 && i && e.input.lastFocus > Date.now() - 200 && Math.max(e.input.lastTouch, e.input.lastClick.time) < Date.now() - 300 && un(r) && (c = xr(e)) && c.eq(S.near(e.state.doc.resolve(0), 1)) ? (e.input.lastFocus = 0, ge(e), this.currentSelection.set(r), e.scrollToSelection()) : (o > -1 || i) && (o > -1 && (e.docView.markDirty(o, s), ac(e)), this.handleDOMChange(o, s, l, a), e.docView && e.docView.dirty ? e.updateState(e.state) : this.currentSelection.eq(r) || ge(e), this.currentSelection.set(r))
+                o < 0 && i && e.input.lastFocus > Date.now() - 200 && Math.max(e.input.lastTouch, e.input.lastClick.time) < Date.now() - 300 && pn(r) && (c = Mr(e)) && c.eq(S.near(e.state.doc.resolve(0), 1)) ? (e.input.lastFocus = 0, ge(e), this.currentSelection.set(r), e.scrollToSelection()) : (o > -1 || i) && (o > -1 && (e.docView.markDirty(o, s), mc(e)), this.handleDOMChange(o, s, l, a), e.docView && e.docView.dirty ? e.updateState(e.state) : this.currentSelection.eq(r) || ge(e), this.currentSelection.set(r))
             }
             registerMutation(e, t) {
                 if (t.indexOf(e.target) > -1) return null;
                 let r = this.view.docView.nearestDesc(e.target);
                 if (e.type == "attributes" && (r == this.view.docView || e.attributeName == "contenteditable" || e.attributeName == "style" && !e.oldValue && !e.target.getAttribute("style")) || !r || r.ignoreMutation(e)) return null;
                 if (e.type == "childList") {
                     for (let f = 0; f < e.addedNodes.length; f++) t.push(e.addedNodes[f]);
                     if (r.contentDOM && r.contentDOM != r.dom && !r.contentDOM.contains(e.target)) return {
                         from: r.posBefore,
                         to: r.posAfter
                     };
                     let i = e.previousSibling,
                         o = e.nextSibling;
-                    if (W && Ce <= 11 && e.addedNodes.length)
+                    if (W && Oe <= 11 && e.addedNodes.length)
                         for (let f = 0; f < e.addedNodes.length; f++) {
                             let {
                                 previousSibling: h,
                                 nextSibling: d
                             } = e.addedNodes[f];
                             (!h || Array.prototype.indexOf.call(e.addedNodes, h) < 0) && (i = h), (!d || Array.prototype.indexOf.call(e.addedNodes, d) < 0) && (o = d)
                         }
-                    let s = i && i.parentNode == e.target ? B(i) + 1 : 0,
+                    let s = i && i.parentNode == e.target ? I(i) + 1 : 0,
                         l = r.localPosFromDOM(e.target, s, -1),
-                        a = o && o.parentNode == e.target ? B(o) : e.target.childNodes.length,
+                        a = o && o.parentNode == e.target ? I(o) : e.target.childNodes.length,
                         c = r.localPosFromDOM(e.target, a, 1);
                     return {
                         from: l,
                         to: c
                     }
                 } else return e.type == "attributes" ? {
                     from: r.posAtStart - r.border,
@@ -7668,81 +7717,88 @@
                 } : {
                     from: r.posAtStart,
                     to: r.posAtEnd,
                     typeOver: e.target.nodeValue == e.oldValue
                 }
             }
         },
-        oo = new WeakMap,
-        so = !1;
+        so = new WeakMap,
+        lo = !1;
 
-    function ac(n) {
-        if (!oo.has(n) && (oo.set(n, null), ["normal", "nowrap", "pre-line"].indexOf(getComputedStyle(n.dom).whiteSpace) !== -1)) {
-            if (n.requiresGeckoHackNode = X, so) return;
-            console.warn("ProseMirror expects the CSS white-space property to be set, preferably to 'pre-wrap'. It is recommended to load style/prosemirror.css from the prosemirror-view package."), so = !0
+    function mc(n) {
+        if (!so.has(n) && (so.set(n, null), ["normal", "nowrap", "pre-line"].indexOf(getComputedStyle(n.dom).whiteSpace) !== -1)) {
+            if (n.requiresGeckoHackNode = X, lo) return;
+            console.warn("ProseMirror expects the CSS white-space property to be set, preferably to 'pre-wrap'. It is recommended to load style/prosemirror.css from the prosemirror-view package."), lo = !0
         }
     }
 
-    function cc(n) {
-        let e;
+    function ao(n, e) {
+        let t = e.startContainer,
+            r = e.startOffset,
+            i = e.endContainer,
+            o = e.endOffset,
+            s = n.domAtPos(n.state.selection.anchor);
+        return qe(s.node, s.offset, i, o) && ([t, r, i, o] = [i, o, t, r]), {
+            anchorNode: t,
+            anchorOffset: r,
+            focusNode: i,
+            focusOffset: o
+        }
+    }
 
-        function t(a) {
-            a.preventDefault(), a.stopImmediatePropagation(), e = a.getTargetRanges()[0]
+    function gc(n, e) {
+        if (e.getComposedRanges) {
+            let i = e.getComposedRanges(n.root)[0];
+            if (i) return ao(n, i)
         }
-        n.dom.addEventListener("beforeinput", t, !0), document.execCommand("indent"), n.dom.removeEventListener("beforeinput", t, !0);
-        let r = e.startContainer,
-            i = e.startOffset,
-            o = e.endContainer,
-            s = e.endOffset,
-            l = n.domAtPos(n.state.selection.anchor);
-        return Ke(l.node, l.offset, o, s) && ([r, i, o, s] = [o, s, r, i]), {
-            anchorNode: r,
-            anchorOffset: i,
-            focusNode: o,
-            focusOffset: s
+        let t;
+
+        function r(i) {
+            i.preventDefault(), i.stopImmediatePropagation(), t = i.getTargetRanges()[0]
         }
+        return n.dom.addEventListener("beforeinput", r, !0), document.execCommand("indent"), n.dom.removeEventListener("beforeinput", r, !0), t ? ao(n, t) : null
     }
 
-    function fc(n, e, t) {
+    function yc(n, e, t) {
         let {
             node: r,
             fromOffset: i,
             toOffset: o,
             from: s,
             to: l
         } = n.docView.parseRange(e, t), a = n.domSelectionRange(), c, f = a.anchorNode;
         if (f && n.dom.contains(f.nodeType == 1 ? f : f.parentNode) && (c = [{
                 node: f,
                 offset: a.anchorOffset
-            }], un(a) || c.push({
+            }], pn(a) || c.push({
                 node: a.focusNode,
                 offset: a.focusOffset
-            })), R && n.input.lastKeyCode === 8)
+            })), B && n.input.lastKeyCode === 8)
             for (let g = o; g > i; g--) {
                 let k = r.childNodes[g - 1],
                     N = k.pmViewDesc;
                 if (k.nodeName == "BR" && !N) {
                     o = g;
                     break
                 }
                 if (!N || N.size) break
             }
         let h = n.state.doc,
-            d = n.someProp("domParser") || Re.fromSchema(n.state.schema),
+            d = n.someProp("domParser") || ve.fromSchema(n.state.schema),
             p = h.resolve(s),
             u = null,
             m = d.parse(r, {
                 topNode: p.parent,
                 topMatch: p.parent.contentMatchAt(p.index()),
                 topOpen: !0,
                 from: i,
                 to: o,
                 preserveWhitespace: p.parent.type.whitespace == "pre" ? "full" : !0,
                 findPositions: c,
-                ruleFromNode: hc,
+                ruleFromNode: bc,
                 context: p
             });
         if (c && c[0].pos != null) {
             let g = c[0].pos,
                 k = c[1] && c[1].pos;
             k == null && (k = g), u = {
                 anchor: g + s,
@@ -7753,15 +7809,15 @@
             doc: m,
             sel: u,
             from: s,
             to: l
         }
     }
 
-    function hc(n) {
+    function bc(n) {
         let e = n.pmViewDesc;
         if (e) return e.parseRule();
         if (n.nodeName == "BR" && n.parentNode) {
             if (F && /^(ul|ol)$/i.test(n.parentNode.nodeName)) {
                 let t = document.createElement("div");
                 return t.appendChild(document.createElement("li")), {
                     skip: t
@@ -7770,107 +7826,100 @@
                 ignore: !0
             }
         } else if (n.nodeName == "IMG" && n.getAttribute("mark-placeholder")) return {
             ignore: !0
         };
         return null
     }
-    var dc = /^(a|abbr|acronym|b|bd[io]|big|br|button|cite|code|data(list)?|del|dfn|em|i|ins|kbd|label|map|mark|meter|output|q|ruby|s|samp|small|span|strong|su[bp]|time|u|tt|var)$/i;
+    var kc = /^(a|abbr|acronym|b|bd[io]|big|br|button|cite|code|data(list)?|del|dfn|em|i|ins|kbd|label|map|mark|meter|output|q|ruby|s|samp|small|span|strong|su[bp]|time|u|tt|var)$/i;
 
-    function uc(n, e, t, r, i) {
+    function xc(n, e, t, r, i) {
         let o = n.input.compositionPendingChanges || (n.composing ? n.input.compositionID : 0);
         if (n.input.compositionPendingChanges = 0, e < 0) {
             let M = n.input.lastSelectionTime > Date.now() - 50 ? n.input.lastSelectionOrigin : null,
-                be = xr(n, M);
+                be = Mr(n, M);
             if (be && !n.state.selection.eq(be)) {
-                if (R && Y && n.input.lastKeyCode === 13 && Date.now() - 100 < n.input.lastKeyCodeTime && n.someProp("handleKeyDown", Rs => Rs(n, Fe(13, "Enter")))) return;
+                if (B && Y && n.input.lastKeyCode === 13 && Date.now() - 100 < n.input.lastKeyCodeTime && n.someProp("handleKeyDown", Bs => Bs(n, Be(13, "Enter")))) return;
                 let Pt = n.state.tr.setSelection(be);
                 M == "pointer" ? Pt.setMeta("pointer", !0) : M == "key" && Pt.scrollIntoView(), o && Pt.setMeta("composition", o), n.dispatch(Pt)
             }
             return
         }
         let s = n.state.doc.resolve(e),
             l = s.sharedDepth(t);
         e = s.before(l + 1), t = n.state.doc.resolve(t).after(l + 1);
         let a = n.state.selection,
-            c = fc(n, e, t),
+            c = yc(n, e, t),
             f = n.state.doc,
             h = f.slice(c.from, c.to),
             d, p;
         n.input.lastKeyCode === 8 && Date.now() - 100 < n.input.lastKeyCodeTime ? (d = n.state.selection.to, p = "end") : (d = n.state.selection.from, p = "start"), n.input.lastKeyCode = null;
-        let u = gc(h.content, c.doc.content, c.from, d, p);
-        if ((tt && n.input.lastIOSEnter > Date.now() - 225 || Y) && i.some(M => M.nodeType == 1 && !dc.test(M.nodeName)) && (!u || u.endA >= u.endB) && n.someProp("handleKeyDown", M => M(n, Fe(13, "Enter")))) {
+        let u = Cc(h.content, c.doc.content, c.from, d, p);
+        if ((tt && n.input.lastIOSEnter > Date.now() - 225 || Y) && i.some(M => M.nodeType == 1 && !kc.test(M.nodeName)) && (!u || u.endA >= u.endB) && n.someProp("handleKeyDown", M => M(n, Be(13, "Enter")))) {
             n.input.lastIOSEnter = 0;
             return
         }
         if (!u)
             if (r && a instanceof w && !a.empty && a.$head.sameParent(a.$anchor) && !n.composing && !(c.sel && c.sel.anchor != c.sel.head)) u = {
                 start: a.from,
                 endA: a.to,
                 endB: a.to
             };
             else {
                 if (c.sel) {
-                    let M = lo(n, n.state.doc, c.sel);
+                    let M = co(n, n.state.doc, c.sel);
                     if (M && !M.eq(n.state.selection)) {
                         let be = n.state.tr.setSelection(M);
                         o && be.setMeta("composition", o), n.dispatch(be)
                     }
                 }
                 return
-            } if (R && n.cursorWrapper && c.sel && c.sel.anchor == n.cursorWrapper.deco.from && c.sel.head == c.sel.anchor) {
-            let M = u.endB - u.start;
-            c.sel = {
-                anchor: c.sel.anchor + M,
-                head: c.sel.anchor + M
-            }
-        }
-        n.input.domChangeCount++, n.state.selection.from < n.state.selection.to && u.start == u.endB && n.state.selection instanceof w && (u.start > n.state.selection.from && u.start <= n.state.selection.from + 2 && n.state.selection.from >= c.from ? u.start = n.state.selection.from : u.endA < n.state.selection.to && u.endA >= n.state.selection.to - 2 && n.state.selection.to <= c.to && (u.endB += n.state.selection.to - u.endA, u.endA = n.state.selection.to)), W && Ce <= 11 && u.endB == u.start + 1 && u.endA == u.start && u.start > c.from && c.doc.textBetween(u.start - c.from - 1, u.start - c.from + 1) == " \xA0" && (u.start--, u.endA--, u.endB--);
+            } n.input.domChangeCount++, n.state.selection.from < n.state.selection.to && u.start == u.endB && n.state.selection instanceof w && (u.start > n.state.selection.from && u.start <= n.state.selection.from + 2 && n.state.selection.from >= c.from ? u.start = n.state.selection.from : u.endA < n.state.selection.to && u.endA >= n.state.selection.to - 2 && n.state.selection.to <= c.to && (u.endB += n.state.selection.to - u.endA, u.endA = n.state.selection.to)), W && Oe <= 11 && u.endB == u.start + 1 && u.endA == u.start && u.start > c.from && c.doc.textBetween(u.start - c.from - 1, u.start - c.from + 1) == " \xA0" && (u.start--, u.endA--, u.endB--);
         let m = c.doc.resolveNoCache(u.start - c.from),
             g = c.doc.resolveNoCache(u.endB - c.from),
             k = f.resolve(u.start),
             N = m.sameParent(g) && m.parent.inlineContent && k.end() >= u.endA,
             ee;
-        if ((tt && n.input.lastIOSEnter > Date.now() - 225 && (!N || i.some(M => M.nodeName == "DIV" || M.nodeName == "P")) || !N && m.pos < c.doc.content.size && !m.sameParent(g) && (ee = S.findFrom(c.doc.resolve(m.pos + 1), 1, !0)) && ee.head == g.pos) && n.someProp("handleKeyDown", M => M(n, Fe(13, "Enter")))) {
+        if ((tt && n.input.lastIOSEnter > Date.now() - 225 && (!N || i.some(M => M.nodeName == "DIV" || M.nodeName == "P")) || !N && m.pos < c.doc.content.size && !m.sameParent(g) && (ee = S.findFrom(c.doc.resolve(m.pos + 1), 1, !0)) && ee.head == g.pos) && n.someProp("handleKeyDown", M => M(n, Be(13, "Enter")))) {
             n.input.lastIOSEnter = 0;
             return
         }
-        if (n.state.selection.anchor > u.start && mc(f, u.start, u.endA, m, g) && n.someProp("handleKeyDown", M => M(n, Fe(8, "Backspace")))) {
-            Y && R && n.domObserver.suppressSelectionUpdates();
+        if (n.state.selection.anchor > u.start && Mc(f, u.start, u.endA, m, g) && n.someProp("handleKeyDown", M => M(n, Be(8, "Backspace")))) {
+            Y && B && n.domObserver.suppressSelectionUpdates();
             return
         }
-        R && Y && u.endB == u.start && (n.input.lastAndroidDelete = Date.now()), Y && !N && m.start() != g.start() && g.parentOffset == 0 && m.depth == g.depth && c.sel && c.sel.anchor == c.sel.head && c.sel.head == u.endA && (u.endB -= 2, g = c.doc.resolveNoCache(u.endB - c.from), setTimeout(() => {
+        B && Y && u.endB == u.start && (n.input.lastAndroidDelete = Date.now()), Y && !N && m.start() != g.start() && g.parentOffset == 0 && m.depth == g.depth && c.sel && c.sel.anchor == c.sel.head && c.sel.head == u.endA && (u.endB -= 2, g = c.doc.resolveNoCache(u.endB - c.from), setTimeout(() => {
             n.someProp("handleKeyDown", function(M) {
-                return M(n, Fe(13, "Enter"))
+                return M(n, Be(13, "Enter"))
             })
         }, 20));
         let te = u.start,
             U = u.endA,
             C, H, ne;
         if (N) {
-            if (m.pos == g.pos) W && Ce <= 11 && m.parentOffset == 0 && (n.domObserver.suppressSelectionUpdates(), setTimeout(() => ge(n), 20)), C = n.state.tr.delete(te, U), H = f.resolve(u.start).marksAcross(f.resolve(u.endA));
-            else if (u.endA == u.endB && (ne = pc(m.parent.content.cut(m.parentOffset, g.parentOffset), k.parent.content.cut(k.parentOffset, u.endA - k.start())))) C = n.state.tr, ne.type == "add" ? C.addMark(te, U, ne.mark) : C.removeMark(te, U, ne.mark);
+            if (m.pos == g.pos) W && Oe <= 11 && m.parentOffset == 0 && (n.domObserver.suppressSelectionUpdates(), setTimeout(() => ge(n), 20)), C = n.state.tr.delete(te, U), H = f.resolve(u.start).marksAcross(f.resolve(u.endA));
+            else if (u.endA == u.endB && (ne = Sc(m.parent.content.cut(m.parentOffset, g.parentOffset), k.parent.content.cut(k.parentOffset, u.endA - k.start())))) C = n.state.tr, ne.type == "add" ? C.addMark(te, U, ne.mark) : C.removeMark(te, U, ne.mark);
             else if (m.parent.child(m.index()).isText && m.index() == g.index() - (g.textOffset ? 0 : 1)) {
                 let M = m.parent.textBetween(m.parentOffset, g.parentOffset);
                 if (n.someProp("handleTextInput", be => be(n, te, U, M))) return;
                 C = n.state.tr.insertText(M, te, U)
             }
         }
         if (C || (C = n.state.tr.replace(te, U, c.doc.slice(u.start - c.from, u.endB - c.from))), c.sel) {
-            let M = lo(n, C.doc, c.sel);
-            M && !(R && Y && n.composing && M.empty && (u.start != u.endB || n.input.lastAndroidDelete < Date.now() - 100) && (M.head == te || M.head == C.mapping.map(U) - 1) || W && M.empty && M.head == te) && C.setSelection(M)
+            let M = co(n, C.doc, c.sel);
+            M && !(B && Y && n.composing && M.empty && (u.start != u.endB || n.input.lastAndroidDelete < Date.now() - 100) && (M.head == te || M.head == C.mapping.map(U) - 1) || W && M.empty && M.head == te) && C.setSelection(M)
         }
         H && C.ensureMarks(H), o && C.setMeta("composition", o), n.dispatch(C.scrollIntoView())
     }
 
-    function lo(n, e, t) {
-        return Math.max(t.anchor, t.head) > e.content.size ? null : Sr(n, e.resolve(t.anchor), e.resolve(t.head))
+    function co(n, e, t) {
+        return Math.max(t.anchor, t.head) > e.content.size ? null : Cr(n, e.resolve(t.anchor), e.resolve(t.head))
     }
 
-    function pc(n, e) {
+    function Sc(n, e) {
         let t = n.firstChild.marks,
             r = e.firstChild.marks,
             i = t,
             o = r,
             s, l, a;
         for (let f = 0; f < r.length; f++) i = r[f].removeFromSet(i);
         for (let f = 0; f < t.length; f++) o = t[f].removeFromSet(o);
@@ -7881,84 +7930,88 @@
         for (let f = 0; f < e.childCount; f++) c.push(a(e.child(f)));
         if (y.from(c).eq(n)) return {
             mark: l,
             type: s
         }
     }
 
-    function mc(n, e, t, r, i) {
-        if (!r.parent.isTextblock || t - e <= i.pos - r.pos || Zn(r, !0, !1) < i.pos) return !1;
+    function Mc(n, e, t, r, i) {
+        if (t - e <= i.pos - r.pos || tr(r, !0, !1) < i.pos) return !1;
         let o = n.resolve(e);
+        if (!r.parent.isTextblock) {
+            let l = o.nodeAfter;
+            return l != null && t == e + l.nodeSize
+        }
         if (o.parentOffset < o.parent.content.size || !o.parent.isTextblock) return !1;
-        let s = n.resolve(Zn(o, !0, !0));
-        return !s.parent.isTextblock || s.pos > t || Zn(s, !0, !1) < t ? !1 : r.parent.content.cut(r.parentOffset).eq(s.parent.content)
+        let s = n.resolve(tr(o, !0, !0));
+        return !s.parent.isTextblock || s.pos > t || tr(s, !0, !1) < t ? !1 : r.parent.content.cut(r.parentOffset).eq(s.parent.content)
     }
 
-    function Zn(n, e, t) {
+    function tr(n, e, t) {
         let r = n.depth,
             i = e ? n.end() : n.pos;
         for (; r > 0 && (e || n.indexAfter(r) == n.node(r).childCount);) r--, i++, e = !1;
         if (t) {
             let o = n.node(r).maybeChild(n.indexAfter(r));
             for (; o && !o.isLeaf;) o = o.firstChild, i++
         }
         return i
     }
 
-    function gc(n, e, t, r, i) {
+    function Cc(n, e, t, r, i) {
         let o = n.findDiffStart(e, t);
         if (o == null) return null;
         let {
             a: s,
             b: l
         } = n.findDiffEnd(e, t + n.size, t + e.size);
         if (i == "end") {
             let a = Math.max(0, o - Math.min(s, l));
             r -= s + a - o
         }
         if (s < o && n.size < e.size) {
             let a = r <= o && r >= s ? o - r : 0;
-            o -= a, o && o < e.size && ao(e.textBetween(o - 1, o + 1)) && (o += a ? 1 : -1), l = o + (l - s), s = o
+            o -= a, o && o < e.size && fo(e.textBetween(o - 1, o + 1)) && (o += a ? 1 : -1), l = o + (l - s), s = o
         } else if (l < o) {
             let a = r <= o && r >= l ? o - r : 0;
-            o -= a, o && o < n.size && ao(n.textBetween(o - 1, o + 1)) && (o += a ? 1 : -1), s = o + (s - l), l = o
+            o -= a, o && o < n.size && fo(n.textBetween(o - 1, o + 1)) && (o += a ? 1 : -1), s = o + (s - l), l = o
         }
         return {
             start: o,
             endA: s,
             endB: l
         }
     }
 
-    function ao(n) {
+    function fo(n) {
         if (n.length != 2) return !1;
         let e = n.charCodeAt(0),
             t = n.charCodeAt(1);
         return e >= 56320 && e <= 57343 && t >= 55296 && t <= 56319
     }
-    var dn = class {
+    var un = class {
         constructor(e, t) {
-            this._root = null, this.focused = !1, this.trackWrites = null, this.mounted = !1, this.markCursor = null, this.cursorWrapper = null, this.lastSelectedViewDesc = void 0, this.input = new dr, this.prevDirectPlugins = [], this.pluginViews = [], this.requiresGeckoHackNode = !1, this.dragging = null, this._props = t, this.state = t.state, this.directPlugins = t.plugins || [], this.directPlugins.forEach(po), this.dispatch = this.dispatch.bind(this), this.dom = e && e.mount || document.createElement("div"), e && (e.appendChild ? e.appendChild(this.dom) : typeof e == "function" ? e(this.dom) : e.mount && (this.mounted = !0)), this.editable = ho(this), fo(this), this.nodeViews = uo(this), this.docView = $i(this.state.doc, co(this), Qn(this), this.dom, this), this.domObserver = new yr(this, (r, i, o, s) => uc(this, r, i, o, s)), this.domObserver.start(), qa(this), this.updatePluginViews()
+            this._root = null, this.focused = !1, this.trackWrites = null, this.mounted = !1, this.markCursor = null, this.cursorWrapper = null, this.lastSelectedViewDesc = void 0, this.input = new mr, this.prevDirectPlugins = [], this.pluginViews = [], this.requiresGeckoHackNode = !1, this.dragging = null, this._props = t, this.state = t.state, this.directPlugins = t.plugins || [], this.directPlugins.forEach(go), this.dispatch = this.dispatch.bind(this), this.dom = e && e.mount || document.createElement("div"), e && (e.appendChild ? e.appendChild(this.dom) : typeof e == "function" ? e(this.dom) : e.mount && (this.mounted = !0)), this.editable = po(this), uo(this), this.nodeViews = mo(this), this.docView = Hi(this.state.doc, ho(this), er(this), this.dom, this), this.domObserver = new xr(this, (r, i, o, s) => xc(this, r, i, o, s)), this.domObserver.start(), Ua(this), this.updatePluginViews()
         }
         get composing() {
             return this.input.composing
         }
         get props() {
             if (this._props.state != this.state) {
                 let e = this._props;
                 this._props = {};
                 for (let t in e) this._props[t] = e[t];
                 this._props.state = this.state
             }
             return this._props
         }
         update(e) {
-            e.handleDOMEvents != this._props.handleDOMEvents && ur(this);
+            e.handleDOMEvents != this._props.handleDOMEvents && gr(this);
             let t = this._props;
-            this._props = e, e.plugins && (e.plugins.forEach(po), this.directPlugins = e.plugins), this.updateStateInner(e.state, t)
+            this._props = e, e.plugins && (e.plugins.forEach(go), this.directPlugins = e.plugins), this.updateStateInner(e.state, t)
         }
         setProps(e) {
             let t = {};
             for (let r in this._props) t[r] = this._props[r];
             t.state = this.state;
             for (let r in e) t[r] = e[r];
             this.update(t)
@@ -7967,44 +8020,44 @@
             this.updateStateInner(e, this._props)
         }
         updateStateInner(e, t) {
             var r;
             let i = this.state,
                 o = !1,
                 s = !1;
-            e.storedMarks && this.composing && (Lo(this), s = !0), this.state = e;
+            e.storedMarks && this.composing && (Wo(this), s = !0), this.state = e;
             let l = i.plugins != e.plugins || this._props.plugins != t.plugins;
             if (l || this._props.plugins != t.plugins || this._props.nodeViews != t.nodeViews) {
-                let p = uo(this);
-                bc(p, this.nodeViews) && (this.nodeViews = p, o = !0)
-            }(l || t.handleDOMEvents != this._props.handleDOMEvents) && ur(this), this.editable = ho(this), fo(this);
-            let a = Qn(this),
-                c = co(this),
+                let p = mo(this);
+                wc(p, this.nodeViews) && (this.nodeViews = p, o = !0)
+            }(l || t.handleDOMEvents != this._props.handleDOMEvents) && gr(this), this.editable = po(this), uo(this);
+            let a = er(this),
+                c = ho(this),
                 f = i.plugins != e.plugins && !i.doc.eq(e.doc) ? "reset" : e.scrollToSelection > i.scrollToSelection ? "to selection" : "preserve",
                 h = o || !this.docView.matchesNode(e.doc, c, a);
             (h || !e.selection.eq(i.selection)) && (s = !0);
-            let d = f == "preserve" && s && this.dom.style.overflowAnchor == null && sa(this);
+            let d = f == "preserve" && s && this.dom.style.overflowAnchor == null && ua(this);
             if (s) {
                 this.domObserver.stop();
-                let p = h && (W || R) && !this.composing && !i.selection.empty && !e.selection.empty && yc(i.selection, e.selection);
+                let p = h && (W || B) && !this.composing && !i.selection.empty && !e.selection.empty && Oc(i.selection, e.selection);
                 if (h) {
-                    let u = R ? this.trackWrites = this.domSelectionRange().focusNode : null;
-                    (o || !this.docView.update(e.doc, c, a, this)) && (this.docView.updateOuterDeco([]), this.docView.destroy(), this.docView = $i(e.doc, c, a, this.dom, this)), u && !this.trackWrites && (p = !0)
+                    let u = B ? this.trackWrites = this.domSelectionRange().focusNode : null;
+                    this.composing && (this.input.compositionNode = sc(this)), (o || !this.docView.update(e.doc, c, a, this)) && (this.docView.updateOuterDeco(c), this.docView.destroy(), this.docView = Hi(e.doc, c, a, this.dom, this)), u && !this.trackWrites && (p = !0)
                 }
-                p || !(this.input.mouseDown && this.domObserver.currentSelection.eq(this.domSelectionRange()) && Da(this)) ? ge(this, p) : (No(this, e.selection), this.domObserver.setCurSelection()), this.domObserver.start()
+                p || !(this.input.mouseDown && this.domObserver.currentSelection.eq(this.domSelectionRange()) && Pa(this)) ? ge(this, p) : (Do(this, e.selection), this.domObserver.setCurSelection()), this.domObserver.start()
             }
-            this.updatePluginViews(i), !((r = this.dragging) === null || r === void 0) && r.node && !i.doc.eq(e.doc) && this.updateDraggedNode(this.dragging, i), f == "reset" ? this.dom.scrollTop = 0 : f == "to selection" ? this.scrollToSelection() : d && la(d)
+            this.updatePluginViews(i), !((r = this.dragging) === null || r === void 0) && r.node && !i.doc.eq(e.doc) && this.updateDraggedNode(this.dragging, i), f == "reset" ? this.dom.scrollTop = 0 : f == "to selection" ? this.scrollToSelection() : d && pa(d)
         }
         scrollToSelection() {
             let e = this.domSelectionRange().focusNode;
             if (!this.someProp("handleScrollToSelection", t => t(this)))
                 if (this.state.selection instanceof x) {
                     let t = this.docView.domAfterPos(this.state.selection.from);
-                    t.nodeType == 1 && Vi(this, t.getBoundingClientRect(), e)
-                } else Vi(this, this.coordsAtPos(this.state.selection.head, 1), e)
+                    t.nodeType == 1 && Li(this, t.getBoundingClientRect(), e)
+                } else Li(this, this.coordsAtPos(this.state.selection.head, 1), e)
         }
         destroyPluginViews() {
             let e;
             for (; e = this.pluginViews.pop();) e.destroy && e.destroy()
         }
         updatePluginViews(e) {
             if (!e || e.plugins != this.state.plugins || this.directPlugins != this.prevDirectPlugins) {
@@ -8027,15 +8080,15 @@
             let r = e.node,
                 i = -1;
             if (this.state.doc.nodeAt(r.from) == r.node) i = r.from;
             else {
                 let o = r.from + (this.state.doc.content.size - t.doc.content.size);
                 (o > 0 && this.state.doc.nodeAt(o)) == r.node && (i = o)
             }
-            this.dragging = new an(e.slice, e.move, i < 0 ? void 0 : x.create(this.state.doc, i))
+            this.dragging = new cn(e.slice, e.move, i < 0 ? void 0 : x.create(this.state.doc, i))
         }
         someProp(e, t) {
             let r = this._props && this._props[e],
                 i;
             if (r != null && (i = t ? t(r) : r)) return i;
             for (let s = 0; s < this.directPlugins.length; s++) {
                 let l = this.directPlugins[s].props[e];
@@ -8058,126 +8111,127 @@
                     e = e.parentElement
                 }
                 return !0
             }
             return this.root.activeElement == this.dom
         }
         focus() {
-            this.domObserver.stop(), this.editable && aa(this.dom), ge(this), this.domObserver.start()
+            this.domObserver.stop(), this.editable && ma(this.dom), ge(this), this.domObserver.start()
         }
         get root() {
             let e = this._root;
             if (e == null) {
                 for (let t = this.dom.parentNode; t; t = t.parentNode)
                     if (t.nodeType == 9 || t.nodeType == 11 && t.host) return t.getSelection || (Object.getPrototypeOf(t).getSelection = () => t.ownerDocument.getSelection()), this._root = t
             }
             return e || document
         }
         updateRoot() {
             this._root = null
         }
         posAtCoords(e) {
-            return ua(this, e)
+            return xa(this, e)
         }
         coordsAtPos(e, t = 1) {
-            return xo(this, e, t)
+            return Mo(this, e, t)
         }
         domAtPos(e, t = 0) {
             return this.docView.domFromPos(e, t)
         }
         nodeDOM(e) {
             let t = this.docView.descAt(e);
             return t ? t.nodeDOM : null
         }
         posAtDOM(e, t, r = -1) {
             let i = this.docView.posFromDOM(e, t, r);
             if (i == null) throw new RangeError("DOM position not inside the editor");
             return i
         }
         endOfTextblock(e, t) {
-            return ba(this, t || this.state, e)
+            return wa(this, t || this.state, e)
         }
         pasteHTML(e, t) {
             return Nt(this, "", e, !1, t || new ClipboardEvent("paste"))
         }
         pasteText(e, t) {
             return Nt(this, e, null, !0, t || new ClipboardEvent("paste"))
         }
         destroy() {
-            this.docView && (Ka(this), this.destroyPluginViews(), this.mounted ? (this.docView.update(this.state.doc, [], Qn(this), this), this.dom.textContent = "") : this.dom.parentNode && this.dom.parentNode.removeChild(this.dom), this.docView.destroy(), this.docView = null)
+            this.docView && (Ga(this), this.destroyPluginViews(), this.mounted ? (this.docView.update(this.state.doc, [], er(this), this), this.dom.textContent = "") : this.dom.parentNode && this.dom.parentNode.removeChild(this.dom), this.docView.destroy(), this.docView = null, ta())
         }
         get isDestroyed() {
             return this.docView == null
         }
         dispatchEvent(e) {
-            return Ha(this, e)
+            return Xa(this, e)
         }
         dispatch(e) {
             let t = this._props.dispatchTransaction;
             t ? t.call(this, e) : this.updateState(this.state.apply(e))
         }
         domSelectionRange() {
-            return F && this.root.nodeType === 11 && Zl(this.dom.ownerDocument) == this.dom ? cc(this) : this.domSelection()
+            let e = this.domSelection();
+            return F && this.root.nodeType === 11 && sa(this.dom.ownerDocument) == this.dom && gc(this, e) || e
         }
         domSelection() {
             return this.root.getSelection()
         }
     };
 
-    function co(n) {
+    function ho(n) {
         let e = Object.create(null);
         return e.class = "ProseMirror", e.contenteditable = String(n.editable), n.someProp("attributes", t => {
             if (typeof t == "function" && (t = t(n.state)), t)
                 for (let r in t) r == "class" ? e.class += " " + t[r] : r == "style" ? e.style = (e.style ? e.style + ";" : "") + t[r] : !e[r] && r != "contenteditable" && r != "nodeName" && (e[r] = String(t[r]))
         }), e.translate || (e.translate = "no"), [ae.node(0, n.state.doc.content.size, e)]
     }
 
-    function fo(n) {
+    function uo(n) {
         if (n.markCursor) {
             let e = document.createElement("img");
             e.className = "ProseMirror-separator", e.setAttribute("mark-placeholder", "true"), e.setAttribute("alt", ""), n.cursorWrapper = {
                 dom: e,
                 deco: ae.widget(n.state.selection.head, e, {
                     raw: !0,
                     marks: n.markCursor
                 })
             }
         } else n.cursorWrapper = null
     }
 
-    function ho(n) {
+    function po(n) {
         return !n.someProp("editable", e => e(n.state) === !1)
     }
 
-    function yc(n, e) {
+    function Oc(n, e) {
         let t = Math.min(n.$anchor.sharedDepth(n.head), e.$anchor.sharedDepth(e.head));
         return n.$anchor.start(t) != e.$anchor.start(t)
     }
 
-    function uo(n) {
+    function mo(n) {
         let e = Object.create(null);
 
         function t(r) {
             for (let i in r) Object.prototype.hasOwnProperty.call(e, i) || (e[i] = r[i])
         }
         return n.someProp("nodeViews", t), n.someProp("markViews", t), e
     }
 
-    function bc(n, e) {
+    function wc(n, e) {
         let t = 0,
             r = 0;
         for (let i in n) {
             if (n[i] != e[i]) return !0;
             t++
         }
         for (let i in e) r++;
         return t != r
     }
 
-    function po(n) {
+    function go(n) {
         if (n.spec.state || n.spec.filterTransaction || n.spec.appendTransaction) throw new RangeError("Plugins passed directly to the view must not have a state component")
     }
     var z = class n extends S {
         constructor(e) {
             super(e, e)
         }
         map(e, t) {
@@ -8197,19 +8251,19 @@
             }
         }
         static fromJSON(e, t) {
             if (typeof t.pos != "number") throw new RangeError("Invalid input for GapCursor.fromJSON");
             return new n(e.resolve(t.pos))
         }
         getBookmark() {
-            return new Nr(this.anchor)
+            return new Dr(this.anchor)
         }
         static valid(e) {
             let t = e.parent;
-            if (t.isTextblock || !kc(e) || !xc(e)) return !1;
+            if (t.isTextblock || !Nc(e) || !Tc(e)) return !1;
             let r = t.type.spec.allowGapCursor;
             if (r != null) return r;
             let i = t.contentMatchAt(e.index()).defaultType;
             return i && i.isTextblock
         }
         static findGapCursorFrom(e, t, r = !1) {
             e: for (;;) {
@@ -8242,28 +8296,28 @@
                 return null
             }
         }
     };
     z.prototype.visible = !1;
     z.findFrom = z.findGapCursorFrom;
     S.jsonID("gapcursor", z);
-    var Nr = class n {
+    var Dr = class n {
         constructor(e) {
             this.pos = e
         }
         map(e) {
             return new n(e.map(this.pos))
         }
         resolve(e) {
             let t = e.resolve(this.pos);
             return z.valid(t) ? new z(t) : S.near(t)
         }
     };
 
-    function kc(n) {
+    function Nc(n) {
         for (let e = n.depth; e >= 0; e--) {
             let t = n.index(e),
                 r = n.node(e);
             if (t == 0) {
                 if (r.type.spec.isolating) return !0;
                 continue
             }
@@ -8271,15 +8325,15 @@
                 if (i.childCount == 0 && !i.inlineContent || i.isAtom || i.type.spec.isolating) return !0;
                 if (i.inlineContent) return !1
             }
         }
         return !0
     }
 
-    function xc(n) {
+    function Tc(n) {
         for (let e = n.depth; e >= 0; e--) {
             let t = n.indexAfter(e),
                 r = n.node(e);
             if (t == r.childCount) {
                 if (r.type.spec.isolating) return !0;
                 continue
             }
@@ -8287,112 +8341,112 @@
                 if (i.childCount == 0 && !i.inlineContent || i.isAtom || i.type.spec.isolating) return !0;
                 if (i.inlineContent) return !1
             }
         }
         return !0
     }
 
-    function Ho() {
+    function _o() {
         return new J({
             props: {
-                decorations: Oc,
+                decorations: Ic,
                 createSelectionBetween(n, e, t) {
                     return e.pos == t.pos && z.valid(t) ? new z(t) : null
                 },
-                handleClick: Mc,
-                handleKeyDown: Sc,
+                handleClick: Ec,
+                handleKeyDown: Dc,
                 handleDOMEvents: {
-                    beforeinput: Cc
+                    beforeinput: Ac
                 }
             }
         })
     }
-    var Sc = Gn({
-        ArrowLeft: mn("horiz", -1),
-        ArrowRight: mn("horiz", 1),
-        ArrowUp: mn("vert", -1),
-        ArrowDown: mn("vert", 1)
+    var Dc = Xn({
+        ArrowLeft: gn("horiz", -1),
+        ArrowRight: gn("horiz", 1),
+        ArrowUp: gn("vert", -1),
+        ArrowDown: gn("vert", 1)
     });
 
-    function mn(n, e) {
+    function gn(n, e) {
         let t = n == "vert" ? e > 0 ? "down" : "up" : e > 0 ? "right" : "left";
         return function(r, i, o) {
             let s = r.selection,
                 l = e > 0 ? s.$to : s.$from,
                 a = s.empty;
             if (s instanceof w) {
                 if (!o.endOfTextblock(t) || l.depth == 0) return !1;
                 a = !1, l = r.doc.resolve(e > 0 ? l.after() : l.before())
             }
             let c = z.findGapCursorFrom(l, e, a);
             return c ? (i && i(r.tr.setSelection(new z(c))), !0) : !1
         }
     }
 
-    function Mc(n, e, t) {
+    function Ec(n, e, t) {
         if (!n || !n.editable) return !1;
         let r = n.state.doc.resolve(e);
         if (!z.valid(r)) return !1;
         let i = n.posAtCoords({
             left: t.clientX,
             top: t.clientY
         });
         return i && i.inside > -1 && x.isSelectable(n.state.doc.nodeAt(i.inside)) ? !1 : (n.dispatch(n.state.tr.setSelection(new z(r))), !0)
     }
 
-    function Cc(n, e) {
+    function Ac(n, e) {
         if (e.inputType != "insertCompositionText" || !(n.state.selection instanceof z)) return !1;
         let {
             $from: t
         } = n.state.selection, r = t.parent.contentMatchAt(t.index()).findWrapping(n.state.schema.nodes.text);
         if (!r) return !1;
         let i = y.empty;
         for (let s = r.length - 1; s >= 0; s--) i = y.from(r[s].createAndFill(null, i));
         let o = n.state.tr.replace(t.pos, t.pos, new b(i, 0, 0));
         return o.setSelection(w.near(o.doc.resolve(t.pos + 1))), n.dispatch(o), !1
     }
 
-    function Oc(n) {
+    function Ic(n) {
         if (!(n.selection instanceof z)) return null;
         let e = document.createElement("div");
         return e.className = "ProseMirror-gapcursor", q.create(n.doc, [ae.widget(n.selection.head, e, {
             key: "gapcursor"
         })])
     }
-    var gn = 200,
-        A = function() {};
-    A.prototype.append = function(e) {
-        return e.length ? (e = A.from(e), !this.length && e || e.length < gn && this.leafAppend(e) || this.length < gn && e.leafPrepend(this) || this.appendInner(e)) : this
+    var yn = 200,
+        R = function() {};
+    R.prototype.append = function(e) {
+        return e.length ? (e = R.from(e), !this.length && e || e.length < yn && this.leafAppend(e) || this.length < yn && e.leafPrepend(this) || this.appendInner(e)) : this
     };
-    A.prototype.prepend = function(e) {
-        return e.length ? A.from(e).append(this) : this
+    R.prototype.prepend = function(e) {
+        return e.length ? R.from(e).append(this) : this
     };
-    A.prototype.appendInner = function(e) {
-        return new wc(this, e)
+    R.prototype.appendInner = function(e) {
+        return new Rc(this, e)
     };
-    A.prototype.slice = function(e, t) {
-        return e === void 0 && (e = 0), t === void 0 && (t = this.length), e >= t ? A.empty : this.sliceInner(Math.max(0, e), Math.min(this.length, t))
+    R.prototype.slice = function(e, t) {
+        return e === void 0 && (e = 0), t === void 0 && (t = this.length), e >= t ? R.empty : this.sliceInner(Math.max(0, e), Math.min(this.length, t))
     };
-    A.prototype.get = function(e) {
+    R.prototype.get = function(e) {
         if (!(e < 0 || e >= this.length)) return this.getInner(e)
     };
-    A.prototype.forEach = function(e, t, r) {
+    R.prototype.forEach = function(e, t, r) {
         t === void 0 && (t = 0), r === void 0 && (r = this.length), t <= r ? this.forEachInner(e, t, r, 0) : this.forEachInvertedInner(e, t, r, 0)
     };
-    A.prototype.map = function(e, t, r) {
+    R.prototype.map = function(e, t, r) {
         t === void 0 && (t = 0), r === void 0 && (r = this.length);
         var i = [];
         return this.forEach(function(o, s) {
             return i.push(e(o, s))
         }, t, r), i
     };
-    A.from = function(e) {
-        return e instanceof A ? e : e && e.length ? new jo(e) : A.empty
+    R.from = function(e) {
+        return e instanceof R ? e : e && e.length ? new Uo(e) : R.empty
     };
-    var jo = function(n) {
+    var Uo = function(n) {
         function e(r) {
             n.call(this), this.values = r
         }
         n && (e.__proto__ = n), e.prototype = Object.create(n && n.prototype), e.prototype.constructor = e;
         var t = {
             length: {
                 configurable: !0
@@ -8410,25 +8464,25 @@
         }, e.prototype.forEachInner = function(i, o, s, l) {
             for (var a = o; a < s; a++)
                 if (i(this.values[a], l + a) === !1) return !1
         }, e.prototype.forEachInvertedInner = function(i, o, s, l) {
             for (var a = o - 1; a >= s; a--)
                 if (i(this.values[a], l + a) === !1) return !1
         }, e.prototype.leafAppend = function(i) {
-            if (this.length + i.length <= gn) return new e(this.values.concat(i.flatten()))
+            if (this.length + i.length <= yn) return new e(this.values.concat(i.flatten()))
         }, e.prototype.leafPrepend = function(i) {
-            if (this.length + i.length <= gn) return new e(i.flatten().concat(this.values))
+            if (this.length + i.length <= yn) return new e(i.flatten().concat(this.values))
         }, t.length.get = function() {
             return this.values.length
         }, t.depth.get = function() {
             return 0
         }, Object.defineProperties(e.prototype, t), e
-    }(A);
-    A.empty = new jo([]);
-    var wc = function(n) {
+    }(R);
+    R.empty = new Uo([]);
+    var Rc = function(n) {
             function e(t, r) {
                 n.call(this), this.left = t, this.right = r, this.length = t.length + r.length, this.depth = Math.max(t.depth, r.depth) + 1
             }
             return n && (e.__proto__ = n), e.prototype = Object.create(n && n.prototype), e.prototype.constructor = e, e.prototype.flatten = function() {
                 return this.left.flatten().concat(this.right.flatten())
             }, e.prototype.getInner = function(r) {
                 return r < this.left.length ? this.left.get(r) : this.right.get(r - this.left.length)
@@ -8447,17 +8501,17 @@
                 if (i) return new e(this.left, i)
             }, e.prototype.leafPrepend = function(r) {
                 var i = this.left.leafPrepend(r);
                 if (i) return new e(i, this.right)
             }, e.prototype.appendInner = function(r) {
                 return this.left.depth >= Math.max(this.right.depth, r.depth) + 1 ? new e(this.left, new e(this.right, r)) : new e(this, r)
             }, e
-        }(A),
-        Tr = A;
-    var Nc = 500,
+        }(R),
+        Er = R;
+    var vc = 500,
         He = class n {
             constructor(e, t) {
                 this.items = e, this.eventCount = t
             }
             popEvent(e, t) {
                 if (this.eventCount == 0) return null;
                 let r = this.items.length;
@@ -8496,15 +8550,15 @@
                 for (let f = 0; f < e.steps.length; f++) {
                     let h = e.steps[f].invert(e.docs[f]),
                         d = new ce(e.mapping.maps[f], h, t),
                         p;
                     (p = a && a.merge(d)) && (d = p, f ? o.pop() : l = l.slice(0, l.length - 1)), o.push(d), t && (s++, t = void 0), i || (a = d)
                 }
                 let c = s - r.depth;
-                return c > Dc && (l = Tc(l, c), s -= c), new n(l.append(o), s)
+                return c > zc && (l = Pc(l, c), s -= c), new n(l.append(o), s)
             }
             remapping(e, t) {
                 let r = new ut;
                 return this.items.forEach((i, o) => {
                     let s = i.mirrorOffset != null && o - i.mirrorOffset >= e ? r.maps.length - i.mirrorOffset : void 0;
                     r.appendMap(i.map, s)
                 }, e, t), r
@@ -8534,15 +8588,15 @@
                         g && l++, r.push(new ce(u, m, g))
                     } else r.push(new ce(u))
                 }, i);
                 let c = [];
                 for (let d = t; d < s; d++) c.push(new ce(o.maps[d]));
                 let f = this.items.slice(0, i).append(c).append(r),
                     h = new n(f, l);
-                return h.emptyItemCount() > Nc && (h = h.compress(this.items.length - r.length)), h
+                return h.emptyItemCount() > vc && (h = h.compress(this.items.length - r.length)), h
             }
             emptyItemCount() {
                 let e = 0;
                 return this.items.forEach(t => {
                     t.step || e++
                 }), e
             }
@@ -8560,20 +8614,20 @@
                             let f = s.selection && s.selection.map(t.slice(r));
                             f && o++;
                             let h = new ce(c.invert(), a, f),
                                 d, p = i.length - 1;
                             (d = i.length && i[p].merge(h)) ? i[p] = d: i.push(h)
                         }
                     } else s.map && r--
-                }, this.items.length, 0), new n(Tr.from(i.reverse()), o)
+                }, this.items.length, 0), new n(Er.from(i.reverse()), o)
             }
         };
-    He.empty = new He(Tr.empty, 0);
+    He.empty = new He(Er.empty, 0);
 
-    function Tc(n, e) {
+    function Pc(n, e) {
         let t;
         return n.forEach((r, i) => {
             if (r.selection && e-- == 0) return t = i, !1
         }), n.slice(t)
     }
     var ce = class n {
             constructor(e, t, r, i) {
@@ -8587,159 +8641,167 @@
             }
         },
         fe = class {
             constructor(e, t, r, i, o) {
                 this.done = e, this.undone = t, this.prevRanges = r, this.prevTime = i, this.prevComposition = o
             }
         },
-        Dc = 20;
+        zc = 20;
 
-    function Ec(n, e, t, r) {
-        let i = t.getMeta(Ne),
+    function Bc(n, e, t, r) {
+        let i = t.getMeta($e),
             o;
         if (i) return i.historyState;
-        t.getMeta(Ac) && (n = new fe(n.done, n.undone, null, 0, -1));
+        t.getMeta(Lc) && (n = new fe(n.done, n.undone, null, 0, -1));
         let s = t.getMeta("appendedTransaction");
         if (t.steps.length == 0) return n;
-        if (s && s.getMeta(Ne)) return s.getMeta(Ne).redo ? new fe(n.done.addTransform(t, void 0, r, yn(e)), n.undone, _o(t.mapping.maps[t.steps.length - 1]), n.prevTime, n.prevComposition) : new fe(n.done, n.undone.addTransform(t, void 0, r, yn(e)), null, n.prevTime, n.prevComposition);
+        if (s && s.getMeta($e)) return s.getMeta($e).redo ? new fe(n.done.addTransform(t, void 0, r, bn(e)), n.undone, Go(t.mapping.maps[t.steps.length - 1]), n.prevTime, n.prevComposition) : new fe(n.done, n.undone.addTransform(t, void 0, r, bn(e)), null, n.prevTime, n.prevComposition);
         if (t.getMeta("addToHistory") !== !1 && !(s && s.getMeta("addToHistory") === !1)) {
             let l = t.getMeta("composition"),
-                a = n.prevTime == 0 || !s && n.prevComposition != l && (n.prevTime < (t.time || 0) - r.newGroupDelay || !Ic(t, n.prevRanges)),
-                c = s ? Dr(n.prevRanges, t.mapping) : _o(t.mapping.maps[t.steps.length - 1]);
-            return new fe(n.done.addTransform(t, a ? e.selection.getBookmark() : void 0, r, yn(e)), He.empty, c, t.time, l == null ? n.prevComposition : l)
-        } else return (o = t.getMeta("rebased")) ? new fe(n.done.rebased(t, o), n.undone.rebased(t, o), Dr(n.prevRanges, t.mapping), n.prevTime, n.prevComposition) : new fe(n.done.addMaps(t.mapping.maps), n.undone.addMaps(t.mapping.maps), Dr(n.prevRanges, t.mapping), n.prevTime, n.prevComposition)
+                a = n.prevTime == 0 || !s && n.prevComposition != l && (n.prevTime < (t.time || 0) - r.newGroupDelay || !Fc(t, n.prevRanges)),
+                c = s ? Ar(n.prevRanges, t.mapping) : Go(t.mapping.maps[t.steps.length - 1]);
+            return new fe(n.done.addTransform(t, a ? e.selection.getBookmark() : void 0, r, bn(e)), He.empty, c, t.time, l == null ? n.prevComposition : l)
+        } else return (o = t.getMeta("rebased")) ? new fe(n.done.rebased(t, o), n.undone.rebased(t, o), Ar(n.prevRanges, t.mapping), n.prevTime, n.prevComposition) : new fe(n.done.addMaps(t.mapping.maps), n.undone.addMaps(t.mapping.maps), Ar(n.prevRanges, t.mapping), n.prevTime, n.prevComposition)
     }
 
-    function Ic(n, e) {
+    function Fc(n, e) {
         if (!e) return !1;
         if (!n.docChanged) return !0;
         let t = !1;
         return n.mapping.maps[0].forEach((r, i) => {
             for (let o = 0; o < e.length; o += 2) r <= e[o + 1] && i >= e[o] && (t = !0)
         }), t
     }
 
-    function _o(n) {
+    function Go(n) {
         let e = [];
         return n.forEach((t, r, i, o) => e.push(i, o)), e
     }
 
-    function Dr(n, e) {
+    function Ar(n, e) {
         if (!n) return null;
         let t = [];
         for (let r = 0; r < n.length; r += 2) {
             let i = e.map(n[r], 1),
                 o = e.map(n[r + 1], -1);
             i <= o && t.push(i, o)
         }
         return t
     }
 
-    function Go(n, e, t, r) {
-        let i = yn(e),
-            o = Ne.get(e).spec.config,
-            s = (r ? n.undone : n.done).popEvent(e, i);
-        if (!s) return;
-        let l = s.selection.resolve(s.transform.doc),
-            a = (r ? n.done : n.undone).addTransform(s.transform, e.selection.getBookmark(), o, i),
-            c = new fe(r ? a : s.remaining, r ? s.remaining : a, null, 0, -1);
-        t(s.transform.setSelection(l).setMeta(Ne, {
-            redo: r,
-            historyState: c
-        }).scrollIntoView())
+    function Vc(n, e, t) {
+        let r = bn(e),
+            i = $e.get(e).spec.config,
+            o = (t ? n.undone : n.done).popEvent(e, r);
+        if (!o) return null;
+        let s = o.selection.resolve(o.transform.doc),
+            l = (t ? n.done : n.undone).addTransform(o.transform, e.selection.getBookmark(), i, r),
+            a = new fe(t ? l : o.remaining, t ? o.remaining : l, null, 0, -1);
+        return o.transform.setSelection(s).setMeta($e, {
+            redo: t,
+            historyState: a
+        })
     }
-    var Er = !1,
-        Uo = null;
+    var Ir = !1,
+        Yo = null;
 
-    function yn(n) {
+    function bn(n) {
         let e = n.plugins;
-        if (Uo != e) {
-            Er = !1, Uo = e;
+        if (Yo != e) {
+            Ir = !1, Yo = e;
             for (let t = 0; t < e.length; t++)
                 if (e[t].spec.historyPreserveItems) {
-                    Er = !0;
+                    Ir = !0;
                     break
                 }
         }
-        return Er
+        return Ir
     }
-    var Ne = new kt("history"),
-        Ac = new kt("closeHistory");
+    var $e = new kt("history"),
+        Lc = new kt("closeHistory");
 
-    function Yo(n = {}) {
+    function Xo(n = {}) {
         return n = {
             depth: n.depth || 100,
             newGroupDelay: n.newGroupDelay || 500
         }, new J({
-            key: Ne,
+            key: $e,
             state: {
                 init() {
                     return new fe(He.empty, He.empty, null, 0, -1)
                 },
                 apply(e, t, r) {
-                    return Ec(t, r, e, n)
+                    return Bc(t, r, e, n)
                 }
             },
             config: n,
             props: {
                 handleDOMEvents: {
                     beforeinput(e, t) {
                         let r = t.inputType,
-                            i = r == "historyUndo" ? Et : r == "historyRedo" ? rt : null;
+                            i = r == "historyUndo" ? At : r == "historyRedo" ? rt : null;
                         return i ? (t.preventDefault(), i(e.state, e.dispatch)) : !1
                     }
                 }
             }
         })
     }
-    var Et = (n, e) => {
-            let t = Ne.getState(n);
-            return !t || t.done.eventCount == 0 ? !1 : (e && Go(t, n, e, !1), !0)
-        },
-        rt = (n, e) => {
-            let t = Ne.getState(n);
-            return !t || t.undone.eventCount == 0 ? !1 : (e && Go(t, n, e, !0), !0)
-        };
-    var vc = ["p", 0],
-        Pc = ["blockquote", 0],
-        Rc = ["hr"],
-        zc = ["pre", ["code", 0]],
-        Bc = ["br"],
+
+    function kn(n, e) {
+        return (t, r) => {
+            let i = $e.getState(t);
+            if (!i || (n ? i.undone : i.done).eventCount == 0) return !1;
+            if (r) {
+                let o = Vc(i, t, n);
+                o && r(e ? o.scrollIntoView() : o)
+            }
+            return !0
+        }
+    }
+    var At = kn(!1, !0),
+        rt = kn(!0, !0),
+        zh = kn(!1, !1),
+        Bh = kn(!0, !1);
+    var Jc = ["p", 0],
+        Wc = ["blockquote", 0],
+        qc = ["hr"],
+        Kc = ["pre", ["code", 0]],
+        $c = ["br"],
         ye = {
             doc: {
                 content: "block+"
             },
             paragraph: {
                 content: "inline*",
                 group: "block",
                 parseDOM: [{
                     tag: "p"
                 }],
                 toDOM() {
-                    return vc
+                    return Jc
                 }
             },
             blockquote: {
                 content: "block+",
                 group: "block",
                 defining: !0,
                 parseDOM: [{
                     tag: "blockquote"
                 }],
                 toDOM() {
-                    return Pc
+                    return Wc
                 }
             },
             horizontal_rule: {
                 group: "block",
                 parseDOM: [{
                     tag: "hr"
                 }],
                 toDOM() {
-                    return Rc
+                    return qc
                 }
             },
             heading: {
                 attrs: {
                     level: {
                         default: 1
                     }
@@ -8789,15 +8851,15 @@
                 code: !0,
                 defining: !0,
                 parseDOM: [{
                     tag: "pre",
                     preserveWhitespace: "full"
                 }],
                 toDOM() {
-                    return zc
+                    return Kc
                 }
             },
             text: {
                 group: "inline"
             },
             image: {
                 inline: !0,
@@ -8839,21 +8901,21 @@
                 inline: !0,
                 group: "inline",
                 selectable: !1,
                 parseDOM: [{
                     tag: "br"
                 }],
                 toDOM() {
-                    return Bc
+                    return $c
                 }
             }
         },
-        Fc = ["em", 0],
-        Vc = ["strong", 0],
-        Lc = ["code", 0],
+        Hc = ["em", 0],
+        jc = ["strong", 0],
+        _c = ["code", 0],
         It = {
             link: {
                 attrs: {
                     href: {},
                     title: {
                         default: null
                     }
@@ -8887,15 +8949,15 @@
                 }, {
                     style: "font-style=italic"
                 }, {
                     style: "font-style=normal",
                     clearMark: n => n.type.name == "em"
                 }],
                 toDOM() {
-                    return Fc
+                    return Hc
                 }
             },
             strong: {
                 parseDOM: [{
                     tag: "strong"
                 }, {
                     tag: "b",
@@ -8904,89 +8966,89 @@
                     style: "font-weight=400",
                     clearMark: n => n.type.name == "strong"
                 }, {
                     style: "font-weight",
                     getAttrs: n => /^(bold(er)?|[5-9]\d{2,})$/.test(n) && null
                 }],
                 toDOM() {
-                    return Vc
+                    return jc
                 }
             },
             code: {
                 parseDOM: [{
                     tag: "code"
                 }],
                 toDOM() {
-                    return Lc
+                    return _c
                 }
             }
         },
-        Dh = new Pe({
+        Lh = new ke({
             nodes: ye,
             marks: It
         });
-    var Jc = ["ol", 0],
-        Wc = ["ul", 0],
-        qc = ["li", 0],
-        Kc = {
+    var Uc = ["ol", 0],
+        Gc = ["ul", 0],
+        Yc = ["li", 0],
+        Xc = {
             attrs: {
                 order: {
                     default: 1
                 }
             },
             parseDOM: [{
                 tag: "ol",
                 getAttrs(n) {
                     return {
                         order: n.hasAttribute("start") ? +n.getAttribute("start") : 1
                     }
                 }
             }],
             toDOM(n) {
-                return n.attrs.order == 1 ? Jc : ["ol", {
+                return n.attrs.order == 1 ? Uc : ["ol", {
                     start: n.attrs.order
                 }, 0]
             }
         },
-        $c = {
+        Qc = {
             parseDOM: [{
                 tag: "ul"
             }],
             toDOM() {
-                return Wc
+                return Gc
             }
         },
-        Hc = {
+        Zc = {
             parseDOM: [{
                 tag: "li"
             }],
             toDOM() {
-                return qc
+                return Yc
             },
             defining: !0
         };
 
-    function Ir(n, e) {
+    function Rr(n, e) {
         let t = {};
         for (let r in n) t[r] = n[r];
         for (let r in e) t[r] = e[r];
         return t
     }
 
-    function Xo(n, e, t) {
+    function Qo(n, e, t) {
         return n.append({
-            ordered_list: Ir(Kc, {
+            ordered_list: Rr(Xc, {
                 content: "list_item+",
                 group: t
             }),
-            bullet_list: Ir($c, {
+            bullet_list: Rr(Qc, {
                 content: "list_item+",
                 group: t
             }),
-            list_item: Ir(Hc, {
+            list_item: Rr(Zc, {
                 content: e
             })
         })
     }
 
     function it(n, e = null) {
         return function(t, r) {
@@ -8994,35 +9056,35 @@
                 $from: i,
                 $to: o
             } = t.selection, s = i.blockRange(o), l = !1, a = s;
             if (!s) return !1;
             if (s.depth >= 2 && i.node(s.depth - 1).type.compatibleContent(n) && s.startIndex == 0) {
                 if (i.index(s.depth - 1) == 0) return !1;
                 let f = t.doc.resolve(s.start - 2);
-                a = new Ae(f, f, s.depth), s.endIndex < s.parent.childCount && (s = new Ae(i, t.doc.resolve(o.end(s.depth)), s.depth)), l = !0
+                a = new Ie(f, f, s.depth), s.endIndex < s.parent.childCount && (s = new Ie(i, t.doc.resolve(o.end(s.depth)), s.depth)), l = !0
             }
-            let c = _t(a, n, e, s);
-            return c ? (r && r(jc(t.tr, s, c, l, n).scrollIntoView()), !0) : !1
+            let c = Ut(a, n, e, s);
+            return c ? (r && r(ef(t.tr, s, c, l, n).scrollIntoView()), !0) : !1
         }
     }
 
-    function jc(n, e, t, r, i) {
+    function ef(n, e, t, r, i) {
         let o = y.empty;
         for (let f = t.length - 1; f >= 0; f--) o = y.from(t[f].type.create(t[f].attrs, o));
         n.step(new E(e.start - (r ? 2 : 0), e.end, e.start, e.end, new b(o, 0, 0), t.length, !0));
         let s = 0;
         for (let f = 0; f < t.length; f++) t[f].type == i && (s = f + 1);
         let l = t.length - s,
             a = e.start + t.length - (r ? 2 : 0),
             c = e.parent;
-        for (let f = e.startIndex, h = e.endIndex, d = !0; f < h; f++, d = !1) !d && ke(n.doc, a, l) && (n.split(a, l), a += 2 * l), a += c.child(f).nodeSize;
+        for (let f = e.startIndex, h = e.endIndex, d = !0; f < h; f++, d = !1) !d && xe(n.doc, a, l) && (n.split(a, l), a += 2 * l), a += c.child(f).nodeSize;
         return n
     }
 
-    function Qo(n, e) {
+    function Zo(n, e) {
         return function(t, r) {
             let {
                 $from: i,
                 $to: o,
                 node: s
             } = t.selection;
             if (s && s.isBlock || i.depth < 2 || !i.sameParent(o)) return !1;
@@ -9050,41 +9112,41 @@
                 c = t.tr.delete(i.pos, o.pos),
                 f = a ? [e ? {
                     type: n,
                     attrs: e
                 } : null, {
                     type: a
                 }] : void 0;
-            return ke(c.doc, i.pos, 2, f) ? (r && r(c.split(i.pos, 2, f).scrollIntoView()), !0) : !1
+            return xe(c.doc, i.pos, 2, f) ? (r && r(c.split(i.pos, 2, f).scrollIntoView()), !0) : !1
         }
     }
 
-    function Zo(n) {
+    function es(n) {
         return function(e, t) {
             let {
                 $from: r,
                 $to: i
             } = e.selection, o = r.blockRange(i, s => s.childCount > 0 && s.firstChild.type == n);
-            return o ? t ? r.node(o.depth - 1).type == n ? _c(e, t, n, o) : Uc(e, t, o) : !0 : !1
+            return o ? t ? r.node(o.depth - 1).type == n ? tf(e, t, n, o) : nf(e, t, o) : !0 : !1
         }
     }
 
-    function _c(n, e, t, r) {
+    function tf(n, e, t, r) {
         let i = n.tr,
             o = r.end,
             s = r.$to.end(r.depth);
-        o < s && (i.step(new E(o - 1, s, o, s, new b(y.from(t.create(null, r.parent.copy())), 1, 0), 1, !0)), r = new Ae(i.doc.resolve(r.$from.pos), i.doc.resolve(s), r.depth));
+        o < s && (i.step(new E(o - 1, s, o, s, new b(y.from(t.create(null, r.parent.copy())), 1, 0), 1, !0)), r = new Ie(i.doc.resolve(r.$from.pos), i.doc.resolve(s), r.depth));
         let l = Ue(r);
         if (l == null) return !1;
         i.lift(r, l);
         let a = i.mapping.map(o, -1) - 1;
         return yt(i.doc, a) && i.join(a), e(i.scrollIntoView()), !0
     }
 
-    function Uc(n, e, t) {
+    function nf(n, e, t) {
         let r = n.tr,
             i = t.parent;
         for (let p = t.end, u = t.endIndex - 1, m = t.startIndex; u > m; u--) p -= i.child(u).nodeSize, r.delete(p - 1, p + 1);
         let o = r.doc.resolve(t.start),
             s = o.nodeAfter;
         if (r.mapping.map(t.end) != t.start + o.nodeAfter.nodeSize) return !1;
         let l = t.startIndex == 0,
@@ -9093,15 +9155,15 @@
             f = o.index(-1);
         if (!c.canReplace(f + (l ? 0 : 1), f + 1, s.content.append(a ? y.empty : y.from(i)))) return !1;
         let h = o.pos,
             d = h + s.nodeSize;
         return r.step(new E(h - (l ? 1 : 0), d + (a ? 1 : 0), h + 1, d - 1, new b((l ? y.empty : y.from(i.copy(y.empty))).append(a ? y.empty : y.from(i.copy(y.empty))), l ? 0 : 1, a ? 0 : 1), l ? 0 : 1)), e(r.scrollIntoView()), !0
     }
 
-    function es(n) {
+    function ts(n) {
         return function(e, t) {
             let {
                 $from: r,
                 $to: i
             } = e.selection, o = r.blockRange(i, c => c.childCount > 0 && c.firstChild.type == n);
             if (!o) return !1;
             let s = o.startIndex;
@@ -9118,31 +9180,31 @@
                 t(e.tr.step(new E(d - (c ? 3 : 1), p, d, p, h, 1, !0)).scrollIntoView())
             }
             return !0
         }
     }
     var Te = class {
         constructor(e, t, r = {}) {
-            this.match = e, this.match = e, this.handler = typeof t == "string" ? Gc(t) : t, this.undoable = r.undoable !== !1, this.inCode = r.inCode || !1
+            this.match = e, this.match = e, this.handler = typeof t == "string" ? rf(t) : t, this.undoable = r.undoable !== !1, this.inCode = r.inCode || !1
         }
     };
 
-    function Gc(n) {
+    function rf(n) {
         return function(e, t, r, i) {
             let o = n;
             if (t[1]) {
                 let s = t[0].lastIndexOf(t[1]);
                 o += t[0].slice(s + t[1].length), r += s;
                 let l = r - i;
                 l > 0 && (o = t[0].slice(s - l, s) + o, r = i)
             }
             return e.tr.insertText(o, r, i)
         }
     }
-    var ts = (n, e) => {
+    var ns = (n, e) => {
             let t = n.plugins;
             for (let r = 0; r < t.length; r++) {
                 let i = t[r],
                     o;
                 if (i.spec.isInputRules && (o = i.getState(n))) {
                     if (e) {
                         let s = n.tr,
@@ -9155,153 +9217,153 @@
                         e(s)
                     }
                     return !0
                 }
             }
             return !1
         },
-        Rh = new Te(/--$/, "\u2014"),
-        zh = new Te(/\.\.\.$/, "\u2026"),
-        Bh = new Te(/(?:^|[\s\{\[\(\<'"\u2018\u201C])(")$/, "\u201C"),
-        Fh = new Te(/"$/, "\u201D"),
-        Vh = new Te(/(?:^|[\s\{\[\(\<'"\u2018\u201C])(')$/, "\u2018"),
-        Lh = new Te(/'$/, "\u2019");
+        Hh = new Te(/--$/, "\u2014"),
+        jh = new Te(/\.\.\.$/, "\u2026"),
+        _h = new Te(/(?:^|[\s\{\[\(\<'"\u2018\u201C])(")$/, "\u201C"),
+        Uh = new Te(/"$/, "\u201D"),
+        Gh = new Te(/(?:^|[\s\{\[\(\<'"\u2018\u201C])(')$/, "\u2018"),
+        Yh = new Te(/'$/, "\u2019");
 
-    function Yc(n, e) {
+    function of(n, e) {
         let t = Object.keys(e);
         return t.length ? !!t.filter(r => e[r] === n[r]).length : !0
     }
 
-    function Ar(n, e, t = {}) {
-        return n.find(r => r.type === e && Yc(r.attrs, t))
+    function vr(n, e, t = {}) {
+        return n.find(r => r.type === e && of(r.attrs, t))
     }
 
-    function Xc(n, e, t = {}) {
-        return !!Ar(n, e, t)
+    function sf(n, e, t = {}) {
+        return !!vr(n, e, t)
     }
 
-    function vr(n, e, t = {}) {
+    function Pr(n, e, t = {}) {
         if (!n || !e) return;
         let r = n.parent.childAfter(n.parentOffset);
         if (!r.node) return;
-        let i = Ar(r.node.marks, e, t);
+        let i = vr(r.node.marks, e, t);
         if (!i) return;
         let o = n.index(),
             s = n.start() + r.offset,
             l = o + 1,
             a = s + r.node.nodeSize;
-        for (Ar(r.node.marks, e, t); o > 0 && i.isInSet(n.parent.child(o - 1).marks);) o -= 1, s -= n.parent.child(o).nodeSize;
-        for (; l < n.parent.childCount && Xc(n.parent.child(l).marks, e, t);) a += n.parent.child(l).nodeSize, l += 1;
+        for (vr(r.node.marks, e, t); o > 0 && i.isInSet(n.parent.child(o - 1).marks);) o -= 1, s -= n.parent.child(o).nodeSize;
+        for (; l < n.parent.childCount && sf(n.parent.child(l).marks, e, t);) a += n.parent.child(l).nodeSize, l += 1;
         return {
             from: s,
             to: a
         }
     }
 
-    function Qc(n) {
+    function lf(n) {
         var e = typeof n;
         return n != null && (e == "object" || e == "function")
     }
-    var At = Qc;
-    var Zc = typeof global == "object" && global && global.Object === Object && global,
-        ns = Zc;
-    var ef = typeof self == "object" && self && self.Object === Object && self,
-        tf = ns || ef || Function("return this")(),
-        bn = tf;
-    var nf = function() {
-            return bn.Date.now()
+    var Rt = lf;
+    var af = typeof global == "object" && global && global.Object === Object && global,
+        rs = af;
+    var cf = typeof self == "object" && self && self.Object === Object && self,
+        ff = rs || cf || Function("return this")(),
+        xn = ff;
+    var hf = function() {
+            return xn.Date.now()
         },
-        kn = nf;
-    var rf = /\s/;
+        Sn = hf;
+    var df = /\s/;
 
-    function of(n) {
-        for (var e = n.length; e-- && rf.test(n.charAt(e)););
+    function uf(n) {
+        for (var e = n.length; e-- && df.test(n.charAt(e)););
         return e
     }
-    var rs = of;
-    var sf = /^\s+/;
+    var is = uf;
+    var pf = /^\s+/;
 
-    function lf(n) {
-        return n && n.slice(0, rs(n) + 1).replace(sf, "")
+    function mf(n) {
+        return n && n.slice(0, is(n) + 1).replace(pf, "")
     }
-    var is = lf;
-    var af = bn.Symbol,
-        ot = af;
-    var ss = Object.prototype,
-        cf = ss.hasOwnProperty,
-        ff = ss.toString,
+    var ss = mf;
+    var gf = xn.Symbol,
+        ot = gf;
+    var ls = Object.prototype,
+        yf = ls.hasOwnProperty,
+        bf = ls.toString,
         vt = ot ? ot.toStringTag : void 0;
 
-    function hf(n) {
-        var e = cf.call(n, vt),
+    function kf(n) {
+        var e = yf.call(n, vt),
             t = n[vt];
         try {
             n[vt] = void 0;
             var r = !0
         } catch (o) {}
-        var i = ff.call(n);
+        var i = bf.call(n);
         return r && (e ? n[vt] = t : delete n[vt]), i
     }
-    var ls = hf;
-    var df = Object.prototype,
-        uf = df.toString;
+    var as = kf;
+    var xf = Object.prototype,
+        Sf = xf.toString;
+
+    function Mf(n) {
+        return Sf.call(n)
+    }
+    var cs = Mf;
+    var Cf = "[object Null]",
+        Of = "[object Undefined]",
+        fs = ot ? ot.toStringTag : void 0;
 
-    function pf(n) {
-        return uf.call(n)
-    }
-    var as = pf;
-    var mf = "[object Null]",
-        gf = "[object Undefined]",
-        cs = ot ? ot.toStringTag : void 0;
-
-    function yf(n) {
-        return n == null ? n === void 0 ? gf : mf : cs && cs in Object(n) ? ls(n) : as(n)
+    function wf(n) {
+        return n == null ? n === void 0 ? Of : Cf : fs && fs in Object(n) ? as(n) : cs(n)
     }
-    var fs = yf;
+    var hs = wf;
 
-    function bf(n) {
+    function Nf(n) {
         return n != null && typeof n == "object"
     }
-    var hs = bf;
-    var kf = "[object Symbol]";
+    var ds = Nf;
+    var Tf = "[object Symbol]";
 
-    function xf(n) {
-        return typeof n == "symbol" || hs(n) && fs(n) == kf
+    function Df(n) {
+        return typeof n == "symbol" || ds(n) && hs(n) == Tf
     }
-    var ds = xf;
-    var us = NaN,
-        Sf = /^[-+]0x[0-9a-f]+$/i,
-        Mf = /^0b[01]+$/i,
-        Cf = /^0o[0-7]+$/i,
-        Of = parseInt;
+    var us = Df;
+    var ps = NaN,
+        Ef = /^[-+]0x[0-9a-f]+$/i,
+        Af = /^0b[01]+$/i,
+        If = /^0o[0-7]+$/i,
+        Rf = parseInt;
 
-    function wf(n) {
+    function vf(n) {
         if (typeof n == "number") return n;
-        if (ds(n)) return us;
-        if (At(n)) {
+        if (us(n)) return ps;
+        if (Rt(n)) {
             var e = typeof n.valueOf == "function" ? n.valueOf() : n;
-            n = At(e) ? e + "" : e
+            n = Rt(e) ? e + "" : e
         }
         if (typeof n != "string") return n === 0 ? n : +n;
-        n = is(n);
-        var t = Mf.test(n);
-        return t || Cf.test(n) ? Of(n.slice(2), t ? 2 : 8) : Sf.test(n) ? us : +n
-    }
-    var Pr = wf;
-    var Nf = "Expected a function",
-        Tf = Math.max,
-        Df = Math.min;
+        n = ss(n);
+        var t = Af.test(n);
+        return t || If.test(n) ? Rf(n.slice(2), t ? 2 : 8) : Ef.test(n) ? ps : +n
+    }
+    var zr = vf;
+    var Pf = "Expected a function",
+        zf = Math.max,
+        Bf = Math.min;
 
-    function Ef(n, e, t) {
+    function Ff(n, e, t) {
         var r, i, o, s, l, a, c = 0,
             f = !1,
             h = !1,
             d = !0;
-        if (typeof n != "function") throw new TypeError(Nf);
-        e = Pr(e) || 0, At(t) && (f = !!t.leading, h = "maxWait" in t, o = h ? Tf(Pr(t.maxWait) || 0, e) : o, d = "trailing" in t ? !!t.trailing : d);
+        if (typeof n != "function") throw new TypeError(Pf);
+        e = zr(e) || 0, Rt(t) && (f = !!t.leading, h = "maxWait" in t, o = h ? zf(zr(t.maxWait) || 0, e) : o, d = "trailing" in t ? !!t.trailing : d);
 
         function p(C) {
             var H = r,
                 ne = i;
             return r = i = void 0, c = C, s = n.apply(ne, H), s
         }
 
@@ -9309,97 +9371,97 @@
             return c = C, l = setTimeout(k, e), f ? p(C) : s
         }
 
         function m(C) {
             var H = C - a,
                 ne = C - c,
                 M = e - H;
-            return h ? Df(M, o - ne) : M
+            return h ? Bf(M, o - ne) : M
         }
 
         function g(C) {
             var H = C - a,
                 ne = C - c;
             return a === void 0 || H >= e || H < 0 || h && ne >= o
         }
 
         function k() {
-            var C = kn();
+            var C = Sn();
             if (g(C)) return N(C);
             l = setTimeout(k, m(C))
         }
 
         function N(C) {
             return l = void 0, d && r ? p(C) : (r = i = void 0, s)
         }
 
         function ee() {
             l !== void 0 && clearTimeout(l), c = 0, r = a = i = l = void 0
         }
 
         function te() {
-            return l === void 0 ? s : N(kn())
+            return l === void 0 ? s : N(Sn())
         }
 
         function U() {
-            var C = kn(),
+            var C = Sn(),
                 H = g(C);
             if (r = arguments, i = this, a = C, H) {
                 if (l === void 0) return u(a);
                 if (h) return clearTimeout(l), l = setTimeout(k, e), p(a)
             }
             return l === void 0 && (l = setTimeout(k, e)), s
         }
         return U.cancel = ee, U.flush = te, U
     }
-    var ps = Ef;
+    var ms = Ff;
     var Q = (n, e = null) => {
         let t = document.createElement(n);
         if (e)
             for (let [r, i] of Object.entries(e)) /^data-|^aria-|^role/.test(r) ? t.setAttribute(r, i) : t[r] = i;
         return t
     };
 
-    function xn(n, e) {
+    function Mn(n, e) {
         let t = document.createElement("article");
-        return t.innerHTML = e, Re.fromSchema(n).parse(t)
+        return t.innerHTML = e, ve.fromSchema(n).parse(t)
     }
 
-    function ms(n) {
+    function gs(n) {
         let {
             schema: e,
             doc: t
-        } = n.state, r = ie.fromSchema(e), i = Q("article");
+        } = n, r = ie.fromSchema(e), i = Q("article");
         return i.appendChild(r.serializeFragment(t.content)), i.innerHTML
     }
-    var gs = (n, e, t) => {
-        let r = ie.fromSchema(n),
-            i = function() {
-                let s = Q("article");
-                return s.appendChild(r.serializeFragment(e.state.doc.content)), s.innerHTML
-            };
-        return ps(() => {
-            let s = i();
-            t.value !== s && (t.value = s, t.dispatchEvent(new InputEvent("input", {
-                bubbles: !0,
-                cancelable: !0
-            })))
-        }, 250)
-    };
-    var ys = n => {
-        let {
-            $from: e,
-            $to: t
-        } = n, r = e.pos, i = t.pos, o = e.nodeAfter, s = t.nodeBefore, l = o && o.isText ? /^\s*/.exec(o.text)[0].length : 0, a = s && s.isText ? /\s*$/.exec(s.text)[0].length : 0;
-        return r + l < i && (r += l, i -= a), {
-            from: r,
-            to: i
-        }
-    };
-    var If = n => new Promise(e => {
+    var ys = (n, e, t) => {
+            let r = ie.fromSchema(n),
+                i = function() {
+                    let s = Q("article");
+                    return s.appendChild(r.serializeFragment(e.state.doc.content)), s.innerHTML
+                };
+            return ms(() => {
+                let s = i();
+                t.value !== s && (t.value = s, t.dispatchEvent(new InputEvent("input", {
+                    bubbles: !0,
+                    cancelable: !0
+                })))
+            }, 250)
+        },
+        bs = n => {
+            let {
+                $from: e,
+                $to: t
+            } = n, r = e.pos, i = t.pos, o = e.nodeAfter, s = t.nodeBefore, l = o && o.isText ? /^\s*/.exec(o.text)[0].length : 0, a = s && s.isText ? /\s*$/.exec(s.text)[0].length : 0;
+            return r + l < i && (r += l, i -= a), {
+                from: r,
+                to: i
+            }
+        };
+    var Vf = n => new Promise(e => {
             let t = document.createElement("div");
             t.innerHTML = `
   <dialog class="prose-editor-dialog">
   <form>
   <p><label>URL</label> <input type="url" name="href" size="50" required></p>
   <p><label>Title</label> <input type="text" name="title" size="50"></p>
   <button type="submit">Update</button>
@@ -9424,41 +9486,41 @@
             let {
                 $from: t,
                 empty: r
             } = n.selection, i = n.schema.marks.link;
             if (r && !i.isInSet(t.marks())) return !1;
             if (e) {
                 let o = t.marks().find(s => s.type === i);
-                If((o == null ? void 0 : o.attrs) || {}).then(s => {
+                Vf((o == null ? void 0 : o.attrs) || {}).then(s => {
                     if (s) {
                         let l;
-                        if (r) l = vr(t, i), e(n.tr.removeMark(l.from, l.to, i).addMark(l.from, l.to, i.create(s)));
+                        if (r) l = Pr(t, i), e(n.tr.removeMark(l.from, l.to, i).addMark(l.from, l.to, i.create(s)));
                         else {
                             let {
                                 from: a,
                                 to: c
-                            } = ys(n.selection);
+                            } = bs(n.selection);
                             e(n.tr.addMark(a, c, i.create(s)))
                         }
                     }
                 })
             }
             return !0
         },
-        bs = (n, e) => {
+        ks = (n, e) => {
             let t = n.schema.marks.link,
                 {
                     $from: r,
                     from: i,
                     to: o
                 } = n.selection,
-                s = vr(r, t);
+                s = Pr(r, t);
             return s && s.from <= i && s.to >= o ? (e && e(n.tr.removeMark(s.from, s.to, t)), !0) : !1
         },
-        Af = n => new Promise(e => {
+        Lf = n => new Promise(e => {
             let t = document.createElement("div");
             t.innerHTML = `
   <dialog class="prose-editor-dialog">
   <form>
   <p><textarea name="html" cols="80" rows="30"></textarea></p>
   <button type="submit">Update</button>
   <button type="button" value="cancel">Cancel</button>
@@ -9471,63 +9533,63 @@
                 r.close()
             }), r.addEventListener("close", () => {
                 t.remove(), e(null)
             }), t.querySelector("button[type=submit]").addEventListener("click", o => {
                 o.preventDefault(), i.reportValidity() && (t.remove(), e(i.html.value))
             }), r.showModal()
         }),
-        ks = n => (e, t) => (t && Af(ms(n)).then(r => {
-            if (r) {
-                let i = xn(n.state.schema, r);
-                t(e.tr.replaceWith(0, e.tr.doc.content.size, i))
+        xs = (n, e) => (e && Lf(gs(n)).then(t => {
+            if (t) {
+                let r = Mn(n.schema, t);
+                e(n.tr.replaceWith(0, n.tr.doc.content.size, r))
             }
         }), !0),
-        xs = (n, e) => (e && e(n.tr.replaceSelectionWith(n.schema.nodes.horizontal_rule.create())), !0);
-    var Ss = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : !1,
-        Ms = n => (e, t) => {
+        Ss = (n, e) => (e && e(n.tr.replaceSelectionWith(n.schema.nodes.horizontal_rule.create())), !0);
+    var Ms = typeof navigator != "undefined" ? /Mac|iP(hone|[oa]d)/.test(navigator.platform) : !1,
+        Cs = n => (e, t) => {
             n[e] = t
         };
 
-    function Cs(n) {
+    function Os(n) {
         let e = {},
-            t, r = Ms(e);
-        if (r("Mod-z", Et), r("Shift-Mod-z", rt), r("Backspace", ts), Ss || r("Mod-y", rt), t = n.nodes.hard_break) {
+            t, r = Cs(e);
+        if (r("Mod-z", At), r("Shift-Mod-z", rt), r("Backspace", ns), Ms || r("Mod-y", rt), t = n.nodes.hard_break) {
             let i = t,
-                o = xt($n, (s, l) => (l && l(s.tr.replaceSelectionWith(i.create()).scrollIntoView()), !0));
-            r("Mod-Enter", o), r("Shift-Enter", o), Ss && r("Ctrl-Enter", o)
+                o = xt(jn, (s, l) => (l && l(s.tr.replaceSelectionWith(i.create()).scrollIntoView()), !0));
+            r("Mod-Enter", o), r("Shift-Enter", o), Ms && r("Ctrl-Enter", o)
         }
-        return (t = n.nodes.bullet_list) && r("Shift-Ctrl-8", it(t)), (t = n.nodes.ordered_list) && r("Shift-Ctrl-9", it(t)), (t = n.nodes.blockquote) && r("Ctrl->", Zt(t)), (t = n.nodes.list_item) && (r("Enter", Qo(t)), r("Mod-[", Zo(t)), r("Mod-]", es(t))), e
+        return (t = n.nodes.bullet_list) && r("Shift-Ctrl-8", it(t)), (t = n.nodes.ordered_list) && r("Shift-Ctrl-9", it(t)), (t = n.nodes.blockquote) && r("Ctrl->", en(t)), (t = n.nodes.list_item) && (r("Enter", Zo(t)), r("Mod-[", es(t)), r("Mod-]", ts(t))), e
     }
 
-    function Os(n) {
+    function ws(n) {
         let e = {},
-            t, r = Ms(e);
+            t, r = Cs(e);
         return (t = n.marks.strong) && (r("Mod-b", G(t)), r("Mod-B", G(t))), (t = n.marks.em) && (r("Mod-i", G(t)), r("Mod-I", G(t))), (t = n.marks.underline) && (r("Mod-Shift-u", G(t)), r("Mod-Shift-U", G(t))), (t = n.marks.strikethrough) && (r("Mod-Shift-s", G(t)), r("Mod-Shift-S", G(t))), (t = n.marks.link) && (r("Mod-k", st), r("Mod-K", st)), e
     }
 
-    function ws(n) {
+    function Ns(n) {
         return new J({
             view(e) {
-                let t = new Rr(n, e);
+                let t = new Br(n, e);
                 return e.dom.parentNode.insertBefore(t.dom, e.dom), t
             }
         })
     }
 
-    function Ns(n, e) {
+    function Ts(n, e) {
         let {
             from: t,
             $from: r,
             to: i,
             empty: o
         } = n.selection;
         return o ? e.isInSet(n.storedMarks || r.marks()) : n.doc.rangeHasMark(t, i, e)
     }
 
-    function vf(n) {
+    function Jf(n) {
         let e = Q("span", {
             className: "prose-menubar__button prose-menubar__button--heading"
         });
         return e.append(Q("span", {
             className: "material-icons",
             textContent: "title",
             title: `heading ${n}`
@@ -9541,110 +9603,113 @@
         return Q("span", {
             className: "prose-menubar__button material-icons",
             textContent: n,
             title: e
         })
     }
 
-    function Ts(n) {
+    function Ds(n) {
+        if (!n.nodes.heading) return [];
         let e = t => ({
-            command: Hn(n.nodes.heading, {
+            command: _n(n.nodes.heading, {
                 level: t
             }),
-            dom: vf(t),
+            dom: Jf(t),
             active(r) {
                 return r.selection.$from.parent.hasMarkup(n.nodes.heading, {
                     level: t
                 })
             }
         });
         return [e(1), e(2), e(3), {
-            command: Hn(n.nodes.paragraph),
+            command: _n(n.nodes.paragraph),
             dom: Z("notes", "paragraph"),
             active(t) {
                 return t.selection.$from.parent.hasMarkup(n.nodes.paragraph)
             }
         }]
     }
 
-    function Ds(n) {
-        return [{
-            command: it(n.nodes.bullet_list),
+    function Es(n) {
+        let e = [],
+            t;
+        return (t = n.nodes.bullet_list) && e.push({
+            command: it(t),
             dom: Z("format_list_bulleted", "unordered list"),
-            active(e) {
+            active(r) {
                 return !1
             }
-        }, {
-            command: it(n.nodes.ordered_list),
+        }), (t = n.nodes.ordered_list) && e.push({
+            command: it(t),
             dom: Z("format_list_numbered", "ordered list"),
-            active(e) {
+            active(r) {
                 return !1
             }
-        }, {
-            command: Zt(n.nodes.blockquote),
+        }), (t = n.nodes.blockquote) && e.push({
+            command: en(t),
             dom: Z("format_quote", "blockquote"),
-            active(e) {
+            active(r) {
                 return !1
             }
-        }, {
-            command: xs,
+        }), (t = n.nodes.horizontal_rule) && e.push({
+            command: Ss,
             dom: Z("horizontal_rule", "horizontal rule"),
-            active(e) {
+            active(r) {
                 return !1
             }
-        }]
+        }), e
     }
 
-    function Es(n) {
-        let e = (t, r, i) => ({
+    function As(n) {
+        let e = (t, r, i) => t ? {
             command: G(t),
             dom: Z(r, i),
-            active: o => Ns(o, t)
-        });
-        return [e(n.marks.strong, "format_bold", "bold"), e(n.marks.em, "format_italic", "italic"), e(n.marks.underline, "format_underline", "underline"), e(n.marks.strikethrough, "format_strikethrough", "strikethrough"), e(n.marks.sub, "subscript", "subscript"), e(n.marks.sup, "superscript", "superscript")]
+            active: o => Ts(o, t)
+        } : null;
+        return [e(n.marks.strong, "format_bold", "bold"), e(n.marks.em, "format_italic", "italic"), e(n.marks.underline, "format_underline", "underline"), e(n.marks.strikethrough, "format_strikethrough", "strikethrough"), e(n.marks.sub, "subscript", "subscript"), e(n.marks.sup, "superscript", "superscript")].filter(Boolean)
     }
 
     function Is(n) {
-        return [{
+        return n.marks.link ? [{
             command: st,
             dom: Z("insert_link", "insert link"),
             active: st
         }, {
-            command: bs,
+            command: ks,
             dom: Z("link_off", "remove link"),
-            active: e => Ns(e, n.marks.link)
-        }]
+            active: e => Ts(e, n.marks.link)
+        }] : []
     }
 
-    function As() {
+    function Rs() {
         return [{
-            command: Et,
+            command: At,
             dom: Z("undo", "undo"),
             active(n) {
                 return !1
             }
         }, {
             command: rt,
             dom: Z("redo", "redo"),
             active(n) {
                 return !1
             }
         }]
     }
 
-    function Pf(n) {
+    function vs() {
         return [{
-            command: ks(n),
+            command: xs,
             dom: Z("code", "edit HTML"),
             active: () => !1
         }]
     }
-    var Rr = class {
+    var Br = class {
         constructor(e, t) {
-            e.push(Pf(t)), this.items = e.flatMap(r => r), this.editorView = t, this.dom = Q("div", {
+            this.items = e.flatMap(r => r), this.editorView = t, this.dom = Q("div", {
                 className: "prose-menubar"
             }), e.filter(r => r.length).forEach(r => {
                 let i = Q("div", {
                     className: "prose-menubar__group"
                 });
                 this.dom.append(i), r.forEach(({
                     dom: o
@@ -9669,37 +9734,57 @@
             })
         }
         destroy() {
             this.dom.remove()
         }
     };
 
-    function vs() {
+    function Ps() {
         return new J({
             view(n) {
-                return new zr(n)
+                return new Fr(n)
             }
         })
     }
-    var zr = class {
+    var Fr = class {
         constructor(e) {
             this.editorView = e, this.editorView.dom.setAttribute("spellcheck", "false")
         }
         update() {}
         destroy() {
             this.editorView.dom.removeAttribute("spellcheck")
         }
     };
-    var Rf = ["u", 0],
-        zf = ["s", 0],
-        Bf = ["sub", 0],
-        Ff = ["sup", 0];
+    var Wf = ["u", 0],
+        qf = ["s", 0],
+        Kf = ["sub", 0],
+        $f = ["sup", 0],
+        Hf = (n, e = null) => {
+            if (!e) return n;
+            let t = ["doc", "paragraph", "text", ...e],
+                r = {},
+                i = {};
+            return n.spec.nodes.forEach((o, s) => {
+                t.includes(o) && (r[o] = s)
+            }), n.spec.marks.forEach((o, s) => {
+                t.includes(o) && (i[o] = s)
+            }), new ke({
+                nodes: r,
+                marks: i
+            })
+        },
+        jf = {
+            html: !0,
+            history: !0,
+            types: null
+        };
 
-    function Ps(n) {
-        let e = {
+    function zs(n, e = {}) {
+        e = e || jf;
+        let t = {
                 nodes: {
                     doc: ye.doc,
                     paragraph: ye.paragraph,
                     heading: ye.heading,
                     blockquote: ye.blockquote,
                     horizontal_rule: ye.horizontal_rule,
                     text: ye.text,
@@ -9712,68 +9797,68 @@
                     underline: {
                         parseDOM: [{
                             tag: "u"
                         }, {
                             style: "text-decoration:underline"
                         }],
                         toDOM() {
-                            return Rf
+                            return Wf
                         }
                     },
                     strikethrough: {
                         parseDOM: [{
                             tag: "s"
                         }, {
                             style: "text-decoration:line-through"
                         }],
                         toDOM() {
-                            return zf
+                            return qf
                         }
                     },
                     sub: {
                         parseDOM: [{
                             tag: "sub"
                         }],
                         toDOM() {
-                            return Bf
+                            return Kf
                         }
                     },
                     sup: {
                         parseDOM: [{
                             tag: "sup"
                         }],
                         toDOM() {
-                            return Ff
+                            return $f
                         }
                     }
                 }
             },
-            t = new Pe(e);
-        t = new Pe({
-            nodes: Xo(t.spec.nodes, "paragraph block*", "block"),
-            marks: t.spec.marks
-        });
-        let r = Sn(Sn({}, Cs(t)), Os(t)),
-            i = [Un(r), Un(vi), Pi(), Ho(), Yo(), ws([Ts(t), Ds(t), Is(t), Es(t), As()]), vs()],
-            o = Q("div", {
+            r = new ke(t);
+        r = new ke({
+            nodes: Qo(r.spec.nodes, "paragraph block*", "block"),
+            marks: r.spec.marks
+        }), r = Hf(r, e.types);
+        let i = Cn(Cn({}, Os(r)), ws(r)),
+            o = [Yn(i), Yn(Pi), zi(), _o(), Xo(), Ns([Ds(r), Es(r), Is(r), As(r), e.history ? Rs() : null, e.html ? vs() : null].filter(Boolean)), Ps()],
+            s = Q("div", {
                 className: "prose-editor"
             });
-        n.before(o);
-        let s = new dn(o, {
-                state: Qt.create({
-                    doc: xn(t, n.value),
-                    plugins: i
+        n.before(s);
+        let l = new un(s, {
+                state: Zt.create({
+                    doc: Mn(r, n.value),
+                    plugins: o
                 }),
-                dispatchTransaction: a => {
-                    s.updateState(s.state.apply(a)), l()
+                dispatchTransaction: c => {
+                    l.updateState(l.state.apply(c)), a()
                 }
             }),
-            l = gs(t, s, n);
+            a = ys(r, l, n);
         return () => {
             try {
-                s.destroy()
-            } catch (a) {}
-            o.remove()
+                l.destroy()
+            } catch (c) {}
+            s.remove()
         }
     }
-    return Js(Vf);
+    return qs(_f);
 })();
```

### Comparing `django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/init.js` & `django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/init.js`

 * *Files 7% similar despite different names*

#### js-beautify {}

```diff
@@ -1,15 +1,15 @@
 /* global DjangoProseEditor */
 
 const marker = "data-django-prose-editor"
 
 function initializeDjangoProseEditor(container) {
     for (let el of container.querySelectorAll(`[${marker}]`)) {
         if (!el.id.includes("__prefix__")) {
-            DjangoProseEditor.createEditor(el)
+            DjangoProseEditor.createEditor(el, JSON.parse(el.getAttribute(marker)))
             el.removeAttribute(marker)
         }
     }
 }
 
 function initializeDjangoInlines() {
     let o
```

### Comparing `django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/material-icons.css` & `django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/material-icons.css`

 * *Files identical despite different names*

### Comparing `django_prose_editor-0.2.1/django_prose_editor/static/django_prose_editor/material-icons.woff2` & `django_prose_editor-0.3.0/django_prose_editor/static/django_prose_editor/material-icons.woff2`

 * *Files identical despite different names*

### Comparing `django_prose_editor-0.2.1/LICENSE` & `django_prose_editor-0.3.0/LICENSE`

 * *Files identical despite different names*

### Comparing `django_prose_editor-0.2.1/README.rst` & `django_prose_editor-0.3.0/README.rst`

 * *Files 18% similar despite different names*

```diff
@@ -52,25 +52,16 @@
 ~~~~~~~~
 
 ProseMirror does a really good job of only allowing content which confirms to a
 particular scheme. Of course users can submit what they want, they are not
 constrainted by the HTML widgets you're using. You should still always sanitize
 the HTML submitted on the server side. A good way to do this is by using the
 ``sanitize`` argument to the ``ProseEditorField``. You can use the following
-snippet to always pass HTML through `html-sanitizer
-<https://github.com/matthiask/html-sanitizer>`__:
-
-.. code-block:: python
-
-    from html_sanitizer.django import get_sanitizer
-
-    description = ProseEditorField(sanitize=get_sanitizer().sanitize)
-
-You can also use the following, which uses a sanitizer instance which allows
-all elements which are allowed by the ProseMirror configuration:
+snippet to always pass HTML through `nh3
+<https://nh3.readthedocs.io/en/latest/>`__:
 
 .. code-block:: python
 
     from django_prose_editor.sanitized import SanitizedProseEditorField
 
     description = SanitizedProseEditorField()
 
@@ -84,8 +75,30 @@
 content. The name would be ``Project.get_description_excerpt`` in the example
 above.
 
 
 Customization
 ~~~~~~~~~~~~~
 
-It's not possible (yet), sorry.
+It's possible to slightly customize the field or widget by passing an optional
+``config`` dictionary. The default configuration is:
+
+.. code-block:: python
+
+    config = {
+        "types": None,    # Allow all nodes and marks
+        "history": True,  # Enable undo and redo
+        "html": True,     # Add a button which allows editing the raw HTML
+    }
+
+If you only want to support paragraphs, strong, emphasis, sub- and superset and
+no history or HTML editing you could add the following field:
+
+.. code-block:: python
+
+    text = SanitizedProseEditorField(
+        config={"types": ["strong", "em", "sub", "sup"]},
+    )
+
+Paragraphs cannot be removed at the moment. Note that the backend doesn't
+sanitize the content to ensure that the HTML doesn't contain only the provided
+tags, that's out of scope for now.
```

### Comparing `django_prose_editor-0.2.1/pyproject.toml` & `django_prose_editor-0.3.0/pyproject.toml`

 * *Files 2% similar despite different names*

```diff
@@ -30,15 +30,15 @@
 dependencies = [
     "Django>=4.2",
 ]
 
 [project.optional-dependencies]
 tests = [
     "coverage",
-    "html-sanitizer",
+    "nh3",
 ]
 
 [project.urls]
 Homepage = "https://github.com/matthiask/django-prose-editor/"
 
 [tool.hatch.build]
 include = ["django_prose_editor/"]
```

### Comparing `django_prose_editor-0.2.1/PKG-INFO` & `django_prose_editor-0.3.0/PKG-INFO`

 * *Files 20% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: django-prose-editor
-Version: 0.2.1
+Version: 0.3.0
 Summary: Prose editor for the Django admin based on ProseMirror
 Project-URL: Homepage, https://github.com/matthiask/django-prose-editor/
 Author-email: Matthias Kestenholz <mk@feinheit.ch>
 License-Expression: BSD-3-Clause
 License-File: LICENSE
 Classifier: Environment :: Web Environment
 Classifier: Framework :: Django
@@ -19,15 +19,15 @@
 Classifier: Topic :: Internet :: WWW/HTTP :: Dynamic Content
 Classifier: Topic :: Software Development
 Classifier: Topic :: Software Development :: Libraries :: Application Frameworks
 Requires-Python: >=3.11
 Requires-Dist: django>=4.2
 Provides-Extra: tests
 Requires-Dist: coverage; extra == 'tests'
-Requires-Dist: html-sanitizer; extra == 'tests'
+Requires-Dist: nh3; extra == 'tests'
 Description-Content-Type: text/x-rst
 
 ===================
 django-prose-editor
 ===================
 
 Prose editor for the Django admin based on ProseMirror. `Announcement blog post <https://406.ch/writing/django-prose-editor-prose-editing-component-for-the-django-admin/>`__.
@@ -80,25 +80,16 @@
 ~~~~~~~~
 
 ProseMirror does a really good job of only allowing content which confirms to a
 particular scheme. Of course users can submit what they want, they are not
 constrainted by the HTML widgets you're using. You should still always sanitize
 the HTML submitted on the server side. A good way to do this is by using the
 ``sanitize`` argument to the ``ProseEditorField``. You can use the following
-snippet to always pass HTML through `html-sanitizer
-<https://github.com/matthiask/html-sanitizer>`__:
-
-.. code-block:: python
-
-    from html_sanitizer.django import get_sanitizer
-
-    description = ProseEditorField(sanitize=get_sanitizer().sanitize)
-
-You can also use the following, which uses a sanitizer instance which allows
-all elements which are allowed by the ProseMirror configuration:
+snippet to always pass HTML through `nh3
+<https://nh3.readthedocs.io/en/latest/>`__:
 
 .. code-block:: python
 
     from django_prose_editor.sanitized import SanitizedProseEditorField
 
     description = SanitizedProseEditorField()
 
@@ -112,8 +103,30 @@
 content. The name would be ``Project.get_description_excerpt`` in the example
 above.
 
 
 Customization
 ~~~~~~~~~~~~~
 
-It's not possible (yet), sorry.
+It's possible to slightly customize the field or widget by passing an optional
+``config`` dictionary. The default configuration is:
+
+.. code-block:: python
+
+    config = {
+        "types": None,    # Allow all nodes and marks
+        "history": True,  # Enable undo and redo
+        "html": True,     # Add a button which allows editing the raw HTML
+    }
+
+If you only want to support paragraphs, strong, emphasis, sub- and superset and
+no history or HTML editing you could add the following field:
+
+.. code-block:: python
+
+    text = SanitizedProseEditorField(
+        config={"types": ["strong", "em", "sub", "sup"]},
+    )
+
+Paragraphs cannot be removed at the moment. Note that the backend doesn't
+sanitize the content to ensure that the HTML doesn't contain only the provided
+tags, that's out of scope for now.
```

